...
from trend_app_protect.compat import string_types, to_bytes
from trend_app_protect.exceptions import ConfigError
from trend_app_protect.logger import log


CONFIG_FILENAME = "trend_app_protect.ini"
SECTION_NAME = "trend_app_protect"
...
class Config(object):
    def __init__(self, defaults=None, autoload=True):
        ...
        self._config = defaults or {
            "plugins_active": ",".join(DEFAULT_PLUGINS)
        }
        ...
        if autoload:
            ...
            if config_file:
                ...
                log.debug("Configuration after loading "
                          "from file: %(config)s", {
                              "config": self._config,
                          })
            else:
                log.info("Can't find config file - using defaults and "
    ...
    def load_ini_config(self, filename, content):
        ...
        if parser.has_section(SECTION_NAME):
            self._config.update(dict(parser.items(SECTION_NAME)))
    ...
    def get(self, name, default=None, datatype=None):
        ...
        """
        environ_name = "TREND_AP_%s" % name.upper()
