...
from module.sonos_extras.sonos_extras import SonosExtrasCliHelper
from module.bin import SonosExtrasCLI
...
def run_command(args):
    return sh.python("-m", "module.bin.SonosExtrasCLI", args).__str__()
...
def test_set_volume():
    ...
    vol = run_command(["v", current_volume - 1])
    assert int(vol) == current_volume - 1
    vol = run_command(["volume", current_volume])
