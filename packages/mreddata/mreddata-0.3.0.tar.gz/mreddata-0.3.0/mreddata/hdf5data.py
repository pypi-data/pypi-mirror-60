...
#TODO: Add attributes dict
	...
	def _getHistogram(self, filename,  histogramName=None, diff=None):
		...
		try:
			...
			gfu = self.__attrs[filename]['gfu']  	#TODO: Think about generalizing this to any naming convention for nIons/gfu
			nIons = self.__attrs[filename]['nIons']
			histogram = Histogram(filename = filename, histname = histogramName, df = df, gfu = gfu, nIons=nIons)
				...
				with hp.File(filename, 'r') as f:
					...
					for key, bounds in self.__nameMap.items():
						if filename in key:
							...
							histogram = Histogram(filename = filename, histname = key.split(" - ")[1], df=df, gfu = gfu, nIons=nIons)
