...
class TxtData(_HistogramList):
	def __init__(self):
		...
		for filename in options.files:
			...
			nIons =list(df[0])[-2] 
			gfu = list(df[0])[-1].split("]")[0]####TODO: include option for having the gfu and nIons at the bottom of the file, or remove this functionality
			...
			self.histogramsDict[filename + " - data"].nIons = int(nIons)
