...
class Histogram:
	...
	def revInt(self):#, update=False):
		...
		try:
			oldDf = self.df.loc[:]
			dff = self.df.loc[:, ('y', 'y2', 'n')][::-1].cumsum()[::-1]
			dff.columns = ['y', 'y2', 'cumn']
			oldDf.update(dff)
			oldDf['cumn'] = dff['cumn']
			self.df = oldDf
			del oldDf, dff
		...
		for h in histograms:
			if (h.df[:len(h.df) - 1][['x', 'w', 'edges']] == newHist.df[:len(newHist.df) - 1][['x', 'w', 'edges']]).all().all():#check that bins match; don't include overflow (null value in edges causes check to fail)
				if 'y' in newHist.df.columns:
					if 'cumn' in newHist.df.columns:##TODO: Have method that simply counts the number of adds without columns, and then renormalizes the 'x', 'w', and 'edges' columns by number of combinations? 
						newHist.df[['y', 'y2', 'xy', 'x2y', 'n', 'cumn']] += h.df.loc[:, ('y', 'y2', 'xy', 'x2y', 'n', 'cumn')] 
					else:
