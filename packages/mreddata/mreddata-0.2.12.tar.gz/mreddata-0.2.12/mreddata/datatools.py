		...
		for h in histograms:
			if (h.df[:len(h.df) - 1][['x', 'w', 'edges']] == newHist.df[:len(newHist.df) - 1][['x', 'w', 'edges']]).all().all():#check that bins match; don't include overflow (null value in edges causes check to fail)
				if 'y' in newHist.df.columns:
					if 'cumn' in h.df.columns:##TODO: Have method that simply counts the number of adds without columns, and then renormalizes the 'x', 'w', and 'edges' columns by number of combinations? 
