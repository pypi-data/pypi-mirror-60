...
# \class Histogram
class Histogram:
	def __init__(self, histname, filename, df = None, label=None, color=None, dashes = (None, None), sortOrder=None, gfu=1, nIons=0):
	...
	def setDF(self, df, yOnly =True):
		if type(df) != type(None):
			if yOnly:
				...
				dfnorm = df.loc[:, ('y_raw')]/(self.gfu * self.nIons)
				dfnorm.columns = ['y_norm']
				df = pd.concat([df, dfnorm], axis=1)
				del dfnorm
				dfnorm = df.loc[:, ('y_norm')][::-1].cumsum()[::-1]
				dfnorm.columns = ['y_int']
				df = pd.concat([df, dfnorm], axis=1)
				del dfnorm
				dfnorm = df.loc[:, ('y_norm')] / df.loc[:, ('w')]
				dfnorm.columns = ['y_diff']
				df = pd.concat([df, dfnorm], axis=1)
				del dfnorm

				#df['y_norm'] = df.loc[:, ('y_raw')]/(self.gfu * self.nIons)
				#df['y_int'] = df.loc[:, ('y_norm')][::-1].cumsum()[::-1]
				#df['y_diff'] = df.loc[:, ('y_norm')] / df.loc[:, ('w')]
				df['y_diff'][0] = 0
				df = df.fillna(0)
				self.df = df
				self._getTotalDose()
	...
	def _getTotalDose(self, weighted=False):####Need way of converting from revInt back to diff/raw for calculating total dose. 
