...
class AuthHandler(RequestHandler):
    ...
    def get(self, path=''):
        ...
        res = requests.get('https://api.github.com/user?access_token={0}'.format(access_token), headers=headers)
        user = res.json()['login']
...
class GistShareHandler(APIHandler):
    ...
    def post(self, path=''):
        ...
        gist = {
            "description": "Created on Cognitive Class Labs",
            "public": True,
            "files": {}
        }
        ...
        if not c.access_token:
            ...
            res = requests.post('https://api.github.com/gists?access_token={0}'.format(c.access_token), json=gist)
