...
class PixelBoardConnector(board_connector.BoardConnector):
    ...
    def sensing_tokens(self, token_type=None, distance: int = 1) -> list:
            ...
            tokens = self.board.get_tokens_at_rect(destination_rect, singleitem=False, exclude=self.token, token_type=token_type)
    ...
    def sensing_token(self, token_type=None, distance: int = 1) -> Union[token.Token, None]:
            ...
            token = self.board.get_tokens_at_rect(destination_rect, singleitem=True, exclude=self.token, token_type=token_type)
            if token:
