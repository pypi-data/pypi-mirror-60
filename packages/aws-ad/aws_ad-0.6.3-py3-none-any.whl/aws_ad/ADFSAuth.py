...
auth file parameters take precedence over environmental variables
    ...
    def log_args_dict(self, info, dict_like):
        ...
        for k, v in list(dict_like.items()):
            if k == 'password':
    ...
    def get_auth_file_parameters(self):
        ...
        if self.profile in config:
            ...
            r = {}
            for key, value in list(config[self.profile].items()):
    ...
    def get_parameters(self):
        ...
        for k, v in list(cmd_line_args.items()):
            if v is not None:
                all_args[k] = v
    ...
    def pick_role(self):
        ...
        if len(roles) > 1:
            ...
            for role in roles:
                ...
                i += 1
            role_nr = int(eval(input("Choose number of role: ")))
