...
class train(object):
    ...
    def __init__(
        ...
        targets = [torch.tensor(target, requires_grad=False) for target in targets]
        ...
        logger.info(
            "{:6s} {:19s} {:12s} {:8s} {:8s}".format(
                "Epoch", "Time Stamp", "Loss", "MAE/img", "MAE/atom"
            )
        )
        logger.info(
            "{:6s} {:19s} {:12s} {:8s} {:8s}".format(
    ...
    def trainer(self, client):
        ...
        while not converged:
            ...
            # MSE per image and per/atom

            mse = client.submit(compute_mae, *(outputs_, self.targets))
            atoms_per_image = torch.cat(self.atoms_per_image)

            mse_atom = client.submit(
                compute_mae, *(outputs_, self.targets, atoms_per_image)
            )
            mse = mse.result()
            mse_atom = mse_atom.result()
            ...
            logger.info(
                "{:6d} {} {:8e} {:4e} {:4e}".format(
                    epoch, ts, loss.detach(), mse, mse_atom
                )
            )

            if self.convergence is None and epoch == self.epochs:
                converged = True
            elif self.convergence is not None and mse < self.convergence["energy"]:
                converged = True
    ...
    @classmethod
    def closure(Cls, chunks, targets, model, lossfxn, atoms_per_image, device, client):
        """Closure
