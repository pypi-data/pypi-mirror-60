...
tuple<float, float> localizeSpike(Spike spike_to_be_localized) {
  ...
  for (int i = 0; i < Parameters::num_com_centers; i++) {
    ...
    if (Parameters::debug) {
      cout << "neighbor_count: " << neighbor_count << endl;
      cout << "curr_max_channel: " << curr_max_channel << endl;
    }
    for (int j = 0; j < neighbor_count; j++) {
      curr_neighbor_channel =
          Parameters::inner_neighbor_matrix[curr_max_channel][j];
      if (Parameters::debug) {
      cout << "curr_neighbor_channel: " << curr_neighbor_channel << endl;
      cout << "masked: " << Parameters::masked_channels[curr_neighbor_channel] << endl;
    }
