/*
 * wq.app v1.2.0 - @wq/map 1.2.0
 * Leaflet integration for @wq/app pages
 * (c) 2013-2020, S. Andrew Sheppard
 * https://wq.io/license
 */

define(['leaflet', './template'], function (L, tmpl) { 'use strict';
...
map.config = {
    ...
    defaults: {
        ...
        autoZoom: {
            ...
            sticky: true,
            // Start new maps in same location as old maps
...
map.maps = {};
map.layers = {};
map.basemaps = {}; // References to generated icons
...
map.init = function (defaults) {
    var app = map.app;
    load(); // Auto-detect whether CRS-aware GeoJSON parser is available

    map.geoJson = L.Proj ? L.Proj.geoJson : L.geoJson; // Assign after module load in case L.Icon.Default.imagePath is overridden

    map.config.defaults.icon.shadowUrl = map.icons['default']._getIconUrl('shadow');
    ...
    Object.keys(app.config.pages).forEach(function (page) {
        ...
        modes.forEach(function (mode) {
            if (mapconf[mode]) {
                if (mapconf[mode].maps && mapconf[mode].maps.main && mapconf[mode].maps.main.autoLayers && !mapconf[mode].maps.main.layers) {
...
map.createBasemap = function (layerconf) {
    ...
    return fn(_objectSpread2({}, layerconf));
};
...
map.addOverlayType('geojson', function (layerconf) {
    ...
    overlay.ready = getData.then(function (geojson) {
        ...
        if (layerconf.draw && geojson.type == 'Feature' && geojson.geometry.type == 'GeometryCollection') {
...
map.createMap = function (routeInfo, divid, mapname, $page) {
    var mapid, mapconf, m, defaults, layerConfs, layers, basemaps, basemap, div; // Load configuration and div id
    ...
    map.basemaps[mapid] = basemaps; // Load layerconfs and add empty layer groups to map
...
function _getConf(page, mode, mapname) {
    ...
    var mapconf = L.extend({}, conf.defaults.maps[mapname] || {}, (conf[mode] || {
        maps: {}
    }).maps[mapname] || {}); // Combine (rather than overwrite) defaults + mode-specific layers

    if (mode && mode != 'defaults' && conf.defaults.maps[mapname] && conf.defaults.maps[mapname].layers) {
        mapconf.layers = conf.defaults.maps[mapname].layers.concat(mapconf.layers || []);
    }
