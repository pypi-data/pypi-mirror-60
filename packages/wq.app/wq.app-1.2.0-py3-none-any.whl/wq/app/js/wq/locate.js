/*
 * wq.app v1.2.0 - @wq/map/locate 1.2.0
...
locate.locators = {}; // wq/app.js plugin
...
locate.Locator = function (map, fields, opts) {
    ...
    var _mode, _marker, _circle, _lastSource, _hasSource; // Mode switching functions (define fields.toggle for default usage)
    ...
    self.update = function (loc, accuracy, source) {
        ...
        _circle.setLatLng(loc).setRadius(accuracy); // Save to fields
    ...
    function _clickMap(evt) {
        ...
        var height = px.getSize().y; // Assume accuracy is equivalent to the real-world length represented
        // by 2 pixels
    ...
    function _updateManual() {
        ...
        self.update(L.latLng(fields.latitude.val(), fields.longitude.val()), null);
    }
    ...
    map.on('click', _clickMap); // Detect cordova-plugin-bluetooth-geolocation
