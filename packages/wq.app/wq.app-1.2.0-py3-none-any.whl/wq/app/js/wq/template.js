/*
 * wq.app v1.2.0 - @wq/template 1.2.0
 * Render Mustache templates and inject as jQuery Mobile pages
 * (c) 2012-2020, S. Andrew Sheppard
 * https://wq.io/license
 */

define(['mustache'], function (m) { 'use strict';

m = m && m.hasOwnProperty('default') ? m['default'] : m;

var tmpl = {}; // Internal variables
...
tmpl.init = function (config) {
    ...
    if (config.defaults) {
        console.warn(new Error('config.defaults is deprecated; use context plugin instead'));
...
tmpl.setDefault = function (key, value) {
    console.warn(new Error('tmpl.setDefault() is deprecated; use context plugin instead'));
    _defaults[key] = value;
};
