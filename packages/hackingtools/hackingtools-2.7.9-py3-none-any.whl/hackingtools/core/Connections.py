...
__headers__ = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36'}
...
def getMyPublicIP(as_service=False):
    ...
    try:
        if as_service:
            return Logger.print_and_return(msg='getMyPublicIP', value='http{s}://{i}:{p}'.format(s=__https__, i=requests.get('__https__://api.ipify.org').text, p=__listening_port__), debug_core=True)
        return Logger.print_and_return(msg='getMyPublicIP', value=requests.get('__https__://api.ipify.org').text, debug_core=True)
    except:
        if as_service:
            return Logger.print_and_return(msg='getMyPublicIP', value='http{s}://127.0.0.1:{p}'.format(s=__https__, p=__listening_port__), debug_core=True)
...
def getMyLanIP(as_service=False):
    ...
    for ip in socket.gethostbyname_ex(socket.gethostname())[-1]:
        if '192.168.1' in ip or '192.168.0' in ip:
            if as_service:
                return Logger.print_and_return(msg='getMyLanIP', value='http{s}://{i}:{p}'.format(s=__https__, i=ip, p=__listening_port__), debug_core=True)
...
def getMyLocalIP(as_service=False, port=True):
    ...
    if isHeroku():
        ...
        if as_service:
            if port:
                return Logger.print_and_return(msg='getMyLocalIP', value='http{s}://{ss}:{p}'.format(s=__https__, ss=__services__[0], p=__listening_port__), debug_core=True)
            return Logger.print_and_return(msg='getMyLocalIP', value=__services__[0], debug_core=True)
        return Logger.print_and_return(msg='getMyLocalIP', value=__services__[0], debug_core=True)
    if as_service:
        if port:
            return Logger.print_and_return(msg='getMyLocalIP', value='http{s}://127.0.0.1:{p}'.format(s=__https__, p=__listening_port__), debug_core=True)
        return Logger.print_and_return(msg='getMyLocalIP', value='http{s}://127.0.0.1'.format(s=__https__), debug_core=True)
...
def __loadActualPort__():
    global __listening_port__
    try:
        __listening_port__ = Logger.print_and_return(msg='__loadActualPort__', value=sys.argv[-1].split(':')[1], debug_core=True)
    except:
...
def __initServices__():
    if not isHeroku():
        ...
        Config.__readConfig__()
        __services__ += Config.getConfig(parentKey='core', key='Connections', subkey='my___services__')
    Logger.printMessage(message='Loaded __services__', description=__services__, color=Logger.__Fore.YELLOW, debug_core=True)
