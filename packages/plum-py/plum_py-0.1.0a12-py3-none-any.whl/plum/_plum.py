...
def _pack_value_with_format(fmt, buffer, offset, value, dump):
    # pylint: disable=too-many-statements,too-many-branches,
    # pylint: disable=too-many-locals,too-many-nested-blocks

    if isinstance(fmt, PlumType):
        offset = fmt.__pack__(buffer, offset, value, dump)
...
def _pack_with_format(fmt, buffer, offset, args, kwargs, dump):
    # pylint: disable=too-many-branches
    try:
        if isinstance(fmt, PlumType):
            if args:
                offset = fmt.__pack__(
                    buffer, offset, args[0], dump.add_record(cls=fmt) if dump else dump)
            ...
            offset = _pack_value_with_format(fmt, buffer, offset, args, dump)
            ...
            offset = _pack_value_with_format(fmt, buffer, offset, kwargs, dump)
            ...
            # generate exception
            offset = _pack_value_with_format(
                fmt, buffer, offset, args[0] if args else kwargs, dump)
                    ...
                    try:
                        ...
                        if not isinstance(cls, PlumType):
                            if dump:
                                ...
                                dump.add_record(
                                    value=str(item), cls=cls + ' (invalid)', separate=True)
                            raise TypeError('invalid plum type')

            if isinstance(item, PlumType):
                if dump:
                    dump.add_record(cls=cls, value='(missing)', separate=True)
                raise TypeError('plum type specified without a value')

            offset = cls.__pack__(
                buffer, offset, item,
                dump.add_record(cls=cls, separate=True) if dump else dump)
            cls = None
        ...
        for name, item in kwargs.items():
            if isinstance(item, Plum):
                offset = item.__pack__(
                    buffer, offset, item,
                    dump.add_record(access=name, separate=True) if dump else dump)
            else:
                try:
                    # get PlumType from the PlumView instance
                    cls = item.__type__
                    ...
                    try:
                        ...
                        if dump:
                            dump.add_record(
                                access=name, value=str(item), separate=True,
                                cls=type(item).__name__ + ' (invalid)')
                        raise TypeError('value specified without a plum type')
                    ...
                    if not isinstance(cls, PlumType):
                        if dump:
                            dump.add_record(
                                access=name, value=str(item), separate=True,
                                cls=(cls.__name__ if isinstance(cls, type)
                                     else str(cls)) + ' (invalid)')
                        raise TypeError('invalid plum type')
...
pack_freeform_doc = pack_freeform.__doc__
