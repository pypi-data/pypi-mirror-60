...
def import_auth():
 try:
  ...
  raise hIlkG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=hIlkE)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=hIlkK)
def get_params(args,names=[],mandatory=hIlkE):
 result=()
 for name in names:
  value=hIlkS
  if name in args:
  ...
  if not value and mandatory:
   raise hIlkG('Please provide %s=... parameter'%name)
  result+=(value)
 if hIlku(result)==1:
  return result[0]
 return result
