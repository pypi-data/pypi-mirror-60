...
def import_auth():
 try:
  ...
  raise nzHNL('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=nzHNM)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=nzHNP)
def get_params(args,names=[],mandatory=nzHNM):
 result=()
 for name in names:
  value=nzHNi
  if name in args:
  ...
  if not value and mandatory:
   raise nzHNL('Please provide %s=... parameter'%name)
  result+=(value)
 if nzHNk(result)==1:
  return result[0]
 return result
