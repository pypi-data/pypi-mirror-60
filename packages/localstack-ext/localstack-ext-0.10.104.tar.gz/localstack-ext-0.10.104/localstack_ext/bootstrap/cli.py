...
def import_auth():
 try:
  ...
  raise bEOjc('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=bEOjP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=bEOjS)
def get_params(args,names=[],mandatory=bEOjP):
 result=()
 for name in names:
  value=bEOjv
  if name in args:
  ...
  if not value and mandatory:
   raise bEOjc('Please provide %s=... parameter'%name)
  result+=(value)
 if bEOjW(result)==1:
  return result[0]
 return result
