...
def import_auth():
 try:
  ...
  raise UlfQI('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=UlfQt)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=UlfQg)
def get_params(args,names=[],mandatory=UlfQt):
 result=()
 for name in names:
  value=UlfQk
  if name in args:
  ...
  if not value and mandatory:
   raise UlfQI('Please provide %s=... parameter'%name)
  result+=(value)
 if UlfQy(result)==1:
  return result[0]
 return result
