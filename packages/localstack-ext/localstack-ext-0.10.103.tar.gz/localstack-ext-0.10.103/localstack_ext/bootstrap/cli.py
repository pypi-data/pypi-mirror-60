...
def import_auth():
 try:
  ...
  raise CJBpi('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=CJBpu)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=CJBpM)
def get_params(args,names=[],mandatory=CJBpu):
 result=()
 for name in names:
  value=CJBpb
  if name in args:
  ...
  if not value and mandatory:
   raise CJBpi('Please provide %s=... parameter'%name)
  result+=(value)
 if CJBpf(result)==1:
  return result[0]
 return result
