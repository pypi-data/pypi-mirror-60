...
from zuper_typing.constants import ZuperTypingGlobals
from zuper_typing.debug_print_ import debug_print_str
...
def test_more2():
    ...
    resolve_types(Entity11)
    a = ipce_from_typelike(Entity11)
    logger.info(Entity11=a)
    ...
    b = ipce_from_typelike(Entity42)
    logger.info(Entity42=b)

    Entity11Y_ipce = ipce_from_typelike(Entity11[Y])
    logger.info(Entity11Y=Entity11Y_ipce)
    assert_equal(
        Entity11Y_ipce["$id"],
        "http://invalid.json-schema.org/test_more2.<locals>.Entity11[Y]#",
    )
    ...
    y = EI(data0=4)
    x = E2I(parent=y)
    assert_object_roundtrip(y, works_without_schema=False)
...
def test_post_init_preserved2():
    ...
    if ZuperTypingGlobals.enable_type_checking:  # pragma: no cover
        try:
            Concrete("a")
        except ValueError:
            pass
...
f = known_failure if ZuperTypingGlobals.enable_type_checking else (lambda x: x)
