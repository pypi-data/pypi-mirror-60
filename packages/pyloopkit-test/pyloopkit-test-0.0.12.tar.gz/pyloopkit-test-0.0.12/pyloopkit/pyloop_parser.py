...
# %% Take an issue report path and name and runs it through the Loop algorithm
def parse_report_and_run(path, name):
    parse_report_and_run_with_name(os.path.join(path, name))

 # %% Take an issue report and run it through the Loop algorithm
def parse_report_and_run_with_name(issue_dict):

    """ Get relevent information from a Loop issue report and use it to
        run PyLoopKit

    Arguments:
    issue_dict -- the dictionary for the issue report

    Output:
    A dictionary of all 4 effects, the predicted glucose values, and the
    recommended basal and bolus
    """
    print("inside parse_report_and_run_with_name")
    input_dict = {}
