...
class ChatSessionManager(models.Manager):
    ...
    def private(self, owner, user):
        ...
        queryset = self.get_queryset().filter(Q(type=ChatSessionTypeEnum.PRIVATE), Q(owner__username=username_first, members__user__username=username_second))
        queryset = (queryset | self.get_queryset().filter(Q(type=ChatSessionTypeEnum.PRIVATE), Q(owner__username=username_second, members__user__username=username_first))).distinct()
        if queryset.exists():
...
def post_save_chat_session_message_receiver(sender, instance, created, *args, **kwargs):
    ...
    if created:
        ...
        user = instance.user
        serializer = ChatSessionMessageDetailsSerializer(instance, many=False, read_only=True, context={'request': get_request()})
        notify(
