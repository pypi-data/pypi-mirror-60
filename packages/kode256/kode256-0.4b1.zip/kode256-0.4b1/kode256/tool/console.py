...
def main():
    ...
    parser.add_argument("-c", "--caption", action="store_true", default=False,
                        help="Write a caption containing the encoded data "
                                    "as human readable text under the barcode") 
    ...
    if ftype == ".svg":
        ...
        with open(args.output, "w") as f:
            f.write(kode256.svg(args.data, args.height,
                                args.thickness, args.caption, args.no_quiet))
        print("Barcode successfully saved as", args.output)

    elif ftype in suptypes:
        img = kode256.image(args.data, args.height,
                                args.thickness, args.caption, args.no_quiet)
        print("Barcode '%s' created" % args.data)
