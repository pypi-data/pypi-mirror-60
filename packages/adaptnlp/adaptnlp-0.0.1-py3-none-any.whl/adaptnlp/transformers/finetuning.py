...
class LMFineTuner:
    ...
    def _initial_setup(self):
        ...
        if self.model_type in ["bert", "roberta", "distilbert", "camembert"] and not self.mlm:
            raise ValueError(
        ...
        if self.config_name:
            self.config = config_class.from_pretrained(self.config_name, cache_dir=self.cache_dir)
        elif self.model_name_or_path:
            self.config = config_class.from_pretrained(self.model_name_or_path, cache_dir=self.cache_dir)
        else:
            self.config = config_class()

        if self.tokenizer_name:
            self.tokenizer = tokenizer_class.from_pretrained(self.tokenizer_name, cache_dir=self.cache_dir)
        elif self.model_name_or_path:
            self.tokenizer = tokenizer_class.from_pretrained(self.model_name_or_path, cache_dir=self.cache_dir)
        else:
            raise ValueError(
        ...
        if self.block_size <= 0:
            ...
            self.block_size = min(self.block_size, self.tokenizer.max_len_single_sentence)
    ...
    def train_one_cycle(
        ...
        if should_continue:
            sorted_checkpoints = self._sorted_checkpoints(checkpoint_prefix="checkpoint", output_dir=output_dir)
        ...
        if self.local_rank in [-1, 0] and use_tensorboard:
            try:
                ...
                logger.warning("WARNING! Tensorboard is a required dependency...`use_tensorboard` is now set as False")
        ...
        scheduler = OneCycleLR(
            optimizer, max_lr=learning_rate, epochs=int(num_train_epochs), steps_per_epoch=int(t_total)
        )

        # Check if saved optimizer or scheduler states exist
        if self.model_name_or_path and os.path.isfile(
                os.path.join(self.model_name_or_path, "optimizer.pt")) and os.path.isfile(
        ...
        model_to_resize = self.model.module if hasattr(self.model,
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                self.model.train()
                outputs = self.model(inputs, masked_lm_labels=labels) if self.mlm else self.model(inputs, labels=labels)
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if self.local_rank in [-1, 0] and logging_steps > 0 and global_step % logging_steps == 0:
                        # Log metrics
                        if (
                                self.local_rank == -1 and evaluate_during_training
                        ):  # Only evaluate when single GPU otherwise metrics may not average well
                            results = self.evaluate(output_dir=output_dir,
                    ...
                    if self.local_rank in [-1, 0] and save_steps > 0 and global_step % save_steps == 0:
                        ...
                        ckpt_output_dir = os.path.join(output_dir, f"{checkpoint_prefix}-{global_step}")
                        ...
                        torch.save(init_locals, os.path.join(ckpt_output_dir, "training_args.bin"))
                        logger.info("Saving model checkpoint to %s", ckpt_output_dir)

                        self._rotate_checkpoints(checkpoint_prefix, save_total_limit=save_total_limit,
    ...
    def evaluate_all_checkpoints(
        ...
        if self.local_rank in [-1, 0]:
            ...
            for checkpoint in checkpoints:
                ...
                self.model.to(self.device)
                result = self.evaluate(output_dir=output_dir, per_gpu_eval_batch_size=per_gpu_eval_batch_size,
                                       prefix=prefix)
    ...
    def _rotate_checkpoints(
        ...
        checkpoints_sorted = self._sorted_checkpoints(checkpoint_prefix, output_dir, use_mtime)
    ...
    def find_learning_rate(
        ...
        optimizer = AdamW(optimizer_grouped_parameters, lr=start_learning_rate, eps=adam_epsilon, **kwargs)
        ...
        while step < iterations:
            ...
            batch_loader = DataLoader(train_dataset, sampler=train_sampler, batch_size=mini_batch_size)
