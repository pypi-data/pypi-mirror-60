# Code used/modified by NovettaNLP author from Hugginface's Transformers
...
from adaptnlp.transformers.utils_squad import (SquadExample, InputFeatures, convert_examples_to_features,
                                               RawResult, RawResultExtended, get_final_text,
...
class BertQuestionAnsweringModel(QuestionAnsweringModel):
    ...
    def _load(self) -> None:
        ...
        self.pretrained_config = self.config.from_pretrained("bert-large-uncased-whole-word-masking-finetuned-squad")
        if "uncased" in model_name:
            tokenizer = self.tokenizer.from_pretrained("bert-large-uncased", do_lower_case=True)
        else:
            tokenizer = self.tokenizer.from_pretrained("bert-large-cased", do_lower_case=False)
        self.pretrained_tokenizer = tokenizer

        model = self.model.from_pretrained(model_name, from_tf=bool(".ckpt" in model_name),
                                           config=self.pretrained_config)
        self.pretrained_model = model
        self.pretrained_model.to(self.device)

    def _load_one_query(self, query: str, context: str, output_examples=True) -> Union[TensorDataset,
                                                                                       List[SquadExample],
                                                                                       List[InputFeatures]]:
        ...
        # Create SquadExample
        examples = []
        example = SquadExample(qas_id=None,
                               question_text=query,
    ...
    def _produce_concrete_predictions(self, all_examples, all_features, all_results, n_best_size=10,
                                      max_answer_length=30, do_lower_case=True, verbose_logging=False,
        ...
        for (example_index, example) in enumerate(all_examples):
            ...
            _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
                "NbestPrediction",
                ["text", "start_index", "end_index", "start_logit", "end_logit"])  # ### start_end_index
            ...
            for pred in prelim_predictions:
                ...
                nbest.append(
                    _NbestPrediction(
                        ...
                        end_index=orig_doc_end))  # ### start_end_index...Make span indices inclusive
                ...
                if "" not in seen_predictions:
                    nbest.append(
                        _NbestPrediction(
                            ...
                            end_logit=null_end_logit,
                            start_index=0,
                            end_index=0))  # ### start_end_index should this be pred.<index>

                # In very rare edge cases we could only have single null prediction.
                # So we just create a nonce prediction in this case to avoid failure.
                if len(nbest) == 1:
                    nbest.insert(0,
                                 _NbestPrediction(text="empty", start_logit=0.0, end_logit=0.0, start_index=0.0,
                                                  end_index=0.0))  # ### start_end_index
            ...
            for (i, entry) in enumerate(nbest):
                ...
                output["start_index"] = entry.start_index  # ### start_end_index MAGIC NUMBERS for adjustment :/
    ...
    def predict(self, query: str, context: str, n_best_size: int = 20) -> Tuple[str, List[OrderedDict]]:
        ...
        all_predictions, all_nbest_json = self._produce_concrete_predictions(examples, features, all_results,
...
class XLNetQuestionAnsweringModel(QuestionAnsweringModel):
    ...
    def _load(self) -> None:
        ...
        tokenizer = self.tokenizer.from_pretrained("xlnet-large-cased", do_lower_case=False)
        self.pretrained_tokenizer = tokenizer
    ...
    def _produce_concrete_predictions(self, all_examples, all_features, all_results, n_best_size=10,
                                      max_answer_length=30, verbose_logging=False):
    ...
    def predict(self, query: str, context: str, n_best_size: int = 20, as_dict: bool = False) -> Union[List[str], dict]:
        ''' Predicts top_n answer spans of query in regards to context
...
class XLMQuestionAnsweringModel(QuestionAnsweringModel):
    ...
    def _load(self) -> None:
        ...
        tokenizer = self.tokenizer.from_pretrained("xlm-mlm-en-2048", do_lower_case=False)
        self.pretrained_tokenizer = tokenizer
    ...
    def _produce_concrete_predictions(self, all_examples, all_features, all_results, n_best_size=10,
        ...
        for (example_index, example) in enumerate(all_examples):
            ...
            for pred in prelim_predictions:
                ...
                final_text = get_final_text(tok_text, orig_text, tokenizer,  # .do_lower_case, (a XLM problem?)
                                            verbose_logging)
