...
from PySide2.QtWidgets import QGraphicsView, QFileDialog, QGraphicsPolygonItem, QGraphicsTextItem, \
     QGraphicsSceneMouseEvent, QGraphicsScene
from nextcrop import ManualCrop
...
class CropWidget(QGraphicsView):
    ...
    def __init__(self, manual_crop: ManualCrop):
        self.s = QGraphicsScene()
        QGraphicsView.__init__(self, self.s)
    ...
    def draw_bounding_box(self, poly: QPolygonF):
        item_poly = QGraphicsPolygonItem(poly)
        item_poly.setPen(QPen(Qt.blue))
        self.scene().addItem(item_poly)
        return item_poly

    def add_image_number(self, center: QPointF, idx: int, delete_image_cb):
