...
class AnnotateCore:
    @staticmethod
    def draw_bounding_boxes(crop_widget, bounding_box_scene_items, bounding_boxes, delete_image_cb):
        crop_widget.clear_bounding_boxes()
        for (bounding_box, idx) in zip(bounding_boxes, range(0, bounding_boxes.__len__())):
            ...
            item = crop_widget.draw_bounding_box(poly)
            # self.crop_widget.add_image_number(center, idx, delete_image_cb)
            bounding_box_scene_items.append(item)

    @staticmethod
    def draw_contours(crop_widget, contours, contour_scene_items):
        for contour in contours:
            poly = np_to_qpolygonf(contour)
            item_poly = crop_widget.draw_contour(poly)
            contour_scene_items.append(item_poly)

    @staticmethod
    def clear_contours(crop_widget, contour_scene_items):
        for scene_item in contour_scene_items:
            crop_widget.remove_item(scene_item)

        contour_scene_items.clear()

    @staticmethod
    def clear_bounding_boxes(crop_widget, bounding_box_scene_items):
        for scene_item in bounding_box_scene_items:
            crop_widget.remove_item(scene_item)

        bounding_box_scene_items.clear()
...
class AnnotateFrame:
    ...
    def draw_contours(self, contours):
        # TODO need to add xf for contours in crop
        return
        for contour in contours:
            poly = np_to_qpolygonf(contour)
