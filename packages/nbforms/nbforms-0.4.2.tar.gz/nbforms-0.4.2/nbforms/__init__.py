...
class Notebook:
    ...
    def __init__(self, config_path="nbforms_config.json", force_reauth=False):
    ...
    def _auth(self):
        ...
        if "auth" in self._config and self._config["auth"] != "default":
            ...
            if self._config["auth"] == "google":
                ...
                assert auth_response.text not in [
                    "INVALID USERNAME", "<h1>Internal Server Error</h1>", "OAUTH REQUIRED FOR THIS USER"
                ], "Was not able to get API key"
