...
class rcaisskeras_pictures():
    def __init__(self):
        ...
        print('==============================')
        print('--函式說明--')
        print('函式1: Crawling_from_chrome(爬取數量,關鍵字)\t\t\t=>用於爬蟲,自動爬取Google圖片\n')
        print('函式2: Resize_picture(要修改的目錄,修改完要放置的目錄,高度,寬度)\t\t\t=>用於將圖片自動修正成同尺寸\n')
        print('函式3: load_picture(要輸入的圖片位置,高度,寬度,色階[1->黑白 3->彩色])\t\t\t\t\t=>用於將圖片做成神經網絡能輸入的樣子\n')
        print('-------回傳(資料,類別)')
        print('函式4: get_onehot_name()\t\t\t\t\t=>用於取得做完one-hot-encoding的對應表\n')
        print('函式5: split_train_test_data(資料,類別,分割比例)\t\t\t\t\t=>用於分割成訓練資料及測試資料(0.3為訓練:測試=>7:3)\n')
...
class rcaisskeras_CNNmodel():
    ...
    def __init__(self):
        print('--函式說明--')
        print('函式1: set_epoch(代數)\t\t\t\t=>用於設定要訓練幾代\n')
        print('函式2: set_train_test_data(訓練變項資料,訓練分類為何,測試變項資料,測試分類為何)\n')
        print('函式3: predict_data(預測的資料夾位置,長,寬,色階)\t\t\t\t=>用於預測新的數據(色階「1」為黑白,「3」為彩色)')
        print('函式4: get_acc()\t\t\t\t=>用於得到此模型的正確率,回傳(訓練正確率,測試正確率)\n')
        print('函式5: run_model\t\t\t\t=>開始跑神經網絡\n')
        print('函式6: show_history()\t\t\t\t=>用於顯示訓練過程\n')
        print('-----------')
    ...
    def run_model(self):
        ...
        #開始使用修正完的參數做預測，並將"預測"的結果放置在classes裡面
        classes = self.model.predict_classes(self.x_test, batch_size=self.batch_size)
    def predict_data(self,predict_dir,height,weight,color):
