...
def _new_look_tag(tag, par_json, subtag, default, only_value):
    ...
    if isinstance(subtag, list):
        if any(tag_value for tag_value in subtag if tag_value in par_json[tag].keys()):
            if only_value:
                tags_list = [content['value'][0] for tag_value in subtag if tag_value in par_json[tag] for content in par_json[tag][tag_value]['content']]
            else:
                tags_list = [content for tag_value in subtag if tag_value in par_json[tag] for content in par_json[tag][tag_value]['content']]
            return tags_list
...
def _response_query(par, function_call, second_call, simple = False):
  ...
  if queries_qt == 0:
    ...
    if response_execution_error:
        html_header_text = ObjHTML('<h3 style="color:red">Script com erro de execução</h3>')
    else:
        html_header_text = ObjHTML('<div><h3 style="color:green">Script executado corretamente</h3></div>')

    if queries_qt > 1:
      response = ResponseFrame(stacked=False)
      header = ResponseFrame(html_header_text)
    else:
      response = ResponseFrame(stacked=True)
      response.content.append(html_header_text)
    ...
    for i in range(0, len(global_queries)):
      query = ObjHTML(
          """<div class="alert alert-secondary" style="margin: 10px;word-break: break-word; white-space: pre-wrap;">
              <h4 style="margin-top: -40px">Query:   """ + str(i+1) + """ </h4>
              <div style="margin-top: -10px"><p style="margin-top: -10%">""" + global_queries[i] +
          """"'</p></div></div>""")

      response.content.insert(len(response.content), query)

    if queries_qt > 1:
      response = ResponseBoard([html_header_text, response])
    else:
