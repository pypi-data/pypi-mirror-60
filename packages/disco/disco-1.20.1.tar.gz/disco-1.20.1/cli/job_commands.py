...
from disco.core.exceptions import DiscoException, TimeOutError
...
def create_job(state, script, inputs, constants, name, run, wait, instance_type, cluster_id,  # pylint: disable=redefined-builtin
               repository_id, script_file_path_in_repo, debug_mode, dont_generate_req_file):
    """
    ...
    try:
        ...
        if script is not None:
            script_file_id = upload_job_script_file(script, cluster_id, show_progress_bar)
            created_job = Job.create(script_file_id, input_file_ids, const_file_ids, name, instance_type,
                                     cluster_id, auto_start=run, upload_requirements_file=not dont_generate_req_file)
        else:
            created_job = Job.create(input_file_ids=input_file_ids, constants_file_ids=const_file_ids, job_name=name,
                                     cluster_instance_type=instance_type, cluster_id=cluster_id,
                                     script_repo_id=repository_id, script_file_path_in_repo=script_file_path_in_repo,
                                     auto_start=run, upload_requirements_file=not dont_generate_req_file)
        job_status = None
        if run:
