...
class Asset(BaseController):
    ...
    def upload(self, file_name, file, cluster=None, show_progress_bar=True):
        ...
        with create_progress_bar(
            ...
            if cluster_type == ClusterType.Azure:
                raise NotImplemented('Azure clusters are not supported')   # pylint: disable=all
            else:
                # ClusterType.AWS OR ClusterType.GCP:
                self._upload_with_callback(data['url'],
                                           file_name,
                                           form_fields=data.get('fields', {}),  # Only for AWS
