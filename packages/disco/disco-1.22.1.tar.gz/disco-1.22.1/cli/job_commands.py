...
@click.option('-did', '--docker-image-id', help='The id of the docker image to run the job in')
@click.option('--debug-mode', hidden=True, is_flag=True)
@click.option('--dont-generate-req-file', hidden=True, is_flag=True)  # pylint: disable=too-many-arguments,too-many-locals, too-many-branches, , too-many-return-statements
@common_options
@pass_state
def create_job(state, script, inputs, constants, name, run, wait, instance_type, cluster_id,  # pylint: disable=redefined-builtin
    ...
    try:
        ...
        const_file_ids, input_file_ids = upload_job_files(input_paths, const_paths, cluster_id, show_progress_bar)
