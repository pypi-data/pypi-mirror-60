...
from typing import Dict, Iterable, List, Optional, Sequence, Set, Text, Tuple, Union
...
def get_entities_relations_attributes_groups(
    ...
    entities: Dict[str, Entity] = set_to_dict(entities_s)
    relations: Dict[str, Relation] = set_to_dict(relations_s)
    attributes: Dict[str, Attribute] = set_to_dict(attributes_s)
    ...
    for entity_id, entity in entities.items():
        if entity.type in GROUPING_ENTITIES:
            ...
            for relation in grouping_relations.values():
                if relation.subj == entity_id:
                    items.add(entities[relation.obj])
