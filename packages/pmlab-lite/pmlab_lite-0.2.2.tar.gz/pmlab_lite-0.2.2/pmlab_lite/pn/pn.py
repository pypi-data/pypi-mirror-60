...
class PetriNet(AbstractPetriNet):
	...
	def index_of_transitions(self, transition, num_of_duplicate=0):
		#maybe change this, to get id and negative number as input to return the index
		index = list(itertools.chain.from_iterable(list(self.transitions.values()))).index(self.transitions[transition][num_of_duplicate])
		return index
	...
	def synchronous_product(self, trace_net):
		...
		#copying the transitions
		for t in self.transitions.keys():
			sp_net.add_transition(t + "_model")
			
		for t in trace_net.transitions.keys():
			for i in range(0, len(trace_net.transitions[t])):
				sp_net.add_transition(t + "_log")
		...
		#making sp transitions, so whenever trace_t has the same name as model_t we create a new sync_t with all the in/outputs from the model_ and trace_t combined
