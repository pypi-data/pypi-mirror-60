...
from .constants import PANDEIA_RTOL, PANDEIA_ATOL, SPECTRAL_LINE_WINDOW, SPECTRAL_LINE_SUBSAMPLE, SPECTRAL_MAX_SAMPLES, SPECTRAL_MAX_CHUNK, SCENE_FOV_PIXBUFFER, SCENE_MAX_PIXELVAL
...
class ModelSceneCube(object):
    ...
    def export_to_fits(self, fitsfile='ModelSceneCube.fits'):
        ...
        c1 = fits.Column(name='wavelength', array=self.wave, format='K')
...
class ConvolvedSceneCube(object):
    ...
    def create_flux_cube(self, background=None):
        ...
        for src in self.scene.sources:
            if np.abs(src.position['x_offset']) > psf_pixsize*10:
                self.single_point_source = False
            elif np.abs(src.position['y_offset']) > psf_pixsize*10:
                self.single_point_source = False
            elif src.shape['geometry'] is not 'point':
                self.single_point_source = False
...
class AdvancedPSF(object):
    ...
    def __init__(self, wave, psf_library, instrument, mode, model_scene=None, psf_source_offset=(0, 0),
        ...
        if (model_scene is None) or (empty_scene):
            self.intensity = np.zeros((scene_npix,scene_npix))
            ...
            if single:
                kernel_npix = kernel.shape[0]
                mini = np.int((scene_npix - kernel_npix)/2) # minimum index of the kernel size within the FOV
                maxi = np.int((scene_npix + kernel_npix)/2) # maximum index of the kernel size within the FOV

                self.intensity = np.zeros((scene_npix, scene_npix))
                # mode='same' forces the FFT output to be the same shape as the input scene to be convolved with
                # (first argument to the function)
                self.intensity[mini:maxi, mini:maxi] = scipy.signal.fftconvolve(model_scene.int[mini:maxi, mini:maxi, windex],kernel,mode='same')
            else:
