...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        plt.hist(self.data)
        plt.title('Histogram of Binomial Instance Variable')
        plt.xlabel('outcome')
        plt.ylabel('count')
    ...
    def pdf(self, k):
        ...
        term1 = math.factorial(self.n) / (math.factorial(k) * (math.factorial(self.n - k)))
        term2 = (self.p ** k) * (1 - self.p) ** (self.n - k)
        return term1 * term2
    ...
    def  __repr__(self):
        return "mean {}, standard deviation {}, probability {}, size{}".format(self.mean, self.stdev, 
                                                                               self.p, self.n)    
