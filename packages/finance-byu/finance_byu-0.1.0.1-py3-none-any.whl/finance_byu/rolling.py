...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky',ddof='default',predispatch='all'):
    ...
    if data.shape[0] < roll:
        d = pd.DataFrame(np.NaN,index=[i for i in range(data.shape[0]-roller)],columns=xvar))
    else:
    ...
    if residuals=='residvol':
        ...
        if data.shape[0] < roll:
            rvol = pd.Series(np.NaN,index=[i for i in range(data.shape[0]-roller)],name='residvol'))
        else:
