...
@njit(parallel=False)
def _residvol_numba(d,yvari,xvari,gamma):
...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky'):
    ...
    if residuals=='residvol':
        zipgrouped = [x for x in zip(grouped,[np.array(d[i,:]) for i in range(d.shape[0])])]
        #retlst2 = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch='all') \
        #         (jl.delayed(_residvol_numba)(g[0],int(yvari),np.array(xvari),g[1]) for g in zipgrouped)
        retlst2 = [_residvol_numba(g[0],int(yvari),np.array(xvari),g[1]) for g in zipgrouped]
