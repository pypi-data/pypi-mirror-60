...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky'):
    ...
    if residuals=='residvol':
        ...
        #         (jl.delayed(_residvol_numba)(g,int(yvari),np.array(xvari),np.array(d.iloc[idx,:])) for idx,g in enumerate(grouped))
        retlst2 = [_residvol_numba(g,int(yvari),np.array(xvari),np.array(d.iloc[idx,:]),d.shape[0]) for idx,g in enumerate(grouped)]
