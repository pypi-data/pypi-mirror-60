...
def _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof):
...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky',ddof='default'):
    """
    ...
    _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof)
    ...
    if residuals=='residvol':
        retlst2 = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch='all') \
