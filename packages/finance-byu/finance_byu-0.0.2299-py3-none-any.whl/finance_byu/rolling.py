...
@jit(parallel=False)
def _residvol_numba(d,yvari,xvari,gamma,count):
    r = np.empty(count)
    for i in range(count):
        r[int(i)] = d[int(i),yvari]-d[int(i),xvari].dot(gamma)
    return np.array([r.std(),np.NaN])
...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky'):
    """
    ...
    if residuals=='residvol':
        retlst2 = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch='all') \
                 (jl.delayed(_residvol_numba)(g,int(yvari),np.array(xvari),np.array(d.iloc[idx,:]),g.shape[0]) for idx,g in enumerate(grouped))
