...
@jit(parallel=False)
def _residvol_numba(d,yvari,xvari,gamma,count,ddof):
    r = np.empty(count)
    for i in range(count):
        r[int(i)] = d[int(i),yvari]-d[int(i),xvari].dot(gamma)
    return np.array([np.sqrt(r.var()*count/(count-ddof)),np.NaN])
...
def _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof,predispatch):
...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky',ddof='default',predispatch='all'):
    """
    ...
    _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof,predispatch)
    ...
    grouped = [j for j in range(roller,data.shape[0])] # new grouping 22996
    ...
    #            (jl.delayed(_ols_numba2)(g,int(yvari),np.array(xvari)) for g in grouped)
    # new grouping 22996 begin
    retlst = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch=predispatch) \
                (jl.delayed(_ols_numba2)(np.array(data.loc[j-roller:j,:]),int(yvari),np.array(xvari)) for j in grouped)
    # new grouping 22996 end
    d = pd.DataFrame(np.vstack(retlst))
    
    if ddof=='default':
        ddof = len(xvar)
    
    if residuals=='residvol':
        
        #retlst2 = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch=predispatch) \
        #         (jl.delayed(_residvol_numba)(g,int(yvari),np.array(xvari),np.array(d.iloc[idx,:]),g.shape[0],ddof) for idx,g in enumerate(grouped))
        # new grouping 22996 begin
        retlst2 = jl.Parallel(n_jobs=n_jobs,backend=backend,batch_size=batch_size,pre_dispatch=predispatch) \
                 (jl.delayed(_residvol_numba)(np.array(data.loc[j-roller:j,:]),int(yvari),np.array(xvari),np.array(d.iloc[idx,:]),g.shape[0],ddof) for idx,g in enumerate(grouped))
        # new grouping 22996 end
        ##retlst2 = [_residvol_numba(g,int(yvari),np.array(xvari),np.array(d.iloc[idx,:]),g.shape[0]) for idx,g in enumerate(grouped)]
        rvol = pd.DataFrame(np.vstack(retlst2)[:,0])
    
    fill = pd.DataFrame(np.NaN,index=[i for i in range(roller)],columns=xvar)
    d.columns = xvar 
    ...
    if residuals != False:
        ...
        if residuals==True:
            if intercept:
                data['yhat'] = data['intercept']
            else:
                data['yhat'] = 0
            for x in xvar:
                data['yhat'] = data['yhat'] + data[x]*d[x]
            d['resid'] = data[yvar] - data['yhat']
        
        elif residuals=='residvol':
            fill = pd.DataFrame(np.NaN,index=[i for i in range(roller)],columns=[0])
            rvol = pd.concat([fill,rvol],axis=0).reset_index(drop=True)
            d['residvol'] = rvol
