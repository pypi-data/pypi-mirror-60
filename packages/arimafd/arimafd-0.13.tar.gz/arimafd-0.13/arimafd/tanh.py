...
class anomaly_detection:
    ...
    def __init__(self,data,anomaly_list):
        """
        Thic class for anomaly deterction aplication of modernized ARIMA model

        values - list of 2 elements: left and rigth boundary of window
        """
    ...
    def proc_tensor(self,window=100,No_metric=1):
        ...
        for i in range(tensor.shape[1]):
            df = pd.DataFrame(tensor[:,i,:],index=self.indices[-len(tensor):])
...
#                 print('Metric2')
            ...
            anomaly_df = ((metric > ucl) | (metric < lcl)).astype(int)
            if iteratot == 0:
                bin_metric = anomaly_df
            else:
                bin_metric+= anomaly_df
            iteratot+=1
    ...
    def evaluate_nab(self,table_of_coef=None):
