...
def _validate(source_directory, run_config, show_linting, show_package_validation):
    ...
    if lint_results not in [TIMED_OUT, []] and FAILED not in lint_results:
        ...
        validation_results['validationLintErrors'] = lint_results if lint_results else False
    ...
    if validate:
        ...
        if validate_thread.is_alive():
            ...
            try:
                ...
                validation_results['validationFiles'] = validate_queue.get(block=False)
            except queue.Empty:
                package_results = FAILED

        if package_results != TIMED_OUT and FAILED not in package_results:
            validation_results['validationPackageErrors'] = True if package_results else False
            # Issue warning
            ...
            validation_results['validationPackageErrors'] = package_results
    else:
