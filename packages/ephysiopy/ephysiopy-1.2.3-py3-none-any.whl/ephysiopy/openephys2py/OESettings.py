...
class Settings(object):
    ...
    def parseChannels(self):
        ...
        for chan_info in self.processors['Sources/Rhythm FPGA'][0].iter():
            ...
            if 'EDITOR' in chan_info.tag:
                for editor in chan_info.iter('EDITOR'):
                    tmp = editor.get('SampleRateString')
                    self.fpga_sample_rate = float(tmp.split()[0])
