...
#
    ...
    if yes or query_yes_no(f"{use_term[0]} terraform {release}?", "yes"):
        ...
        print(cs("unpacked at:", "pink"), bold(os.path.join(home, rand_str, "terraform")))
        if best_path_owner == "root":
            ...
            cp_cmd = f"sudo cp {os.path.join(home, rand_str, 'terraform')} {best_path}"
            backup_cmd = f"sudo cp {os.path.join(best_path, 'terraform')} {os.path.join(best_path, 'terraform.it.backup')}"
        else:
            cp_cmd = f"cp {os.path.join(home, rand_str, 'terraform')} {best_path}"
