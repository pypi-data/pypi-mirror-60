...
class prodRf:
	...
	def kkm_export_transactions(self, kkm_id, kwargs, file_name = False):
		...
		if not file_name:
			try:
				file_name = re.findall('filename="(.+)"', self.r.headers['Content-Disposition'])[0]
			except IndexError:
				file_name = f'report-kkt-{kkm_id}.xlsx'
		elif not file_name.endswith('.xlsx') or not file_name.endswith('.xls'):
			try:
				file_name = file_name + '\\' + re.findall('filename="(.+)"', self.r.headers['Content-Disposition'])[0]
			except IndexError:
				file_name = = file_name + f'\\report-kkt-{kkm_id}.xlsx'
