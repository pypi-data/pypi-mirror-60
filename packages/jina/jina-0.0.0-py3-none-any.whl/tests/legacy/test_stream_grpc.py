...
from jina.helper import TimeContext
from jina.main.parser import set_frontend_parser, set_router_parser
from jina.peapods.base import SocketType, MessageHandler, Pea as BS
from jina.peapods.frontend import FrontendService
from jina.proto import RequestGenerator, jina_pb2_grpc
...
class Router1(RouterService):
    ...
    def _handler_default(self, msg: 'jina_pb2.Message'):
...
class TestStreamgRPC(unittest.TestCase):
    ...
    def test_grpc_frontend(self):
        ...
        with RouterService(p_args), FrontendService(args), grpc.insecure_channel(
            ...
            # self.assertEqual(resp.envelope.request_id, len(self.all_bytes))  # idx start with 0, but +1 for final FLUSH
    ...
    def test_async_block(self):
        ...
        with FrontendService(args), Router1(p1_args), Router2(p2_args), grpc.insecure_channel(
            ...
            with TimeContext('non-blocking call'):  # about 26s = 32s (total) - 3*2s (overlap)
                ...
                for r in resp:
                    # self.assertEqual(r.envelope.request_id, id)
                    id += 1
            ...
            with TimeContext('blocking call'):  # should be 32 s
                for r in RequestGenerator.train(self.all_bytes2, batch_size=1):
                    resp = stub.Call(r)
                    # self.assertEqual(resp.envelope.request_id, id)
                    id += 1
