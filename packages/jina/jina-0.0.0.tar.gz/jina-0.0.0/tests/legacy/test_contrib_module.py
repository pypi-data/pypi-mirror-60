...
class TestContribModule(unittest.TestCase):
    def setUp(self):
        self.exec_yaml_path = os.path.join(os.path.dirname(__file__),
                                      'contrib', 'dummy.yml')
        self.dump_yaml_path = os.path.join(os.path.dirname(__file__),
                                           'dummy-dump.yml')

    def tearDown(self):
        # reload jina module on every unit test
        if os.path.exists(self.dump_yaml_path):
            os.remove(self.dump_yaml_path)

    def test_load_contrib(self):
        os.environ['JINA_CONTRIB_MODULE'] = module_path
        from jina.encoder.base import BaseEncoder, BaseTextEncoder
        a = BaseEncoder.load_yaml(self.exec_yaml_path)
    ...
    def test_bad_path(self):
        os.environ['JINA_CONTRIB_MODULE'] = 'blah'
        try:
            from jina.encoder.base import BaseEncoder
        except AttributeError:
            pass
