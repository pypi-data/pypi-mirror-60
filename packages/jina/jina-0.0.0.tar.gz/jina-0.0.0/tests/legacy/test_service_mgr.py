...
from jina.main.parser import set_router_parser, set_frontend_parser, set_encoder_parser, set_indexer_parser
from jina.peapods.base import Pod, SocketType, ParallelType
...
class TestServiceManager(unittest.TestCase):
    ...
    def _test_multiple_router(self, backend='thread', num_parallel=5):
        a = set_router_parser().parse_args([
            ...
            '--parallel_runtime', backend
        ])
        with Pod(RouterService, a):
            pass
    ...
    def _test_grpc_multiple_router(self, backend='thread', num_parallel=5):
        ...
        with Pod(RouterService, p_args), FrontendService(args), grpc.insecure_channel(
            ...
            self.assertEqual(resp.envelope.request_id, 0)
