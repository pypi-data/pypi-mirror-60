#!/usr/bin/python3
import pytest
from topoly.topoly_surfaces import c_lasso_type
...
good_output = '0 4 1 X X -33 +34 -35 -43 XX 0 2 1 X X -33 -43 XX LS2--C XX 10.4002 30 2.42151\n'
lasso00_good_out = '0 0 1 X X XX 0 0 1 X X XX  L0 XX 14.3203 30 2.32774\n'

def test_surfaces():
    print("Testing PY surfaces\n")
    
    res = c_lasso_type(chain = full_array, loop_beg = 1, loop_end = 30)
    res_str = res.decode('UTF-8')
    print("result:\n" + res_str.replace('\n', ''))
    try:
        assert res_str == good_output
    except:
        print('surfaces test1 failed :(\n')
    else: 
        print('surfaces test1 passed :)\n')

    res = lasso_type(coordinates = full_array, loop_beg = 1, loop_end = 30)
    res_str = res.decode('UTF-8')
    print("result:\n" + res_str.replace('\n', ''))
    try:
        assert res_str == good_output
    except:
        print('surfaces test2 failed :(\n')
    else: 
        print('surfaces test2 passed :)\n')

    res = lasso_type(coordinates = os.path.join(os.path.dirname(os.path.abspath(__file__)),'../data/lasso_00.xyz'), loop_beg = 1, loop_end = 30)
    res_str = res.decode('UTF-8')
    print("result:\n" + res_str.replace('\n', ''))
    try:
        assert res_str == lasso00_good_out
    except:
        print('surfaces test2 failed :(\n')
    else:
        print('surfaces test2 passed :)\n')
