...
def projectileMotionEq(find=False, angleUnitRadians=True, printEq=False, **kwargs):
    ...
    # try:
    #     kwargs["alpha0"] = radians(kwargs["alpha0"]) if not(angleUnitRadians) else kwargs["alpha0"]
    # except:
    #     pass
    
    if not(angleUnitRadians):
        kwargs = dictionary_add_modify(kwargs,alpha0=radians(kwargs["alpha0"]))
    kwargs = dictionary_add_modify(kwargs,g=scipy.constants.g)

    # try:
    #     kwargs["g"] =scipy.constants.g if kwargs["g"]
...
def projectileMotion2D(v0, alpha0, t, angleUnitRadians="radians"):
    """Quick version of projectile motion
    returns [x,y,vx,vy]"""
    g = scipy.constants.g
    alpha0 = radians(alpha0) if angleUnitRadians=="degrees" else alpha0
...
if __name__ == "__main__":
    projectileMotionEq(find="t", angleUnitRadians="degrees", printEq=True, alpha0=45, y=0)
    a = kinematicsEq("d", printEq=True, v=15, a=9.81, t=4)
