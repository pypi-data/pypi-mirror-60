...
class RexSteerWalkEnv(rex_gym_env.RexGymEnv):
  ...
  """
    ...
    def reset(self):
        ...
        super(RexSteerWalkEnv, self).reset(initial_motor_angles=initial_motor_angles,
                                                 reset_duration=0.5)
    ...
    def _convert_from_leg_model(self, leg_pose):
        ...
        for i in range(NUM_LEGS):
            motor_pose[int(3 * i)] = leg_pose[int(3 * i)]
            motor_pose[int(3 * i + 1)] = leg_pose[int(3 * i + 1)]
            motor_pose[int(3 * i + 2)] = leg_pose[int(3 * i + 2)]
        return motor_pose
    ...
    def _signal(self, t):
        ...
        # WALK
        # second_leg = np.array([0, 0, swing + 0.3,
        #                        0, 0, extension - 0.4,
        #                        0, 0, extension - 0.4,
        #                        0, 0, swing + 0.3])
        # first_leg = np.array([0.1, 0, extension - 0.4,
        #                       -0.1, 0, swing + 0.3,
        #                       0.1, 0, swing + 0.3,
        #                       -0.1, 0, extension - 0.4])

        second_leg = np.array([-0.1, 0, 0.2,
                               0.1, 0, -0.2,
                               -0.1, 0, -0.2,
                               0.1, 0, 0.2])
        first_leg = np.array([0.1, 0.1, -0.2,
                              -0.1, 0.1, 0.2,
                              0.1, -0.1, 0.2,
                              -0.1, -0.1, -0.2])
        if ith_leg:
