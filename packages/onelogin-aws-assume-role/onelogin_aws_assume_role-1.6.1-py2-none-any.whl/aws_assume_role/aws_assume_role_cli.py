...
def write_data_to_cache(contents):
    ...
    if hasattr(contents, 'password'):
        del contents['password']
    if hasattr(contents, 'mfa_verify_info') and hasattr(contents['mfa_verify_info'], 'otp_token'):
...
def main():
    ...
    for i in iterations:
        ...
        if result is not None:
            if not is_valid_saml_assertion(b64decode(result['saml_response'])):
                result = None

        if not options.cache_saml:
            if i == 0:
                print("Skipping check for a valid SAML Response cached")
        else:
            ...
            elif i != 0 and missing_onelogin_data:
                print("The cached SAML expired, asking for some info required to generate a new SAML Response")
                clean_cache()
        ...
        if ask_for_user_again:
            ...
            ask_for_user_again = False
            ask_for_role_again = True
        elif (i == 0 and result is None) or missing_onelogin_data:
