...
class RegressionFrame(object):
    ...
    def initialize_df_from_equation(self, equation, current_bounding_box, existing_df=None,):
        ...
        if drop_zero_from_dependent_variable:
            current_invalid_mask = (first_array != self.global_aligned_inputs[self.dependent_variable_label].ndv) & (first_array != 0)
        else:
            current_invalid_mask = first_array != self.global_aligned_inputs[self.dependent_variable_label].ndv
        ...
        for c, variable_transform in enumerate(self.equation_dict['variable_transforms']):
            if variable_transform[0] == 'mask':
                ...
                hb.esacci_extended_short_class_descriptions
        ...
        for c, necessary_variable in enumerate(self.equation_dict['necessary_variables']):
            ...
            if necessary_variable not in df.columns:
                ...
                L.debug('Loading ' + str(necessary_variable) + ' to df from ' + str(self.global_aligned_inputs[necessary_variable].path) + ' with shape ' + str(hb.get_shape_from_dataset_path(self.global_aligned_inputs[necessary_variable].path)))
        ...
        for c, variable_transform in enumerate(self.equation_dict['variable_transforms']):
            if variable_transform == 'intercept':
                if 'intercept' not in df.columns:
                    ...
                    data = np.concatenate((data, current_array.flatten()[:, np.newaxis]), axis=1)  # Funny newaxis is because needed to pass a 2d array to match data
