...
def reclassify_array(input_array, rules, not_in_dict_behavior='zero'):
    ...
    elif casted_array.dtype == 'uint8':
        if isinstance(list(rules.values())[0], float):
            reclassified_array = hb.memory_view_reclassify_float_array_by_dict_to_uint8(casted_array, rules)
        else:
            reclassified_array = hb.memory_view_reclassify_uint8_array_by_dict_to_uint8(casted_array, rules)
    else:
        if isinstance(list(rules.values())[0], float):
...
def reclassify_flex(input_flex, rules, output_path, target_datatype, target_nodata=None, compress=False, values_required=False):
    if compress is True:
        gtiff_creation_options = hb.DEFAULT_GTIFF_CREATION_OPTIONS
    ...
    if isinstance(input_flex, hb.ArrayFrame):
        if input_flex.data_type == 7:
            ...
            # target_datatype = input_flex.data_type
            target_nodata = input_flex.ndv
            hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_array_by_dict_to_uint8, output_path,
                                    target_datatype, target_nodata,
                                    gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                    calc_raster_stats=True)
            # hb.reclassify_raster_hb(
