...
def make_vector_path_global_pyramid(input_path, output_path=None, pyramid_index_columns=None, calculate_stats=True, overwrite_stats=False,
                                    clean_temporary_files=False, raise_exception=False, verbose=False):
    ...
    try:
        ...
        raise NameError('Unable to read GPKG at ' + str(input_path) + ' and encountered exception: ' + exception)
    ...
    if 'pyramid_id' not in gdf.columns:
        if verbose:
        ...
        for column_name in pyramid_index_columns:
            # gdf[column_name].astype(np.int64)
            try:
                gdf[column_name] = gdf[column_name].astype(np.int64)
                updated_pyramid_index_columns.append(column_name)
            except:
                columns_to_add_ids.append(column_name)
                updated_pyramid_index_columns.append(column_name + '_as_id')

        for column_name in columns_to_add_ids:
            ...
            unique_values = list(np.unique(gdf[column_name]))
            ...
            gdf[column_name + '_as_id'] = gdf[column_name].replace(replacement_dict)


        for c, column_name in enumerate(updated_pyramid_index_columns):
            if c == 0:
                gdf['pyramid_id'] = gdf[column_name] + 1000
            else:
                temp_series = gdf[column_name] + 1000
                gdf['pyramid_id'] = gdf['pyramid_id'].map(str) + temp_series.map(str)

        unique_pyramid_ids = np.unique(gdf['pyramid_id'])
        dissolved_gdf = None
    ...
    if rewrite_necessary is True:
        ...
        displacement_path = hb.temp('.gpkg', filename_start='displaced_by_make_path_global_pyramid_on_' + str(hb.file_root(input_path)), folder=os.path.split(input_path)[0], remove_at_exit=clean_temporary_files)
        temp_write_path = hb.temp('.gpkg', filename_start='temp_write_' + str(hb.file_root(input_path)), folder=os.path.split(input_path)[0], remove_at_exit=clean_temporary_files)

        try:
            gdf.to_file(temp_write_path, driver='GPKG')
        except NameError as exception:
            raise NameError('Unable to write GPKG. Encountered exception: ' + exception)

        if output_path:
            hb.create_directories(os.path.split(output_path)[0])
            os.rename(temp_write_path, output_path)
            if dissolved_gdf is not None:
                dissolved_output_path = hb.suri(output_path, 'dissolved')
                dissolved_gdf.to_file(dissolved_output_path, driver='GPKG')
        else:
            if os.path.exists(temp_write_path):
                os.rename(input_path, displacement_path)
                os.rename(temp_write_path, input_path)
                if dissolved_gdf is not None:
                    dissolved_output_path = hb.suri(input_path, 'dissolved')
                    dissolved_gdf.to_file(dissolved_output_path, driver='GPKG')
