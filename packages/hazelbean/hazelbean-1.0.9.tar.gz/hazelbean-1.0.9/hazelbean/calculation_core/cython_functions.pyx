...
def reclassify_uint8_array_by_dict_to_uint8(np.ndarray[DTYPEUINT8_t, ndim=2] input_array, dict rules):

    cdef int r, c
    cdef int n_rows = input_array.shape[0]
    cdef int n_cols = input_array.shape[1]

    cdef np.ndarray[DTYPEUINT8_t, ndim=2] output_array = np.empty([n_rows, n_cols], dtype=DTYPEUINT8)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef unsigned char[::, ::1] memory_view_reclassify_uint8_array_by_dict_to_uint8(unsigned char[::, ::1] input_array, dict rules, str not_in_dict_behavior='zero'):
    cdef size_t n_rows = input_array.shape[0]
    cdef size_t n_cols = input_array.shape[1]
    cdef unsigned char[::, ::1] output_array = np.empty([n_rows, n_cols], dtype=DTYPEUINT8)

    if not_in_dict_behavior == 'ignore':
        output_array = np.copy(input_array).astype(DTYPEUINT8)
    elif not_in_dict_behavior == 'zero':
        output_array = np.zeros((n_rows, n_cols)).astype(DTYPEUINT8)

    for r in range(n_rows):
        # print('memory_view_reclassify_uint8_array_by_dict_to_uint8 ' + str(r / n_rows))
        for c in range(n_cols):
            if input_array[r,c] in rules:
                output_array[r,c] = rules[input_array[r,c]]
            # else:
