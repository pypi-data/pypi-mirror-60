            ...
            if fit_type == 'linear':
                ...
                *z, r = res['x']
                ...
                if fit_type == 'circular':
                    ...
                    a0 = np.max(points[:, 1])
                    points = np.vstack([points[:, 0] / a0,
                                       1 - points[:, 1] / a0]).T

                    P, = fit_bashforth_adams(points).x
                    θs, pred = sim_bashforth_adams(P)
                    ϕ[L] = -np.min(θs)
                    ϕ[R] = np.max(θs)
                    ...
                    baseline_width = R0 * a0

                    volume = (a0 ** 3)*np.pi*R0*(R0 + R0*P - 2*np.sin(θ))
                    x = pred[:, 0]*a0 + center
                    y = np.array([np.dot(a, np.power(y, range(len(a)))) + y
                                  for y in (pred[:, 1] - 1)*a0])
                    image_axes.plot(x, y, 'r-')
