...
def sim_bashforth_adams(p=50):
    def bashforth_adams(t, y, p):
        x, z = y
        t = t / 180 * np.pi
        dxdphi = x * np.cos(t) / (x * z + x * p - np.sin(t))
        dzdphi = x * np.sin(t) / (x * z + x * p - np.sin(t))
        return dxdphi, dzdphi

    height = lambda t, y, p: y[1] - 1
    height.terminal = True
    sol_l = solve_ivp(bashforth_adams, (0, -180) , (1e-5, 0), args=(p, ),
                      method='BDF',
                      t_eval=np.linspace(0, -180, num=500), events=height)
    sol_r = solve_ivp(bashforth_adams, (0, 180) , (1e-5, 0), args=(p, ),
                      method='BDF',
                      t_eval=np.linspace(0, 180, num=500), events=height)
...
def fit_bashforth_adams(data, p=50):

    def calc_error(params):
        p, = params

        _, pred = sim_bashforth_adams(p)
    ...
    x_0 = (p, )
    bounds = [[0,1000]]
    # opt = dual_annealing(calc_error, bounds,
    #                      x0=x_0, maxiter=20,
    #                      local_search_options={'method':'TNC',
    #                                            'options':{'maxiter':1000,
    #                                                       'disp':True}})

    opt = minimize(calc_error, x_0, method='Nelder-Mead',
                   options={'disp':False})
    return opt
