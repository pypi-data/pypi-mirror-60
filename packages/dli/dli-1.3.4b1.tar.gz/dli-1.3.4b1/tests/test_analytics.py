...
class TestAnalyticsHandler:
    ...
    def test_handles_internal_exceptions(self, dli_client):
        ...
        session = Mock(post=Mock(side_effect=exception_to_be_thrown), has_expired=Mock(return_value=False))
        dli_client._session = session
        dli_client._session.has_expired.return_value = False
    ...
    def test_event_not_sent_when_client_has_expired(self, dli_client):
        dli_client._session.has_expired.return_value = True
        analytics_handler = AnalyticsHandler(dli_client)
        properties = {
            'package_id': '1',
            'dataset_id': '2',
