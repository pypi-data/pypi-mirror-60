...
class TimeSeriesEtl:
    ...
    def get_waze_alerts_ts(self, table: str, type: str, period: int, from_date: datetime = None) -> pd.DataFrame:
        ...
        query = "select * from {0}.{1} where type = '{2}' {3} order by pubMillis".format(
            self.schema, table, type, date_constraint)
        df = self.dao.execute_query(query)
        df['start_date'] = pd.to_datetime(df['creationMillis'], unit='ms').dt.floor(str(period) + 'min')
    ...
    def add_net_info_to_waze_ts(self, waze_ts: pd.DataFrame, net_service: NetworkService, max_dist: float = 0.00003):
        """
        ...
        def add_network_data(row: pd.Series):
            # We have "tested" that this value of max_dist is enough to take links that are close to the point without
        ...
        # There are a series of columns in the schema that are not important and that we can drop
        non_important_cols = ['link_country', 'link_county', 'link_district', 'link_frc', 'link_geom', 'link_linearid',
                              'link_miles', 'link_oid', 'link_roadnumber', 'link_shapesrid', 'link_specialroa',
                              'link_state', 'link_xdgroup']
        return enriched_waze.drop(non_important_cols, axis=1)
