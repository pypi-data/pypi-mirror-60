...
class Job:
    def __init__(self, stream_id: Optional[UUID], run_at: datetime, status: JobStatus, stream_collection_id: Optional[UUID], id: UUID, remaining_time_millis: int, percentage_progress: int, created_by: UUID, errors: Optional[str], ended_at: Optional[datetime]):
...
def _job_from_payload(json: dict) -> Union[DtlError, Job]:
    ...
    return Job(stream_id, run_at, status, stream_collection_id, job_id,
               remaining_time_millis, percentage_progress, created_by, errors, ended_at)
