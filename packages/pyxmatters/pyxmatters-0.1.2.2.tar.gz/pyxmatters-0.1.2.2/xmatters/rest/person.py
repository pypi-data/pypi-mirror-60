    ...
    def getPerson(self, id, filter='?embed=roles,supervisors', retry=0):
        ...
        try:
            ...
            if (xMattersAPI.statusCodeSuccess(response.status_code)):
                ...
                self.log.debug(defName + 'Error occured while retrieving Person: ' + id + ' Response: ' + str(response.content))
                jsonStr = None
    ...
    def getPeople(self, filter='?embed=roles,devices&offset=0&limit=1000', retry=0):
        ...
        try:
            ...
            if (xMattersAPI.statusCodeSuccess(response.status_code)):
                ...
                self.log.debug(defName + 'Error occured while retrieving People. Response: ' + str(response.content))
                jsonStr = None
        except Exception as e:
            self.log.error(defName + 'Unexpected exception:' + str(e))
            jsonStr = None
    ...
    def createPerson(self, data, retry=0):
        ...
        try:
            ...
            if (xMattersAPI.statusCodeSuccess(response.status_code)):
                ...
                self.log.debug(defName + 'Error occured while creating Person: ' + name + ' Response: ' + str(response.content))
                jsonStr = None
        except Exception as e:
            self.log.error(defName + 'Unexpected exception:' + str(e))
            jsonStr = None
    ...
    def modifyPerson(self, data, retry=0):
        ...
        try:
            ...
            if (xMattersAPI.statusCodeSuccess(response.status_code)):
                ...
                self.log.debug(defName + 'Error occured while Modifying Person: ' + name + ' Response: ' + str(response.content))
                jsonStr = None
        except Exception as e:
    ...
    def removePerson(self, id, retry=0):
        ...
        try:
            ...
            if (xMattersAPI.statusCodeSuccess(response.status_code)):
                jsonStr = response.json()
                self.log.debug(defName + json.dumps(jsonStr))
                self.log.debug(defName + 'Person removed: ' + jsonStr['targetName'] + '. Response: ' + str(response.content))
            elif (xMattersAPI.tooManyRequests(response.status_code)):
                self.log.error(defName + 'Status Code: '+str(response.status_code)+'. Too many requests.')
                if (retry < 3):
                    retry = retry+1
                ...
                self.log.debug(defName + 'Error occured while removing Person: ' + id + ' Response: ' + str(response.content))
                jsonStr = None
        except Exception as e:
            self.log.error(defName + 'Unexpected exception:' + str(e))
            jsonStr = None
