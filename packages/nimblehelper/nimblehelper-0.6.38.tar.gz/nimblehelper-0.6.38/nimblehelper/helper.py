...
class NimbleHelper:
    ...
    def run_request(cls, gateway_url, x_consumer_id, params, soap_action, service_name, logger, nim_headers):
        ...
        args = {
            "action": soap_action,
            "data": Utils.adjust_string_length(json.dumps(params), max_request_response_size),
            "headers": headers,
            "url": gateway_url,
        ...
        logger.debug({
            ...
            "response_content": Utils.adjust_string_length(decoded_response, max_request_response_size)
        })
        ...
        if "status" in response and int(response["status"]) >= 400:
            logger.error({
                ...
                "type": "Error",
                "user": None,
                "data": Utils.adjust_string_length(decoded_response,
    ...
    @classmethod
    def request_manager(cls, gateway_url, soap_action, params, x_consumer_id, hierarchy, service_name, logger=None, nim_headers=None):
        if not nim_headers:
            nim_headers = dict()
