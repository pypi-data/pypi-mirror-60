...
class Analytics(object):
    ...
    def report2(self, df):
        ...
        pdfkit.from_file("report/index.html", "report/report.pdf")
    ...
    def __product_stats_drawing(self, df, report):
        ...
        plt.barh(ind, match, label='Match', color='#1de9B6')
        plt.barh(ind, no_match, label='No Match', color='#29b6f6', left=match)
        plt.barh(ind, error, label='Error', color='#b39ddb', left=match+no_match)
    ...
    def __extract_data_for_match_rate_plot_from_report(self, report):
        return report[["product_name", "product_match_rate", "product_error_rate"]].drop_duplicates().reset_index(drop=True)
    ...
    def __data_stats(self, df, report):
        ...
        counts = {}
        for col in list(df):
            if not col.startswith("inputs."):
                cleaned_col = col.split(".", 1)[-1] # chop provider name
                cleaned_col = cleaned_col.replace("[*]", "[]")
                if cleaned_col in ['client_id', 'row_id', 'error', 'is_hit']:
                    continue
                output = outputs.loc[outputs["attribute"] == cleaned_col]
                if len(output) == 0:
                    type = "Unknown"
                else:
                    type = output["type"].iloc[0]
                count = counts.get(type)
                if count:
                    counts[type] = count + 1
                else:
                    counts[type] = 1
        top = { k: v for k, v in sorted(counts.items(), key=lambda item: item[1]) }
        types = list(top.keys())[:10]
    ...
    def __generate_html(self, df, report):
        ...
        text_file.write("<h1 style='margin: auto;width: 50%;padding: 10px;font-family:Lato Regular;font-size:32px;font-weight:400;line-height:38px;letter-spacing:-0.3px;color:#387BA0'>Data Quality Report</h1>\n")
        text_file.write("<h2 style='font-size:24px;font-weight:300;line-height: 32px;letter-spacing:-0.22px;color: #4CA9BD;font-face:Roboto'>Product Stats</h2>\n")
        text_file.write(self.__generate_product_stats(df, report))
        text_file.write("<h2 style='font-size:24px;font-weight:300;line-height: 32px;letter-spacing:-0.22px;color: #4CA9BD;font-face:Roboto'>Data Types</h2>\n")
        text_file.write(self.__generate_data_stats(df, report))
        text_file.write("<h2 style='font-size:24px;font-weight:300;line-height: 32px;letter-spacing:-0.22px;color: #4CA9BD;font-face:Roboto'>Attribute Stats</h2>\n")
        text_file.write("<h3 style='font-face:Roboto;font-size:18px;font-weight:300;line-height:27px;letter-spacing:0;color:#64BAE8'>Top Attributes by Fill Rate</h3>\n")
        text_file.write("<img src='most_filled.png'>\n")
        text_file.write("<h3 style='font-face:Roboto;font-size:18px;font-weight:300;line-height:27px;letter-spacing:0;color:#64BAE8'>Least Filled Attributes</h3>\n")
        text_file.write("<img src='least_filled.png'>\n")
        text_file.write("<h3 style='font-face:Roboto;font-size:18px;font-weight:300;line-height:27px;letter-spacing:0;color:#64BAE8'>Complete list of Attributes</h3>\n")
...
#        text_file.write(report.to_html(index=False, classes="attribute_stats"))
    ...
    def __most_filled_attributes_drawing(self, df, report):
        ...
        objects = list(report['attribute_name'][:20])
        y_pos = np.arange(len(objects))
        performance = list(report['attribute_fill_rate'][:20])
    ...
    def __generate_complete_list_of_attributes(self, df, report):
        s = "<table margin='2%'><tr><th align=left><h3>Attribute Name</h3></th><th align=left><h3>Count/Percentage</h3></th></tr>\n"
        for index, row in report.iterrows():
            s += "<tr>\n"
            s += "<th align=left valign=top>" + row["attribute_name"] + "</th>\n"
            s += "<td>\n"
            s += "<table>\n"
            s += "<tr><td align=right>&nbsp;</td><td>&nbsp;</td></tr>\n"
            s += "<tr><td align=right>Blank</td><td>" + str(row["attribute_fill_rate"]) + "</td></tr>\n"
            s += "<tr><td align=right>Non Blank</td><td>" + str(100 - row["attribute_fill_rate"]) + "</td></tr>\n"
            s += "<tr><td align=right>Unique</td><td>" + str(row["unique_values"]) + "</td></tr>\n"
            s += "<tr><td align=right>Most Common Values</td><td>" + str(row["most_common_values"]) + "</td></tr>\n"
            s += "</table>\n"
            s += "</td>\n"
            s += "</tr>\n"
        return s
