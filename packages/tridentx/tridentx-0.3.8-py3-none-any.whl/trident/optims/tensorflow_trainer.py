...
class Model(ModelBase):
    ...
    def _initial_graph(self, inputs=None, output=None, input_shape=None):
        ...
        for i in range(len(self.model.outputs)):
            self.targets[self.model.output_names[i]] = Input(self.model.outputs[i].shape,
                                                             name='target_' + self.model.output_names[i])
    ...
    def do_gradient_update(self, log_gradients=False):
        ...
        self.optimizer.apply_gradients(zip(self.training_context['grads'],self.model.trainable_variables))  # loss, metrics = self.model.train_on_batch(self.training_context['current_input'], self.training_context['current_target'])  # if self.training_context['is_collect_data']:  #     if len(loss)==1:  #         self.training_context['losses']['total_losses'] = loss[0]  #         self.training_context['current_loss']= loss[0]  #         k=list(self.losses.keys())[0]  #         if  k not in self.training_context['losses']:  #             self.training_context['losses'][k] = []  #         self.training_context['losses'][k]= loss[0]  #     else:  #         self.training_context['losses']['total_losses'] = loss[0]  #         self.training_context['current_loss'] = loss[0]  #         n=1  #         for k, v in self._losses.items():  #             if k not in self.training_context['losses']:  #                 self.training_context['losses'][k] = []  #                 self.training_context['losses'][k] = loss[n]  #                 n+=1  #     n=0  #     for k, v in self._metrics.items():  #         if k not in self.training_context['metrics']:  #             self.training_context['metrics'][k] = []  #             self.training_context['metrics'][k] = metrics[n]  #             n += 1
        ...
        try:
            ...
            with tf.GradientTape() as tape:
                ...
                for k, v in self._losses.items():
                    ...
                    this_loss = v(target, output) if callable(v) else v.call(target,
                                                                             output)  # v.call(target,output) if
                    # hasattr(v, 'call') else
                    self.training_context['current_loss'] = self.training_context[
                                                                'current_loss'] + this_loss * loss_weight
                    if is_collect_data:
                ...
                for k, v in self._output_regs.items():
                    ...
                    this_loss = v(output)
                    self.training_context['current_loss'] = self.training_context['current_loss'] + this_loss  # self
                ...
                if is_collect_data:
                    self.training_context['losses']['total_losses'].append(
                        self.training_context['current_loss'].numpy())
                ...
                for k, v in self._metrics.items():
                    ...
                    if is_collect_data:
                        self.training_context['metrics'][k].append(
                            v.call(test_target, output).numpy() if hasattr(v, 'forward') else v(test_target, output).numpy())
