...
class Conv2d_Block(Layer):
    def __init__(self, kernel_size=(3,3), num_filters=None, strides=1, auto_pad=True,padding_mode=PaddingMode.replicate, activation='relu',
        ...
        if normalization.lower().strip() in ['spectral','spec','sp' ,'s'] or isinstance(normalization,SpectralNorm):
    ...
    def build(self, input_shape):
        if self._built == False:
            ...
            output_shape[0] = self.num_filters
            if self.is_spectral:
                self.conv = SpectralNorm(conv).to(_device)
            else:
                self.conv = conv
            if self.norm != None and not self.is_spectral:
                self.norm.input_shape=output_shape
            self.to(self.device)
    def forward(self, *x):
        x=enforce_singleton(x)
        if self.add_noise==True and self.training == True:
            ...
            x=x+noise
        x = self.conv(x)
        if self.norm is not None and not self.is_spectral:
            x = self.norm(x)
        if self.activation is not None:
            x = self.activation(x)
        if self.dropout_rate > 0:
