...
def load_mnist(dataset_name='mnist', **kwargs):
    ...
    for kind in ['train', 'test']:
        labels_file = '{0}-labels-idx1-ubyte.gz'.format('t10k' if dataset_name == 'mnist' and kind == 'test' else kind)
        images_file = '{0}-images-idx3-ubyte.gz'.format('t10k' if dataset_name == 'mnist' and kind == 'test' else kind)
        ...
        with gzip.open(images_path, 'rb') as imgpath:
            ...
            imagedata = ImageDataset(images, expect_image_type=ExpectImageType.gray,
...
def load_cifar(dataset_name='cifar10', kind='train', is_flatten=None, is_onehot=None):
    ...
    if is_onehot == True:
        if _backend == 'pytorch':
            warnings.warn('Pytorch not prefer onehot label, are you still want onehot label?', category='dataloading',
                          stacklevel=1, source='load_cifar')
