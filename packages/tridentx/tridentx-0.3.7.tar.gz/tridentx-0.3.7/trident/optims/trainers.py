...
class ModelBase(object):
    ...
    def reset_training_context(self):
        self.training_context = {'losses': OrderedDict(),  # loss_wrapper
                                 ...
                                 'loss_history': None, 'metric_history': None, 'base_lr': self.base_lr,  # current loss
                                 'current_lr': None,  # current loss
                                 'save_path': None, 'callbacks': self.callbacks}
    ...
    def print_batch_progress(self, print_batch_progress_frequency):
        ...
        progress_bar(self.training_context['current_batch'], self.training_context['total_batch'],
            'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.3e}| epoch: {3}'.format(
                np.array(self.training_context['losses']['total_losses'][-1 * slice_cnt:]).mean(), ','.join(
                    ['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v[-1 * slice_cnt:]).mean()) for k, v in
                     self.training_context['metrics'].items()]), self.training_context['current_lr'],
    ...
    def print_epoch_progress(self, print_epoch_progress_frequency):
        progress_bar(self.training_context['current_epoch'], self.training_context['total_epoch'],
                     'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}'.format(
                         self.training_context['losses']['total_losses'], ','.join(
                             ['{0}: {1:<8.3%}'.format(snake2camel(k), v) for k, v in
                              self.training_context['metrics'].items()]), self.training_context['current_lr']),
                     name=self.name)
    ...
    def train_model(self, input, target, current_epoch, current_batch, total_epoch, total_batch, is_collect_data=True,
                    is_print_batch_progress=True, is_print_epoch_progress=True, log_gradients=False, log_weights=False,
                    accumulate_grads=False, test_input=None, test_target=None):
        try:
            ...
            test_input, test_target = self.do_on_data_received(test_input, test_target)
            ...
            if accumulate_grads == False:
                ...
                for k, v in self._metrics.items():
                    ...
                    self.training_context['tmp_metrics'][k].append(
                        float(to_numpy(v.forward(output, test_target))) if hasattr(v, 'forward') else float(
                            to_numpy(v(output, test_target))))
...
class TrainingPlan(object):
    ...
    def start_now(self, collect_data_inteval=1):
        try:
            ...
            if self.tile_image_save_path is None:
                self.tile_image_save_path = 'Results'
            if self.tile_image_save_path is not None:
                folder, file = os.path.split(self.tile_image_save_path)
                if len(folder) == 0:
                    folder = self.tile_image_save_path
                if not os.path.exists(folder):
                    try:
                        os.makedirs(folder)
                    except Exception as e:
                    ...
                    for mbs, iter_data in enumerate(data_loader):
                        ...
                        if len(iter_data) == 1:
                            ...
                            if iter_testdata is not None:
                                test_input = iter_testdata[0]
                                test_target = test_input.copy()

                        if len(iter_data) == 2:
                            input, target = iter_data[0], iter_data[1]
                            if iter_testdata is not None:
                                test_input, test_target = iter_testdata[0], iter_testdata[1]

                        # input, target = Variable(input).to(self.device), Variable(target).to(self.device)

                        for k, trainitem in self.training_items.items():
                            trainitem.train_model(input.copy(), target.copy(), epoch, mbs, self.num_epochs,
                        ...
                        if self.need_tile_image == True and self.tile_image_unit == 'batch' and mbs % self.tile_image_frequency == 0:
                            ...
                            if self.tile_image_include_output:
                                ...
                                output_arr = to_numpy(output).transpose(
                                    [0, 2, 3, 1]) if _backend != 'tensorflow' else to_numpy(output)
                        ...
                        if (mbs > 0 and mbs % self.tile_image_frequency == 0) :#or ( mbs == len(data_loader.batch_sampler) - 1):
                            loss_history = []
                            metric_history = []

                            for trainitem in self.training_items.value_list:
                                loss_hist = OrderedDict()
                                if 'total_losses' not in loss_hist:
                                    loss_hist['total_losses'] = []
                                loss_hist['total_losses'].extend(trainitem.batch_loss_history['total_losses'])
                                loss_hist['total_losses'].extend(trainitem.training_context['losses']['total_losses'])
                                loss_history.append(loss_hist)

                                metric_hist = OrderedDict()
                                for k, v in trainitem.training_context['metrics'].item_list:
                                    if k not in metric_hist:
                                        metric_hist[k] = []
                                    if k in trainitem.batch_metric_history:
                                        metric_hist[k].extend(trainitem.batch_metric_history[k])
                                    metric_hist[k].extend(v)
                                metric_history.append(metric_hist)

                            #
                            # loss_history = [trainitem.batch_loss_history+trainitem.training_context['losses'] for trainitem in self.training_items.value_list]
                            # metric_history = [trainitem.batch_metric_history+trainitem.training_context['metrics']  for trainitem in self.training_items.value_list]
                            #
    ...
    def resume(self):
        if self.execution_id is None:
            ...
            print('collect_data_inteval : ', current_batch)
            print('resume start')
            ...
            for epoch in range(current_epoch, self.num_epochs):
                try:
                    input = None
                    target = None
                    for mbs1, iter_data in enumerate(data_loader):
                        mbs = current_batch + mbs1
                        if len(iter_data) == 1:
                            input = iter_data[0]
                            target = input.copy()
                        if len(iter_data) == 2:
        ...
        try:
            ...
            for mbs, iter_data in enumerate(data_loader):
                if mbs < num_steps:
                    ...
                    for k, trainitem in self.training_items.items():
                        trainitem.train_model(input, target, 0, mbs, 1, num_steps,
                                              is_collect_data=mbs % collect_data_inteval == 0,
                                              is_print_batch_progress=self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0,
