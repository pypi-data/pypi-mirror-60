...
#	2019-01-04	Added check for data rows with more columns than column headers.
#				Version 1.1.0. RDN.
#	2020-01-26	Added an option to check that the order of columns in the CSV
#				file is the same as in the specifications.  RDN.
# ============================================================================
...
class CsvChecker():
	...
	def __init__(self, fmt_spec, colname, column_required_default, data_required_default, column_position):
		self.name = colname
...
Arguments:
  CSV file name   The name of a comma-separated-values file to check."""
	...
	parser.add_option("-p", "--position", action="store_true", dest="position", default=False,
						help="Position (order) of columns in the CSV file must match that in the specification.")
...
def read_format_specs(fmt_file, column_required, data_required, chkopts="chkcsvoptions"):
	...
	:rtype: A dictionary of column type checking functions, indexed by column name.
	"""
	...
	cols = {}
	for i, col in enumerate(speccols):
		cols[col] = CsvChecker(fmtspecs, col, column_required, data_required, i)
	return cols


def check_csv_file(csv_fname, cols, halt_on_err, columnexit, \
		linelength, caseinsensitive, encoding=None, match_position=False):
	...
	:param match_position: Whether or not the position (order) of columns in the CSV file must match that in the specifications.
	:rtype: A list of error messages as strings.
	"""
	errorlist = []
	...
	if len(req_missing) > 0:
		...
		return errorlist
	# Exit if there are extra columns and either the option to exit is set or the column positions must match.
	if columnexit or match_position:
		if caseinsensitive:
		...
		if len(extra) > 0:
			...
			return errorlist
	# Report an error if the position (order) of columns is required to be the same and it is not.
	if match_position:
		spec_col_order = [c[0] for c in sorted([(c[1].name, c[1].column_position) for c in cols.items()], key=lambda p: p[1])]
		same_order = True
		if caseinsensitive:
			colnames_l = [ c.lower() for c in colnames ]
			specnames_l = [ c.lower() for c in spec_col_order ]
			for i, cname in enumerate(colnames_l):
				if specnames_l[i] != cname:
					same_order = False
					break
		else:
			for i, cname in enumerate(colnames):
				if spec_col_order[i] != cname:
					same_order = False
					break
		if not same_order:
			errorlist.append(("The order of columns in the CSV file is not the same as in the specifications", csv_fname, 1))
...
def main():
	...
	errorlist = check_csv_file(csv_file, cols, opts.haltonerror, 
		opts.columnexit, opts.linelength, opts.caseinsensitive, opts.encoding, opts.position)
	if len(errorlist) > 0:
