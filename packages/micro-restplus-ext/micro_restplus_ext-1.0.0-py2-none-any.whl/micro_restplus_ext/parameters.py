...
class PutFormParameters(Parameters):

    @classmethod
    def perform_update(cls, operations, obj, state=None):
        """
        ...
        if state is None:
            state = {}
        # log.info(type(operations[0]))

        for operation, value in operations.items():
            if not cls._process_update_operation({'field_name': operation, 'value': value},
                                                 obj=obj, state=state):
                log.info(
    ...
    @classmethod
    def _process_update_operation(cls, operation, obj, state):
        """
        Args:
