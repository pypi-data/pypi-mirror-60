...
from .labrun.lab import LabRunner, Resources
...
@click.option("--cpu", default=None, help="Amount of CPU to use, e.g., 0.5")
@click.option("--memory", default=None, help="Amount of memory to use, e.g., 2G")
@click.option("--ephemeral-storage", default=None, help="Amount of ephemeral storage, e.g., 3G")
@click.option("--gpu", default=None, help="Amount of GPU to use, e.g., 1")
@click.option(
...
def submit(
    ...
    :param Optional[str] cpu: CPU limit for run
    :param Optional[str] memory: Memory limit for run
    :param Optional[str] ephemeral-storage: Ephemeral storage to use
    :param Optional[str] gpu: GPU limit for run
    ...
    lab_runner.run(
        ...
        resources=Resources(cpu=cpu, memory=memory, ephemeral_storage=ephemeral_storage, gpu=gpu),
...
@click.pass_obj
def logs(obj, run_id: str, run_token: str) -> None:
    logger, lab_runner = obj
    url = lab_runner.get_logs_url(run_id, run_token)
    print(url)
