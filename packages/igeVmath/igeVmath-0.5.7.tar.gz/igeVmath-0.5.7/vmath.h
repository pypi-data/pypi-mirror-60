...
void vmath_mat4_fromTransform(const float* pos, const float* rot, int rotSize, const float* scale, const float* shear, float* out) {
...
void vmath_linearCombine(const float* a, const float* b, float a1, float b1, float* out) {
	out[0] = (a1 * a[0]) + (b1 * b[0]);
	out[1] = (a1 * a[1]) + (b1 * b[1]);
	out[2] = (a1 * a[2]) + (b1 * b[2]);
}
...
float RoundingFloat(float roundNum) {
...
void vmath_mat4_toTransform(const float* mat, bool rotEuler, float* pos, float* rot, float* scale, float* shear) {
