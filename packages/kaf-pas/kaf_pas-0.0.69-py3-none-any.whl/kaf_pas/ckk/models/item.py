            ...
            try:
                for item in Item_refs.objects.get_descendants(id=item.id, order_by_clause='order by level desc'):
                    if not soft_delete:
                        ...
                        for item_lotsman_document in Item.objects.filter(id=item.child_id, lotsman_document__isnull=False):
                            item_lotsman_document.lotsman_document.props &= ~Lotsman_documents_hierarcy.props.beenItemed
                            item_lotsman_document.lotsman_document.save()
                            item_lotsman_document_cnt += 1
