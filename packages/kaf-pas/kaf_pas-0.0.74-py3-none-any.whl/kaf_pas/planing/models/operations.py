            ...
            try:
                ...
                for item in Operation_refs.objects.get_descendants(id=operation.id, order_by_clause='order by level desc'):
                    if not soft_delete:
                        ...
                        Operations.objects.filter(id=item.child_id).delete()
                    else:
                        Operations.objects.filter(id=item.child_id).soft_delete()
                ...
                with connection.cursor() as cursor:
                            ...
                            if len(outcome_items.find(lambda outcome_item: outcome_item.item_id == child_id)) == 0:
                                ...
                                if cnt1 > 0:
                                    ...
                                    for launch_operations_item in Launch_operations_item.objects.filter(item_id=child_id, launch_id=launch_id).order_by('num'):
                                        ...
                                        Operation_refs.objects.get_or_create(parent=income_operation, outcome=outcome_operation)
                    ...
                    for row in rows:
                        ...
                        for parents_row in parents_rows:
                            ...
                            Operation_refs.objects.get_or_create(parent=_child.last_operation, outcome=first_operation.last_operation)
                            deleted, _ = Operation_refs.objects.filter(parent__isnull=True, outcome=first_operation.last_operation).delete()
                        progress.setCntDone(cnt=cnt, id=id_progress1)
                        cnt += 1
