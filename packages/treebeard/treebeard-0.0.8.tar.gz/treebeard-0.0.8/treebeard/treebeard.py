...
from treebeard.helper import get_notebook_name

local_url = 'http://localhost:8080/notebooks/'
remote_url = "https://scheduler-cvee2224cq-ew.a.run.app/notebooks/"
...
def run(t):
    ...
    endpoint = f"{remote_url}{notebook_name}"
...
def cancel():
    ...
    click.echo(f"...run request submitted! Check status with `treebeard status`")
    spinner.stop()
...
def status():
    ...
    for run in json_data:
        ...
        start_time = parser.isoparse(run['build_start_time'])
        time_string = timeago.format(start_time, now=now)
        run_id = run['runId']
        click.echo(f"http://treebeard.io/admin/{run_id} {time_string}")
