...
class Store(object):
    ...
    @db_session
    def update(self, condition, data=None, meta=None, fuzzy=True, patch='jsonpath', force=False, begin=begin, end=end, debug=False):
        elems, _ = self.search(condition, mode='raw', fuzzy=fuzzy, debug=debug)
    ...
    @db_session
    def delete(self, condition, fuzzy=True, begin=None, end=None, debug=False):
        elems, _ = self.search(condition, mode='raw', fuzzy=True, debug=debug)
        elems = self.adjust_slice(elems, for_update=True, begin=begin, end=end)
        for elem in elems:
            elem.delete()
