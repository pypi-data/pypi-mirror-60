...
class Group(HLObject, MutableMappingHDF5):
    ...
    def _get_link_db(self):
        if self._link_db is None:
            ...
            if self._link_db is None:
                # Couldn't get from objdb, make request to server
                req = "/groups/" + self.id.uuid + "/links"
                rsp_json = self.GET(req)
                links_json = rsp_json['links']

                self._link_db = {}
                # construct a dict from the link list we get from the server
    ...
    def _h5path_is_member(self, h5path):
        # return True if the given h5path should be a member of this group,
        # false if it is anothere group
        if h5path[0] == '/' and h5path[1:].find('/') == -1 and self._isroot():
            in_group = True
            h5path = h5path[1:]  # normalize the name
        elif h5path.find('/') == -1:
            in_group = True
        else:
            in_group = False  
    ...
    def _get_link_json(self, h5path):
        ...
        if self._h5path_is_member(h5path) and h5path in linkdb:
            # found in linkdb, return it
        ...
        if tgt_json and self._h5path_is_member(h5path):
    ...
    def __delitem__(self, h5path):
        ...
        # remove from link from linkdb if it's a member
        if self._h5path_is_member(h5path):
            if h5path[0] == '/':
                name = h5path[1:]  # normalize
            else:
                name = h5path
            link_db = self._get_link_db()
    ...
    def __contains__(self, name):
        ...
        if name == '/':
            return True  # root is everywhere!

        if name.startswith("/") and self._isroot():
            name = name[1:]
