...
class PurgeHistoryRestServlet(RestServlet):
    PATTERNS = client_patterns("/purge_history/(?P<room_id>[^/]*)(/(?P<event_id>[^/]+))?", v1=True)
    ...
    async def on_POST(self, request, room_id, event_id):
        ...
        requester = await self.auth.get_user_by_req(request)
        power_level_event = await  self.auth.state.get_current_state(
        ...
        if power_level_event:
            ...
            power_level = event_auth.get_user_power_level(requester.user.to_string(), auth_events)
            logger.info("1111 %d", power_level)

        if power_level < 100:
            raise SynapseError(403, "You don't have permission.")
...
class PurgeHistoryStatusRestServlet(RestServlet):
    PATTERNS = client_patterns( "/purge_history_status/(?P<purge_id>[^/]+)", v1=True)
