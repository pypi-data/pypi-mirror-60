...
def compute_plipcsv_rows(plipxml, structrure_id=None, ligand_ids=None):
    """
    ...
    Args:
        ...
        ligand_id: ligands selection, keep all ligands if None
    ...
    for bs_elt in get_bindingsite_elts(plipxml):
        ligand_id = get_ligand_id(bs_elt)
        if ligand_ids is None or ligand_id == ligand_ids or ligand_id in ligand_ids:
...
def get_plipcsv_columns():
    return ['structure_id', 'ligand_id'] + plip_lig_property_names + ['interaction'] + plip_interaction_property_names


def compute_plipcsv_df(plipxml, structrure_id=None, ligand_ids=None):
    df = pd.DataFrame(compute_plipcsv_rows(plipxml, structrure_id, ligand_ids), columns=get_plipcsv_columns())
    return df
