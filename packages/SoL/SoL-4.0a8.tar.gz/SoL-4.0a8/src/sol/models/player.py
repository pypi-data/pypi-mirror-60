...
class Player(GloballyUnique, Base):
    ...
    birthdate = Column(
        ...
        info=dict(label=_('Birthdate'),
                  hint=_('Date of birth of the player, needed for “Under XX” rankings.')))
    ...
    def obfuscatedFirstname(self):
        a32 = adler32(self.firstname.encode('ascii', errors='xmlcharrefreplace')
                      + self.guid.encode('ascii'))
        suffix = b32encode(a32.to_bytes(4, 'little')).rstrip(b'=').decode('ascii')
        return self.firstname[0] + suffix.lower()

    @property
    def obfuscatedLastname(self):
        a32 = adler32(self.lastname.encode('ascii', errors='xmlcharrefreplace')
                      + self.guid.encode('ascii'))
        suffix = b32encode(a32.to_bytes(4, 'little')).rstrip(b'=').decode('ascii')
        return self.lastname[0] + suffix.lower()

    @property
    def obfuscatedNickname(self):
        nickname = self.nickname
        if nickname:
            a32 = adler32(nickname.encode('ascii', errors='xmlcharrefreplace')
                          + self.guid.encode('ascii'))
            suffix = b32encode(a32.to_bytes(4, 'little')).rstrip(b'=').decode('ascii')
            return nickname[0] + suffix.lower()
        else:
            return nickname
    ...
    def caption(self, html=None, localized=True, css_class=None, ignore_privacy=False):
        ...
        if ignore_privacy or self.acceptedDiscernibility():
