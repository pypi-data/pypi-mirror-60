    ...
    if username and password:
        ...
        if adminuser and username == adminuser:
            ...
            if user is None or user.state != 'C' or not user.check_password(password):
                if user is not None and user.state != 'C':
...
def lost_password(request):
    ...
    if email:
        ...
        try:
            ...
            if user.state != 'C':
                logger.warning('Password reset request for not yet confirmed user "%s" from %s'
                               ' (%s)', email, ipaddress, hostname)
                return {'success': False,
                        'message': t(NO_SUCH_USER),
                        'errors': {'email': t(INVALID_EMAIL)}}
...
def reset_password(request):
    ...
    if user is None or user.state != 'C':
        logger.warning('Password reset token %r is valid, but user is not confirmed'
                       ' or has been deleted', id)
        raise HTTPGone()
...
def change_password(request):
    ...
    if user_id is not None and oldpassword is not None and newpassword is not None:
        ...
        if user.check_password(oldpassword):
            ...
            logger.info('Password reset for "%s" completed')
