...
from ..common.constants import FILENAME,PACKAGE,SETVALUE_CALL,CRYPTO_USE,CRYPTO_IN_SETVALUE,CONFIDENCE_LEVEL_SETVALUE, NET_USE,LOCATION_SETVALUE ,LOCATION_CRYPTO_SETVALUE , NUM_SETVALUE_METHODS, ALL_SETVALUE_METHODS, TIME_TAKEN_SETVALUE, APP_NAME, APP_VERSION_CODE, APP_VERSION_NAME
# Custom constants.
METHOD_INTERNAL = 0
METHOD_EXTERNAL = 1
METHOD_EMPTY = 2
...
class WorkerSetvalue():
    ...
    def analyse_apk(self,filename):
        ...
        try:
            ...
            print("Exception with Androguard")
            print(sys.exc_info()[2])
            output = {FILENAME:filename,PACKAGE:"Error",SETVALUE_CALL:"Error",
                        CRYPTO_USE:"Error", CRYPTO_IN_SETVALUE:"Error",
                         CONFIDENCE_LEVEL_SETVALUE: "Error" , NET_USE: "Error" ,LOCATION_SETVALUE: "Error" ,LOCATION_CRYPTO_SETVALUE: "Error" , NUM_SETVALUE_METHODS: "Error" , ALL_SETVALUE_METHODS: "Error", TIME_TAKEN_SETVALUE: str(runtime) }
        ...
        if ((self.androguard_a == None) or 
                ...
                (self.androguard_dx == None)):
            runtime = self.elapsed_time()
            print("Androguard objects empty")
            output = {FILENAME:filename,PACKAGE:"Error",SETVALUE_CALL:"Error",
                        CRYPTO_USE:"Error", CRYPTO_IN_SETVALUE:"Error",
                         CONFIDENCE_LEVEL_SETVALUE: "Error" , NET_USE: "Error" ,LOCATION_SETVALUE: "Error" ,LOCATION_CRYPTO_SETVALUE: "Error" , NUM_SETVALUE_METHODS: "Error" , ALL_SETVALUE_METHODS: "Error", TIME_TAKEN_SETVALUE: str(runtime) }
            sess.reset()
        ...
        apk_version_code = self.androguard_a.get_androidversion_code()
        apk_version_name = self.androguard_a.get_androidversion_name()
        apk_app_name = self.androguard_a.get_app_name()
        ...
        try:
            ...
            if self.found_crypto != True:
                pass
            runtime = self.elapsed_time()
            output = {FILENAME:filename,PACKAGE:apk_package,APP_VERSION_CODE:apk_version_code,         
                        APP_VERSION_NAME:apk_version_name, APP_NAME: apk_app_name,SETVALUE_CALL:str(self.setvalue),
                        CRYPTO_USE:str(self.crypto), CRYPTO_IN_SETVALUE:str(self.found_crypto),
                         CONFIDENCE_LEVEL_SETVALUE: self.confidence_level , NET_USE: str(self.net_use) ,LOCATION_SETVALUE: str(self.location_setvalue) ,LOCATION_CRYPTO_SETVALUE: str(self.location_ble_crypto)  , NUM_SETVALUE_METHODS: str(self.num_ble_methods) , ALL_SETVALUE_METHODS: str(self.all_ble_methods), TIME_TAKEN_SETVALUE: str(runtime) }
            sess.reset()
            sleep(0.1)
            return output
        except Exception as e:
            runtime = self.elapsed_time()
            print("Failure in initial checks")
            print(sys.exc_info()[2])
            output = {FILENAME:filename,PACKAGE:apk_package,SETVALUE_CALL:"Error",
                        CRYPTO_USE:"Error", CRYPTO_IN_SETVALUE:"Error",
                         CONFIDENCE_LEVEL_SETVALUE: "Error" , NET_USE: "Error" ,LOCATION_SETVALUE: "Error" ,LOCATION_CRYPTO_SETVALUE: "Error" , NUM_SETVALUE_METHODS: "Error" , ALL_SETVALUE_METHODS: "Error", TIME_TAKEN_SETVALUE: str(runtime) }
            sess.reset()
            sleep(0.1)
