...
class Stroke(Callback):
    ...
    def on_epoch_end(self, batch, logs=None):
        if(self.index is None):
            ...
            if(self.verbose):
                out = ("Gave " + self.model.name + " a stroke on " + str(int(self.vratio * 100)) + "% of its layers.")
                print(out)
            ...
            if(self.verbose):
                out = ("Gave " + self.model.get_layer(index=self.index).name + " a stroke on " + str(int(self.vratio * 100)) + "% of its layers.")
                print(out)
