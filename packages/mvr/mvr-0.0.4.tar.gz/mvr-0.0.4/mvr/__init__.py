...
def rank_matrix(A, burn_in=10**6, samples=2000, sample_spacing=10000, repetitions=100):
    ...
    initial_ranking = [0 for _ in range(n)]
    for k_out_rank, index in enumerate(np.flip(np.sum(A, axis=1).argsort())):
        initial_ranking[index] = k_out_rank
...
def get_violation_count(A, ranking):
    _ranking = List()
    for rank in ranking:
        _ranking.append(rank)

    return _get_violation_count(A, _ranking)


@njit
def _get_violation_count(A, ranking):
    count = 0
    n = len(ranking)
    for i in range(n):
        for j in range(n):
