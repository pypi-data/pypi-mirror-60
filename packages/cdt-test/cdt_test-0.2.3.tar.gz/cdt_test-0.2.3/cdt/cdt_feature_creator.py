...
from cdt import hand
from cdt import stroke_classification
from cdt import digit_classification
from cdt import storke_segmentation
from cdt import area_classification

# from cdt_util import *
from cdt.cdt_util import count_center
from cdt.cdt_util import get_strokes_index
from cdt.cdt_util import get_bounding_box
from cdt.cdt_util import merge_multiple_strokes
from cdt.cdt_util import count_stroke_length
from cdt.cdt_util import get_cloest_distance
from cdt.cdt_util import get_polar_coordinates
from cdt.cdt_util import get_real_child
from cdt.cdt_util import INF
from cdt.cdt_util import count_distance
from cdt.cdt_util import dcmp
from cdt.cdt_util import overlap
...
class CdtModel:
    ...
    def make_cdt_feature(self, command_strokes, copy_strokes, hour, minute, buffer):
        ...
        # 数字平均长度
        cdt_feature[2 * cnt + 2] = (cdt_feature[173] * command_digit_num + cdt_feature[cnt + 173] * copy_digit_num) / (
                command_digit_num + copy_digit_num)
        # 数字平均高度
        cdt_feature[2 * cnt + 3] = (cdt_feature[176] * command_digit_num + cdt_feature[cnt + 176] * copy_digit_num) / (
                command_digit_num + copy_digit_num)
        # 数字平均宽度
        cdt_feature[2 * cnt + 4] = (cdt_feature[177] * command_digit_num + cdt_feature[cnt + 177] * copy_digit_num) / (
                command_digit_num + copy_digit_num)
        # 总笔画数
        ...
        # 思考时间与总时间的比值
        cdt_feature[2 * cnt + 7] = math.fabs(cdt_feature[197] - cdt_feature[195]) + math.fabs(
            cdt_feature[cnt + 197] - cdt_feature[cnt + 195]) / cdt_feature[2 * cnt + 6]
        return cdt_feature
    ...
    def make_single_clock_feature(self, strokes, hour, minute, buffer):
        ...
        # 为每一个笔画设置长方体的包围盒，就是记录左上角和右下角坐标及时间点
        ...
        # 将区域分出数字区域和指针区域
        svm_label = self.svm_model.predict_by_data(strokes, center, internal_indexs, bounding_box, area_indexs,
                                                   kmeans_label, edge_strokes)
        ...
        # 合并区域使得补足划分不足的部分
        for i in range(3):
            digit_area_indexs, digit_label = self.repair_under_split(strokes, internal_indexs, digit_area_indexs,
                                                                     digit_label, center)
            # print(digit_area_indexs,digit_label)
            digit_area_indexs, digit_label = self.repair_over_split(strokes, internal_indexs, digit_area_indexs,
                                                                    digit_label, center)
            # print(digit_area_indexs,digit_label)
        ...
        # 处理多余数字
        digit_info = \
            self.choose_clock_digit(digit_info, self.digit_angles, self.digit_size, digit_area_indexs, digit_label)[0]
        ...
        if len(edge_indexs) > 0:
            ...
            clock_feature[cnt] = get_cloest_distance(clean_edge_strokes[:int(clean_edge_strokes.shape[0] / 3), :2],
                                                     clean_edge_strokes[int(clean_edge_strokes.shape[0] * 2 / 3):, :2])
            cnt += 1
            # 钟表外圈起点和终点的角度差
            clock_feature[cnt] = math.fabs(get_polar_coordinates(clean_edge_strokes[0], center)[1] -
                                           get_polar_coordinates(clean_edge_strokes[-1], center)[1])
            cnt += 1
        else:
            cnt += 8
        # 指针特征
        ...
        if len(temp_hand_indexs) > 0:
            ...
            #
            # i = 0
            # for data in hand_strokes:
            #     for j in range(len(data)):
            #         sheet1.write(i, j, data[j])
            #     i = i + 1
            # f.save('/Users/zihao/Desktop/hand.xlsx')
            # print("保存成功")
            # ghgghhg

            # minute_hand, hour_hand, hand_center = self.hand_detector.hand_detect(hand_strokes)
            ...
            if min_hand_indexs > 0:
                # 画时针之前到画第一笔时针的间隔时间
                clock_feature[cnt] = math.fabs(strokes[strokes_indexs[min_hand_indexs, 1] - 1, 2] - strokes[
                    strokes_indexs[min_hand_indexs - 1, 1] - 1, 2])
            cnt += 1
            # 区分时针分针
            if (hour >= 10):
                hour -= 10
            ...
            if len(digit_info[hour]) > 0:
                ...
                for i in range(len(digit_info[hour])):
                    digit_strokes = merge_multiple_strokes(strokes, internal_indexs,
                                                           digit_area_indexs[digit_info[hour][j][0]])
            ...
            if len(digit_info[minute]) > 0:
                ...
                for i in range(len(digit_info[minute])):
                    digit_strokes = merge_multiple_strokes(strokes, internal_indexs,
                                                           digit_area_indexs[digit_info[minute][j][0]])
                    digit_center = count_center(digit_strokes)
            ...
            # 计算各自与正确角度的夹角
            first_hour_bias = min(
                [math.fabs(hour_hand_angle - hour_digit_angle) for hour_digit_angle in hour_digit_angles])
            first_minute_bias = min(
                [math.fabs(minute_hand_angle - minute_digit_angle) for minute_digit_angle in minute_digit_angles])
            second_hour_bias = min(
                [math.fabs(minute_hand_angle - hour_digit_angle) for hour_digit_angle in hour_digit_angles])
            second_minute_bias = min(
                [math.fabs(hour_hand_angle - minute_digit_angle) for minute_digit_angle in minute_digit_angles])
            ...
            # 时针长度与分针长度的比值
            clock_feature[cnt] = count_distance(hour_hand[0], hour_hand[1], hand_center[0],
                                                hand_center[1]) / count_distance(minute_hand[0], minute_hand[1],
    ...
    def draw_result_picture(self, strokes, strokes_indexs, internal_indexs, digit_area_indexs, edge_ellipse, buffer):
        ...
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # 将坐标轴设置为距离边缘无margin
        plt.axis([0, 400, 0, 400])  # 设置窗口大小为 400 * 400
        plt.axis('off')  # 关掉坐标轴
    ...
    def choose_clock_digit(self, digit_info, digit_angles, digit_size, digit_area_indexs, digit_label):
        ...
        for i in range(3, 10):
            ...
            if digit_size_difference[i] < 0:
                ...
                clock_digit_area_info[i] = digit_info[i][min_digit_score_indexs][0], \
                                           digit_info[i][min_digit_score_indexs][3]
                for j in range(len(digit_info[i])):
        ...
        for i in range(len(error_area)):
            for j in range(i + 1, len(error_area)):
                if not error_area[i] or not error_area[j]:
                    continue
                if math.fabs(error_area[i][2] - error_area[j][2]) <= 30 and math.fabs(
                        error_area[i][3] - error_area[j][3]) <= 15:
                    merge_strokes = merge_multiple_strokes(strokes, internal_indexs,
                                                           digit_area_indexs[error_area[i][0]] + digit_area_indexs[
                                                               error_area[j][0]])
                    ...
                    if self.check_merge_status(error_area[i], error_area[j], merge_score, adequate_digit, lack_digit,
                                               0):
                        temp_list = [error_area[i][0], merge_score[0, 1], (error_area[i][2] + error_area[j][2]) / 2,
                                     (error_area[i][3] + error_area[j][3]) / 2]
    ...
    def check_merge_status(self, area_info1, area_info2, merge_score, adequate_digit, lack_digit, times):
        # 特殊情况
        if area_info1[4] == 1 and area_info2[4] == 1 and (
                merge_score[0, 0] == 0 or merge_score[0, 0] == 1 or merge_score[0, 0] == 8):
            return False
        if area_info1[4] == 0 and area_info2[4] == 0 and (merge_score[0, 0] == 0 or merge_score[0, 0] == 1):
            return False
        if ((area_info1[4] == 1 and area_info2[4] == 0) or (area_info1[4] == 0 and area_info2[4] == 1)) and (
                merge_score[0, 0] == 0 or merge_score[0, 0] == 1 or merge_score[0, 1] <= 0.8):
            return False
        # 效果增强
    ...
    def repair_under_split(self, strokes, internal_indexs, digit_area_indexs, digit_label, center):
        ...
        for under_split_area in under_split_areas:
            ...
            if self.check_over_split_status(under_split_area, digit_size_difference, adequate_digit, lack_digit,
                                            child_areas[max_split_score_index], child_result[max_split_score_index],
                                            child_score[max_split_score_index]):
    ...
    # 判断区域是否应该划分
    def check_over_split_status(self, under_split_area, digit_size_difference, adequate_digit, lack_digit, child_area,
                                child_result, child_score):
        # 分出的数字全是缺少的数字
                ...
                if name.endswith(".csv"):
                    ...
                    if name[indexs_ + 1] == '1':
                        ...
                        if new_name in files:
                            command_strokes = np.loadtxt(open(os.path.join(root, name), "rb"), delimiter=",",
                                                         skiprows=0)
                            copy_strokes = np.loadtxt(open(os.path.join(root, new_name), "rb"), delimiter=",",
                                                      skiprows=0)
