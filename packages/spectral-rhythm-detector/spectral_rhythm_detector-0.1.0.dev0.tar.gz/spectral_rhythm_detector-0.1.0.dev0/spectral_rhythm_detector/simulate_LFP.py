...
def simulate_LFP(time, spectral_rhythm_times, spectral_rhythm_amplitude=2.0,
    ...
    spectral_rhythm_signal = np.sin(
        2 * np.pi * time * spectral_rhythm_frequency)
    signal = []

    try:
        iter(spectral_rhythm_times)
    except TypeError:
        spectral_rhythm_times = [spectral_rhythm_times]

    for spectral_rhythm_time in spectral_rhythm_times:
        carrier = norm(loc=spectral_rhythm_time + (spectral_rhythm_width / 2),
                       scale=spectral_rhythm_width / 4).pdf(time)
        carrier /= carrier.max()
        signal.append((spectral_rhythm_amplitude / 2) *
                      (spectral_rhythm_signal * carrier))
