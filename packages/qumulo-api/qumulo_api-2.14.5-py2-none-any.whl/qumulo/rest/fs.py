...
def update_request_with_stream_ref(reqbody, stream_id, stream_name, label=""):
    assert (stream_id is None) or (stream_name is None)
    if stream_id:
        id_label = label + "_" + "stream_id"
        reqbody.update({id_label: stream_id})
    elif stream_name:
        name_label = label + "_" + "stream_name"
        reqbody.update({name_label: stream_name})

@request.request
def copy(conninfo, credentials, source_path=None, source_id=None,
        source_snapshot=None, target_path=None, target_id=None,
        source_stream_name=None, source_stream_id=None, target_stream_name=None,
        target_stream_id=None, update_progress=None):
    uri = build_files_uri(["copy-chunk"])
    method = "POST"
    ...
    target_ref_key = "target_id" if target_id else "target_path"

    body = {
        source_ref_key: source_ref,
        target_ref_key: target_ref
    }
    update_request_with_stream_ref(
        body, source_stream_id, source_stream_name, label="source")
    update_request_with_stream_ref(
        body, target_stream_id, target_stream_name, label="target")

    if source_snapshot:
        body.update({'source_snapshot': source_snapshot})

    do_request = lambda body : request.rest_request(
        conninfo, credentials, method, str(uri), body=body)

    result = do_request(body)

    if result.data and update_progress:
        copy_limit_bytes = int(result.data['target_offset'])
        # Update the progress bar after every request to account for bytes
        # copied by the last request
        update_progress(copy_limit_bytes)

    while result.data:
        result = do_request(result.data)
        if update_progress:
            update_progress(copy_limit_bytes)
...
def create_stream(conninfo, credentials, stream_name, path=None, id_=None):
    ...
    return request.rest_request(
        conninfo, credentials, method, str(uri), body=config)
