...
class CopyCommand(qumulo.lib.opts.Subcommand):
    NAME = "fs_copy"
    DESCRIPTION = "Server-side copy files"
    ...
    def main(conninfo, credentials, args):
        ...
        default_stream_attrs, _etag = fs.get_file_attr(
            conninfo,
            credentials,
            path=args.source,
            snapshot=args.source_snapshot,
            stream_id='0')
        total_bytes = int(default_stream_attrs['size'])

        try:
            dir_path, name = os.path.split(args.target)
            fs.create_file(conninfo, credentials, name, dir_path=dir_path)
        except request.RequestError as e:
            if e.error_class != 'fs_entry_exists_error':
                raise
            if args.overwrite:
                # set target file to source file size to clear existing content
                fs.set_file_attr(
                    conninfo, credentials, path=args.target, size=total_bytes)
            else:
                raise ValueError("Target file {} exists. Use --overwrite "
                    "to overwrite it.".format(args.target))

        # early return if src file is empty
        if total_bytes == 0:
            return
        with tqdm(
                desc='Copying',
            ...
            fs.copy(
                ...
                update_progress=copy_progress.update)
