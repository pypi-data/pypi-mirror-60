...
# TODO sjelin: delete this nonsense. Non-ASCII characters aren't invalid in an
# error message, and this function doesn't even filter out non-ASCII characters
# on instances of `newstr`/`unicode`/python3 `str`.
...
class RequestError(Exception):
    ...
    def __init__(self, status_code, status_message, json_error):
        ...
        # TODO sjelin: whY aRe wE ENcOdiNg tO AsCIi?! fIX THiS NOnSeNSe!
        formatted = str(formatted, 'ascii') or ' ' * 4 + '(no backtrace)'
        message = str('Error {}: {}: {}\nBacktrace:\n{}').format(
...
class APIRequest(object):
    ...
    def __init__(
        ...
        if body_file is not None:
            ...
            self.body_text = body
            self.body_file = StringIO(str(self._body_text()))
