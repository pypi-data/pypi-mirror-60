...
def check_ascii(val, friendly_name):
    ...
    try:
        ...
        # '?'.  We start by calling the __str__ method directly so that we can
        # avoid the newstr wrapper and encode into ascii while replacing any
        # invalid characters with the unicode replacement character.  This is
        # not the format we want so we take a round trip of encoding and
        # decoding throught bytes while using ascii encoding and replacing
        # invalid characters. :/
        # TODO sjelin: or we how about we just leave the original unicode in the
        # exception??? I'm going to delete this nonsense.
        raise ValueError(
            'The string "{}" for {} contains invalid characters'.format(
                str(val.__str__(), 'ascii', 'replace')
