...
Author: Shai Bennathan - shai.bennathan@intel.com
...
def _make_version_file(project):
    return f'src\\{project}\\version.py'
...
class PylintCommand(ProjectCommand):
    ...
    def __pylint_call(self, *targets):
        ...
        for target in targets:
            ...
            cmd = CheckedCall(f'pylint {target}')
...
class PylintProjectTestCommand(PylintCommand):
    ...
    def _collect_target(self):
        targets = []
        for subdir, _, files in walk(path.join('src', self.project)):
            targets.append([path.join(subdir, target)
                            for target in files if target.endswith('.py')])
        return targets
...
class TwineCommand(ProjectCommand):
    ...
    def finalize_options(self):
        """Post-process received options"""
        self.package = f'dist\\{self.project}-{Package(self.project).version}.tar.gz'
    ...
    def _twine(self, command):
        ...
        """
        cmd = CheckedCall(f'twine {command} --config-file {self.config}'
...
class SetVersionCommand(Command):
    """A custom command to set the project's version file"""

    description = 'Set the project\'s version file'
    user_options = [
        ('value=', None, 'Major.Minor version override'),
        ('branch=', None, 'Branch name: \'master\' for non-rc, all others for rc'),
        ('build=', None, 'Build ID'),
        ('owner=', None, 'Copyright holder'),
    ...
    def __init__(self, dist):
        ...
        self.owner = None
        self.project = None
        super(SetVersionCommand, self).__init__(dist)
    ...
    def finalize_options(self):
        """Post-process received options"""
        assert self.value, 'version override not specified'
        assert self.project, 'project name not specified'
        self.version = \
            f'{self.value}' \
            f'{"." if self.branch == "master" else "rc"}' \
            f'{self.build}'

    def run(self):
        """Write provided version to the project's version file"""
        self.announce(
            'Setting version to: ' + self.version,
            level=INFO)
        with open(_make_version_file(self.project), 'w') as version_file:
            version_file.writelines([
                '"""',
                f'{self.project.title()} module version',
                'AUTO-GENERATED BY SETUP.PY',
                f'(C) {self.owner.title()} {date.today().year}',
                '"""',
                '',
...
# TODO: move into the package and use it once published :)
class Package:
    """Python package
    ...
    def __init__(self, project):
        ...
        """
        self._version_file = _make_version_file(project)
        self.project = project
        self.version = self._get_version_from_file()
