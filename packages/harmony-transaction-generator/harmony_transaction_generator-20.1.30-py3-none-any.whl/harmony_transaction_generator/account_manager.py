...
class BatchTransactions:
    ...
    def add(self, from_address, to_address, src_shard, dst_shard, amount,
        ...
        using `nonce` to the buffer of batched transactions. The `passphrase` is used to unlock the keystore file.
        The `error_ok` flag can be set to true to indicate that the added transaction will not block the rest
        of the batch should it fail to send.
        """
    ...
    def send(self, endpoint, wait_for_confirm=None):
        ...
        This will return a list of dictionaries that contain information (and possibly errors) of
        ...
        response = json_load(cli.single_call(command, error_ok=True, timeout=300))  # Timeout in 5-min
