...
class NetworkKey(Key):
    ...
    def identity(self, addr, random=None):
        random = random if random is not None else secrets.token_bytes(8)

        assert len(random) == 8

        hash = aes_ecb(
            self.identity_key,
            bitstring.pack("pad:48, bits:64, uintbe:16", random, addr).bytes,
        )

        return hash[-8:], random
