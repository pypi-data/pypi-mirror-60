...
if(len(sys.argv)<2):
    ...
    sys.exit(2)
try:
    opts,args = getopt.getopt(sys.argv[1:],"hvPVo:fmM:",["pdf"])
...
if(Verbose):
        ...
        print ('Total white-noise at sampling time: %f' % (W/math.sqrt(Sampling)))
        print ('Nyquist frequency [muHz]: %f' % nyq)
        print ('frequency resolution [muHz]: %f' % f[0])

DataSystematic = None
if (Systematics['Enable']):
    DataSystematic = sls.ExtracSystematicDataMagRange(Systematics['Table'],StarVpMag) 

TimeShift = Instrument['TimeShift']
       

nt = time.size
full_ts = np.zeros((nt,NGroup,NCamera,4))
...
for iMC in range(nMC):
    for i in range(NGroup):
        ...
        for j in range(NCamera): 
            ...
            if(DataSystematic is not None):
                ...
                tmp = (sls.SimSystematicError(Sampling,nt,DataSystematic,seed=seeds[i*NCamera+1+j])-1.)
                cumul += tmp 
                tsSC *= (1. + tmp)
            ts *= (1. + cumul)        
            if(Transit['Enable']):
    ...
    single_ts = np.sum(full_ts,axis=(1,2))/float(NGroup*NCamera)
    ...
    if(FullOutput):
        full_ts = full_ts.reshape((nt*NGroup*NCamera,4))  
        np.savetxt(fname,full_ts,fmt='%12.2f %20.15e %1i %1i',header=hd + '\nTime [s], Flux variation [ppm], Group ID, Camera ID')
    elif(MergedOutput):
        merged_ts = np.zeros((nt,NGroup,3))
        for G in range(NGroup):
            merged_ts[:,G,0] = rebin1d(full_ts[:,G,:,0].flatten(),nt)/float(NCamera)
            merged_ts[:,G,1] = rebin1d(full_ts[:,G,:,1].flatten(),nt)/float(NCamera)
            merged_ts[:,G,2] = G+1
        merged_ts = merged_ts.reshape((nt*NGroup,3)) 
        np.savetxt(fname,merged_ts,fmt='%12.2f %20.15e %1i',header=hd + '\nTime [s], Flux variation [ppm], Group ID')
    else:
...
#    plt.plot(f[1:], 0.5*( (mps_SC[1:]  - 2*W**2*1e-6/NCamera) /(NGroup)),'m',lw=2,label='systematics') # all Camera
    ...
    plt.ylabel(r'[ppm$^2$/$\mu$Hz]')   
    if(Pdf):
        fname = OutDir + StarName+ '_fig3.pdf'
    else: 
