...
def ExtracSystematicDataMagRange(filename,Mag):
    '''
    Mag : Vp magnitude (PLATO V reference magnitude)
    '''
    ...
    for d in Data:
        if( (d['Mag']+magcorr-0.25<=Mag) and (Mag<d['Mag']+magcorr+0.25) ):
            ...
            i += 1
    if(i==0):
        raise SLSError('No systematic error parameters for magnitude = %f \
        (PLATO V reference magnitude). Please turn-off systematic error or \
         change the star magnitude' % Mag)
    return DataOut


def SimSystematicError(Sampling,N,Data,seed=None,full=None,time_shift=0.):
    ...
    NData = len(Data)
    Tupd = np.array([])
# struct = { "parameter" : p , "chi2" : chi2val, 'MaskUpdNb': Nupd ,'MaskUpTime': MaskUpTime, 'MaskID': mskID , 'MaskSize': mskSize, 'LCmean' : lcmean}
    ...
    for q in range(Nquarter):
        ...
        MaskUpdNb = dataq['MaskUpdNb']
        MaskUpTime = dataq['MaskUpTime']
        Parameters =  dataq['parameter']
