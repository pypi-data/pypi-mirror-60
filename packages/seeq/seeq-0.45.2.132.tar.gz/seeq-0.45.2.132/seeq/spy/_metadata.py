    ...
    for k, v in d.items():
        if k in attr_map:
            ...
            if not pd.isna(v):
                ...
                if isinstance(p.value, (np.bool_, np.bool)):
                    # Swagger can't deserialize an np.bool_, so we have to make sure to scrub them.
                    # Note that code you would expect to convert actually has no effect, like bool(p.value)
                    # This was the only thing I found that properly converted. (Mark Derbecker 2020-01-30)
                    p.value = True if p.value else False
