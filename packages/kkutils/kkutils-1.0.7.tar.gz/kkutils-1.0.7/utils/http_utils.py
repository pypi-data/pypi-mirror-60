...
class BaseRequest(metaclass=RequestMeta):
    ...
    def _retry(self, url, kwargs, retry, reason=None):
        ...
        try:
            ...
            self.logger.error(f'retry: {retry}, url: {url}, {e}')
            time.sleep(self.sleep)
            return self._retry(url, _kwargs, retry - 1, str(e))
...
class TornadoClient(Pycurl):
    ...
    async def _retry(self, url, kwargs, retry):
        ...
        if resp.code >= 500 and retry >= 0:
            self.logger.error(f'retry: {retry}, url: {url}, {resp.reason}')
