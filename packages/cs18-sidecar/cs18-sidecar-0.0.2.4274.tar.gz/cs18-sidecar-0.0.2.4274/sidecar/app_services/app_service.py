...
from sidecar.services.what_is_my_ip import WhatIsMyIp
...
class AWSAppService(AppService):
    ...
    def _get_ip(self):
        if not self.my_ip:
            with self._lock:
                if not self.my_ip:
                    self.my_ip = self.ip_service.get_ip()
        return self.my_ip
    ...
    def _add_permissions_for_sidecar(self, sg_id: str, permissions: List[Dict]):
        ec2_client = self._session.get_ec2_client()
        sidecar_public_cidr = "{}/32".format(self._get_ip().rstrip('\n'))
