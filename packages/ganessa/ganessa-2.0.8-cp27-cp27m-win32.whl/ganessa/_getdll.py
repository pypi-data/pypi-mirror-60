...
USE_ENV = version_info.major < 3 or version_info.minor < 8
class _Dummy(object):
    '''Dummy class with a close() method'''
    def __init__(self, arg=None):
        pass
    def close(self):
        pass
if USE_ENV:
    add_dll_directory = _Dummy
else:
...
def _getdllinfo(bth):
    ...
    for ganessa_dir, f in zip(('GANESSA_DIR', 'PICCOLO_DIR'), dlls):
        if ganessa_dir in environ:
            ...
            exists, change = _add_dir_to_path(gandir, f)
            if exists:
                print(f, 'found in environment variable', ganessa_dir)
                import_ok = _import_ganessa(f, bth, dlls)
                change.close()
                if import_ok:
                    break
            else:
        ...
        for gandir, f in product(unistr(environ['path']).split(';'), dlls):
            if OP.exists(OP.join(gandir, f)):
                ...
                change = add_dll_directory(gandir)
                import_ok = _import_ganessa(f, bth, dlls)
                change.close()
                if import_ok:
                    break
...
def _add_dir_to_path(ndir, dll):
    ...
    if status:
        if USE_ENV:
            ...
            changed = add_dll_directory(ndir)
