...
################################################################################
    ...
    def GetIDBySortableName(self, sorted_username):
        ...
        if not hasattr(self, 'sortableNamesWithID'):
            ...
            for student in self.GetCourseUsers():
                ...
                self.sortableNamesWithID[self.GetUserProfile(str(student["id"]))["sortable_name"].lower().replace(',', '').replace(' ','').replace('-', '')] = str(student["id"])
        try: 
            return self.sortableNamesWithID[sorted_username]
        except:
            return None

    def GetAllStudentUserNames(self):
        ...
        all_students = self.GetCourseUsers()
        print (all_students)
        ...
        for i in all_students:
            if "email" in i:
                ...
                self.canvasAllUserNames[id].append(i["email"].strip("@gatech.edu"))
    ...
    def ImportRubric(self, filename):
        ...
        with open(filename, "r")  as csvfile:
            ...
            for row in reader:
                ...
                self.SubmitGrade(course_id, assignment_id, user_id, params)
                print("Grade posted")
                rowCount += 1
################################################################################
