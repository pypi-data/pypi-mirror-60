    ...
    def find_runners(self, edited_filename=None, tags=[]):
        ...
        for RunnerType in self._runner_types:
            if not RunnerType.has_tags(tags):
                continue
            if (
                edited_filename is None 
                or 
                RunnerType.can_edit(edited_filename)
            ):
                names_and_tags.append((
                    RunnerType.runner_name(),
                    RunnerType.runner_tags(),
                ))
...
class Factories(object):
    ...
    def find_runners(self, edited_filename, tags=[]):
        ...
        for factory in self._factories:
            for nat in factory.find_runners(
                edited_filename, tags
            ):
    ...
    def get_runner_versions(self, runner_name, tags=[]):
        ...
        '''
        for factory in self._factories:
            versions = factory.get_runner_versions(
                runner_name, tags, 
            )
...
def test():
    ...
    assert(runners == [('MyRunner1', ['A', 'B', 'C', 'D']), ('MyRunner2', ['C', 'D', 'E', 'F'])])
