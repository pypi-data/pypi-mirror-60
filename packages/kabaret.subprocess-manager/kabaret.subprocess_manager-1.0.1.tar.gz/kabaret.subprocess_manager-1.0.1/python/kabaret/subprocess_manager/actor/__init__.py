    ...
    def _execute(self):
        ...
        for factory, RunnerType in factories.list_runner_types():
            ...
            for version in supported_versions:
                sortable = [
                    factory_name, group_name, group_icon,
                    runner_name, runner_icon,
                ]
...
class Get_Runner_Versions(Cmd):
    ...
    def _execute(self):
        return self.actor().get_factories().get_runner_versions(
            self.runner_name, self.tags
        )
...
class Run(Cmd):
    ...
    def _decode(
        self, 
        runner_name, tags=[], version=None,
        label=None, extra_argv=[], extra_env={},
    ):
        self.runner_name = runner_name
        self.tags = tags
...
class SubprocessManager(Actor):
    ...
    def __init__(self, session):
        ...
        self._factories.ensure_factory(
            get_system_factory()
        )
    ...
    def ensure_factory(self, runner_factory):
        ...
        self.session().dispatch_event(
            'subprocess_manager.factory_added', 
            factory_name=runner_factory.name
        )
