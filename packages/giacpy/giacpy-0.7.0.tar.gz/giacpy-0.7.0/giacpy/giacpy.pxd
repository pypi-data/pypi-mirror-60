...
cdef extern from "giac/giac.h" namespace "giac":
    ...
    #int GIAC_make_thread "make_thread"( gen & ,int ,giac_callback & ,void * ,const context * ) except +
#    gen GIAC_eval "thread_eval" (gen , int, context *)  except +
    int CASmonitor "check_thread"(context * ) except +
    gen GIAC_symbof "symb_of"(gen &,gen &) except +
...
cdef extern from "giacpymisc.h":
     void unsetsecure_run() except +
     void ressetctrl_c() except +
     void setctrl_c() except +
     int testctrl_c() except +
     int CASmakethread "makethread"(gen &,int ,gen * ,context * ) except +
     void CASkillthread "killthread"(context *) except +
     ...
     #
#     int htmlbrowserhelp(char * ) except +
...
# We put some C++ declaration of here to let them be available from other cython modules.
cdef class Pygen:
    cdef gen * gptr # a pointer toward the corresponding c++ element of type giac::gen

########################################################
# A global context pointer. One by giac session.
########################################################
cdef context * context_ptr
