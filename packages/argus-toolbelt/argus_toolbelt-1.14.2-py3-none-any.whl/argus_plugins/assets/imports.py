...
def hosts(
    ...
    for host in assets:
        if "name" not in host or "ipAddresses" not in host:
            raise ValueError(
                ...
                "and/or 'ipAddresses' not in %s"
                "This might also be caused if you did not use the same "
                "delimiter as specified or default (,) in the csv." % str(host)
            )
    messages = []
    ...
    for service_name, host_list in services_to_attach_hosts.items():
        try:
            for service in search_service_assets(name=[service_name], customerID=[int(customer['data']['id'])])["data"]:
