...
class Fist:
    ...
    def _reg_req(self, other_fist_name, addr, set_routing_id=False):
    ...
    def reg_req_master(self):
        ...
        return self._reg_req(MASTER_FIST_NAME, self.master_addr, set_routing_id=True)
    ...
    def _req(self, fist_name, frame):
        ...
        try:
            ...
            if e.errno == 156384763:
                self.logger.warning('[req] invalid sock state error, to recreate sock and resend frame')
                sock = self._context.socket(zmq.REQ)
                sock.connect(self.fist_req_addrs[fist_name])
                sock.send(frame.buf)
                self.req_socks[fist_name] = sock
    ...
    def req(self, fist_name, obj, msg_type, req_id, err_id=0):
        ...
        f.set_source(self.get_source_id())
        f.set_data(obj)
        self._req(fist_name, f)
        ret = self._recv_req(self.req_socks[fist_name], Fist.REQ_WAIT_TIME)
        if ret is None:
            raise Exception("Timeout in req {}!".format(fist_name))
        return Frame(ret)

    def req_master(self, frame):
        self._req(MASTER_FIST_NAME, frame)
        ret = self._recv_req(self.req_socks[MASTER_FIST_NAME], Fist.REQ_WAIT_TIME)
        if ret is None:
