...
from pyus.core.cython.imaging.functors.windows import _Hamming


class Hamming(StandardWindow):

    def __init__(self, f_number: Real, dtype=None):

        # Call super constructor
        super(Hamming, self).__init__(f_number=f_number, dtype=dtype)

        # Attach backend functor
        self._functor = _Hamming(f_number)
    ...
    def _call(self, im_pos: np.ndarray, el_pos: np.ndarray) -> np.ndarray:
        ...
        factor = np.where(
            distance < aperture / 2,
            alpha + beta * np.cos(2 * np.pi * distance / aperture),
            factor
        )
