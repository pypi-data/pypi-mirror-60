...
def do_it(*, owner, reponame, action, args=None):  # Shia LeBeouf!
    auth = load_auth()
    if action == "cache" or (action == "report" and args.refresh):
        issues = get_em_all(owner=owner, reponame=reponame, auth=auth)
        cache_issues(owner=owner, reponame=reponame, issues=issues)
    else:
        try:
            issues = load_issues(owner=owner, reponame=reponame)
        except FileNotFoundError:
...
if __name__ == "__main__":
    ...
    do_it(owner=args.owner, reponame=args.repo, action=args.action, args=args)
