...
from spell.cli.commands.cluster_aws import create_aws, eks_init, add_s3_bucket, update_aws_cluster, delete_aws_cluster
from spell.cli.commands.cluster_gcp import create_gcp, gke_init, add_gs_bucket, update_gcp_cluster, delete_gcp_cluster
...
@click.command(name="add-docker-registry",
               short_help="Configures your cluster to enable runs with docker images in the private registry"
                          " hosted by your cloud provider. Currently only supports Google Container Registry.")
@click.pass_context
@click.option("--cluster-name", default=None, help="Name of cluster to add registry permissions to")
def add_docker_registry(ctx, cluster_name):
    """
    ...
    if cluster_type == 'AWS':
        # TODO(xiaomin)
        # ctx.invoke(add_ec_registry, registry_name=registry_name, cluster=cluster, profile=profile)
        raise ExitException("Integration to Amazon Container Registry is currently under development")
    elif cluster_type == 'GCP':
...
@click.command(name="delete-docker-registry",
               short_help="Removes your cluster's access to docker images in the private registry"
                          " hosted by your cloud provider. Currently only supports Google Container Registry.")
@click.pass_context
@click.option("--cluster-name", default=None, help="Name of cluster to remove registry permissions from")
def delete_docker_registry(ctx, cluster_name):
    """
    This command removes your cluster's access to docker iamges in the private registry hosted by your cloud provider.
    """
    cluster = deduce_cluster(ctx, cluster_name)
    ...
    if cluster_type == 'AWS':
        # TODO(xiaomin)
        # ctx.invoke(delete_ec_registry, registry_name=registry_name, cluster=cluster, profile=profile)
        raise ExitException("Integration to Amazon Container Registry is currently under development")
    elif cluster_type == 'GCP':
...
@click.option("-c", "--cluster", "cluster_name", type=str,
              help="The name of the Spell cluster that you would like to delete. "
                   "If it's not specified, it will default to "
                   "the ONE cluster the current owner has, or fail if the current owner has more than one cluster.")
