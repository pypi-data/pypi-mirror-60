...
class RunsService(object):
    ...
    def new(self, **kwargs):
        ...
        # set workflow id
        if "workflow_id" not in kwargs and self.client.active_workflow:
            kwargs["workflow_id"] = self.client.active_workflow.id
        # create request
        run_req = RunRequest(run_type="user", conda_file=conda_env_contents, **kwargs)
