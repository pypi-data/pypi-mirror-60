    ...
    if path.isfile(file_or_dir) and \
       (file_or_dir.endswith('yml') or file_or_dir.endswith('yaml')):
        ...
        if not 'DATABASE_URL' in environ:
            raise click.BadParameter('SQL directory as argument but DATABASE_URL not set',
                                     param_hint='DATABASE_URL')
        if not 'SOURCE' in environ:
            raise click.BadParameter('SQL directory as argument but SOURCE not set',
                                     param_hint='SOURCE')
        ...
        if 'source' in step:
            ...
            if 'sql' in step:
                ...
                if 'directory' in step['sql']:
                    transform = SQLTransform(mode, src, dst, directory=step['sql']['directory'])
                elif 'file' in step['sql']:
                    transform = SQLTransform(mode, src, dst, filename=step['sql']['file'])
                else:
