...
from pyvss.const import (
    API_ENDPOINT_BASE, DATETIME_FMT, DEFAULT_DEBUG, DEFAULT_TIMEOUT,
    PACKAGE_NAME, VALID_VM_BUILD_PROCESS, VALID_VM_USAGE, VSKEY_STOR_ENDPOINT,
...
class VssManager(object):
    ...
    def __repr__(self) -> str:
        ...
        return "<{}({})".format(self.__class__.__name__, view)
    ...
    def get_supported_vmx_types(self, only_type=True):
        """
        Gets Virtual Machine virtual hardware versions supported
        by the API instance.
    ...
    def validate_vm_vmx_version(self, n_type):
        """
        Validates supported vmx version
    ...
    def create_vm_snapshot(
        self, uuid, desc, date_time, valid=24, consolidate=False
    ):
        """
    ...
    def needs_consolidation(self, uuid):
        """
        Checks if Vm requires disk consolidation
        :param uuid: Virtual Machine Uuid
        :type uuid: str
        :return: boolean
        """
        consolidation = self.get_vm_consolidation(uuid)
    ...
    def create_vm(
        ...
        **kwargs
        ...
        """
        Creates single Virtual Machine.
    ...
    def create_vm_from_clone(
        ...
        **kwargs
        ...
        :param source: Source virtual machine uuid
        :type source: str
        :param description: Brief description of what the virtual
          machine will host
        :type description: str
        ...
        new_vm_spec = dict(
            description=description, built_from='clone', source_vm=source
        )
        new_vm_spec['name'] = (
    ...
    def create_vms_from_clone(
        ...
        **kwargs
        ...
        """
        Deploy multiple or a single virtual machine from a source
                        virtual machine.
        ...
        :param source: Source virtual machine (powered off)
        :param description: Brief description of what the virtual
          machine will host
        :type description: str
        ...
        if name:
            ...
            names = [
                '%s_%s' % (source_spec['name'], i) for i in range(1, count + 1)
            ]
        new_vms_spec = dict(
            description=description,
            built_from='clone',
    ...
    def get_supported_scsi_controllers(self, only_type=True):
        """
        Gets supported Virtual Machine scsi controller types
        supported by the API instance.
    ...
    def get_vm_disk_capacity(self, uuid, unit):
        """
        ...
        :return: int
        """
        disk = self.get_vm_disk(uuid, unit)
        if not disk:
    ...
    def get_supported_disk_backing_modes(self, only_type=True):
        """
        Gets supported Virtual Machine Disk Backing modes
        supported by the API instance.
    ...
    def get_vm_disk_backing(self, uuid, disk):
        """
        Gets Virtual Machine disk backing data mode.
    ...
    def update_vm_disk_backing_mode(self, uuid, disk, mode, **kwargs):
        """
        Updates given Virtual Machine Disk backing mode.
    ...
    def is_powered_on_vm(self, uuid):
        """
        Checks if given Virtual Machine is On.

        :param uuid: Virtual Machine Uuid
        :type uuid: str
        :return: bool
        """
        data = self.get_vms(filter='uuid,eq,%s' % uuid)
        if not data:
            return None
        else:
    ...
    def is_powered_off_vm(self, uuid):
        """
        Checks if given Virtual Machine is Off.
    ...
    def suspend_vm(self, uuid, **kwargs):
        """
         This method suspends a given Virtual Machine.
         ...
         """
        json = self.update_vm_state(uuid=uuid, state='suspend', **kwargs)
        return json
    ...
    def rename_vm(self, uuid, name, **kwargs):
        """
        Update Virtual Machine name. This does not change
    ...
    def is_enabled_vm_vmrc_copy_paste(self, uuid):
        """
        Check if Virtual Machine Remote Console Copy Paste
        Settings are enabled.
        :param uuid: Virtual Machine Uuid
        :type uuid: str
        :return: bool
        """
        json = self.get_vm_vmrc_copy_paste(uuid)
        return json.get('enabled', False)

    def get_vm_vmrc_copy_paste(self, uuid, options=False, **kwargs):
        """
        Get Virtual Machine Remote Console Copy Paste
        Settings.
        ...
        json = self.request(
            '/vm/{uuid}/console/copy-paste'.format(uuid=uuid),
            params=params,
            **kwargs
        )
        return json.get('data')

    def enable_vm_vmrc_copy_paste(self, uuid, **kwargs):
        """
        Enable the Copy/Paste between the VMRC client
        and Windows/Linux Virtual Machine.

        :param uuid: Virtual Machine Uuid
        :type uuid: str
        :return: obj
        """
        if self.is_enabled_vm_vmrc_copy_paste(uuid):
    ...
    def disable_vm_vmrc_copy_paste(self, uuid, **kwargs):
        """
        Disable the Copy/Paste between the VMRC client
        and Windows/Linux Virtual Machine.
        ...
        :return: obj
        """
        if not self.is_enabled_vm_vmrc_copy_paste(uuid):
    ...
    def get_supported_vss_options(self, only_option=True):
        """
        Gets Virtual Machine VSS Options supported
        by the API instance.
    ...
    def enable_vm_vss_option(self, uuid, option_name, **kwargs):
        """
    ...
    def disable_vm_vss_option(self, uuid, option_name, **kwargs):
        """
    ...
    def delete_vm_vss_ha_group(self, uuid):
        """
        Removes VM from availability group
        ...
        """
        json = self.request(
            '/vm/{uuid}/vss/ha_group'.format(uuid=uuid), method=self.DELETE
        )
        return json.get('data')
    ...
    def validate_vm_scsi_controller_type(self, n_type):
        """
        Validates supported disk scsi controller type
    ...
    def validate_vm_disk_backing_mode(self, n_type):
        """
        Validates supported disk backing mode
        :param n_type: Backing mode
        :param n_type: str
        :return: str
        """
