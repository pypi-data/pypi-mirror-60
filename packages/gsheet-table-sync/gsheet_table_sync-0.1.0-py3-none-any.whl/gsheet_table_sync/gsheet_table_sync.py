...
class GsheetTableSync:
    ...
    def set_worksheet(self, worksheet_id):
        ...
        if worksheet_id in self.worksheet_lookup:
            ...
            raise Exception(f"Unable to find worksheet {self.worksheet_id}")
    ...
    def update_row(self, row_number, row_data):
        ...
        range_name = "{}!A{}".format(self.worksheet_title, row_number)

        try:
            resp = self.sheet.values().update(spreadsheetId=self.spreadsheet_id, body=body, range=range_name, valueInputOption='USER_ENTERED').execute()
    ...
    def _pre_diff_sheet_data(self, sheet_range, new_data, key):
        ...
        for key in new_data[0].keys():
            tmp.append(key)
        if sorted(tmp) != sorted(header):
            raise Exception(f"Header mismatch between current sheet data vs new data")
