...
class DMETProblemDecompositionMicrosoftQSharpTest(unittest.TestCase):
    ...
    def test_h10ring_vqe_uccsd_microsoft_qsharp_size1(self):
        """
        ...
        self.assertAlmostEqual(energy_vqe, -5.367532792556453, delta=1e-3)

        
    def test_h10ring_vqe_uccsd_microsoft_qsharp_size2(self):
        """
           DMET on H10 ring with fragment size two, using VQE-UCCSD backend
           from Microsoft
        """
        mol = gto.Mole()
        mol.atom = H10_RING
        ...
        dmet.electron_localization_method = meta_lowdin_localization
        dmet.electronic_structure_solver = vqe
        energy_vqe = dmet.simulate(mol, [2,2,2,2,2])

        print(energy_vqe, -5.365734391857011)
        
        # DMET + FCI
        self.assertAlmostEqual(energy_vqe, -5.365734391857011, delta=1e-3)
