...
import ccalafiore.stats.basics as cc_stats_basics
from scipy.stats import t
# from scipy.stats.stats import _ttest_finish
from ccalafiore.combinations import n_conditions_to_combinations


def scores_raw_to_scores_formatted(scores_raw, axis_independent_variable=0, dtype=None):
    ...
    for i_condition in range(n_conditions_iv):
        ...
        index_0_raw[axis_independent_variable_raw] = i_condition
...
def scores_raw_to_paired_t_test(
        scores_raw, axis_samples=-1, axis_independent_variable=0, alpha=0.05, tails='2', keepdims=False):

    np.seterr(divide='ignore', invalid='ignore')

    shape_scores_raw = scores_raw.shape
    n_axes_scores_raw = len(shape_scores_raw)
    axis_samples_raw = axis_samples % n_axes_scores_raw
    axis_independent_variable_raw = axis_independent_variable % n_axes_scores_raw

    scores_formatted = scores_raw_to_scores_formatted(
        scores_raw, axis_independent_variable=axis_independent_variable_raw)

    if axis_samples_raw > axis_independent_variable_raw:
        axis_samples_formatted = axis_samples_raw + 2
    else:
        axis_samples_formatted = axis_samples_raw

    axis_comparisons_formatted = axis_independent_variable

    diff = cc_stats_basics.scores_to_diff_of_scores(
    ...
    means_of_diff = cc_stats_basics.scores_to_means(
        diff, axes=axis_samples_diff, keepdims=keepdims)
    std_err_of_diff = cc_stats_basics.scores_to_standard_error(
...
def scores_formatted_to_paired_t_test(
    ...
    diff = cc_stats_basics.scores_to_diff_of_scores(
        scores_formatted, axis_comparisons=axis_comparisons_formatted, keepdims=keepdims)
