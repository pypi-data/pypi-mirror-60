...
class DummyMotorChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_POSITION, default_value=0, default_type=np.int64, set_function=self._move_absolute,
                              unit="nm", set_value_when_set_function=False, min_value=-1e9, max_value=1e9)
        self.create_attribute(ATTR_POSITION_LOWER_LIMIT, default_value=-1e9, default_type=np.int64, min_value=-1e9,
                              max_value=1e9, unit="nm", set_function=self._set_lower_limit)
        self.create_attribute(ATTR_POSITION_UPPER_LIMIT, default_value=1e9, default_type=np.int64, min_value=-1e9,
        ...
        self.create_attribute(ATTR_VELOCITY, default_value=0, default_type=np.int64, min_value=-1e4, max_value=1e4,
                              unit="nm/sec")
    ...
    def _move_absolute(self, value, callback=None):
        ...
        def move_thread(setpoint):
            ...
            counter = 0
            while self[ATTR_VELOCITY][VALUE] != 0 and self.is_status(STATUS_BUSY):
                # Velocity in nm / 100ms
                counter += 1
                velocity = int(self.get_raw_value(ATTR_VELOCITY) / 10.)
                new_position = self.get_raw_value(ATTR_POSITION) + velocity
                if direction == 1:
