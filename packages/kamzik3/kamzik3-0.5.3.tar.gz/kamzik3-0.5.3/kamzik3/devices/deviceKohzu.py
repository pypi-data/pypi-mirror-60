...
class DeviceKohzuChannel(DeviceChannel):
    ...
    def _init_attributes(self):
        ...
        self.create_attribute(ATTR_VELOCITY, default_value=0, default_type=np.int64, min_value=0, max_value=3e6,
                              unit="nm/s", set_function=self.set_velocity)
    ...
    def set_velocity(self, value):
        value = int(value / self[ATTR_STEP_RESOLUTION][VALUE])
        self.set_value(ATTR_START_SPEED, 2)
        self.set_value(ATTR_TOP_SPEED, 10)
        self.set_value(ATTR_TOP_SPEED, value)
        self.set_value(ATTR_START_SPEED, int(value / 2.2))
    ...
    def subject_update(self, key, value, subject):
        ...
        if self.connected and isinstance(key, tuple) and key[0] == self.channel:
            ...
            if attribute == ATTR_FEEDBACK_CONTROL_TYPE:
                ...
                value = self[ATTR_POSITION].remove_offset_factor(value)
