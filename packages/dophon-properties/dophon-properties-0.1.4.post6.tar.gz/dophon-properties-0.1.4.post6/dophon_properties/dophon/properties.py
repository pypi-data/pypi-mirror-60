...
from . import default_properties as def_prop
...
def read_self_prop():
    ...
    try:
        ...
        # 遍历子配置
        if 'sub' in user_properties and isinstance(user_properties['sub'], list):
            # 加载子配置
            for sub_prop in user_properties['sub']:
                __cache_sub_prop = __import__(sub_prop, fromlist=True)
                # 整合配置
                for __cache_sub_prop_arg in dir(__cache_sub_prop):
                    """
                    默认子配置覆盖主配置
                    """
                    if not re.match('^__.*__$', __cache_sub_prop_arg):
                        user_properties[__cache_sub_prop_arg] = getattr(__cache_sub_prop, __cache_sub_prop_arg)
...
def prop_init():
    try:
        ...
        with open(os.getcwd() + os.sep + 'application.py', 'wb') as app_prop:
            ...
            #                                                                               str) else value)) + '\n',
            #                   encoding='utf-8'))
            app_prop.write(bytes(f"""# properties detail please read \\
# default_properties.py in dophon_properties module \\
# inside dophon module package you used.
# author: CallMeE
""", encoding='utf-8'))
        return read_self_prop()
