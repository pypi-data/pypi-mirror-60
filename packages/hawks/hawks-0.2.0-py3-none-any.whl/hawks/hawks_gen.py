...
class BaseGenerator:
    ...
    def process_config(params):
        ...
        full_config = _merge_default_config(params, base_config)
        ...
        #                     raise Exception(
        #                         f"'{second_key}' is not a valid key for the config (in '{first_key}' settings)"
        #                     ) from e
        ...
        if full_config["hawks"]["folder_name"] is None:
            ...
            if any_saving:
                full_config["hawks"]["folder_name"] = datetime.today().strftime("%Y_%m_%d-%H%M%S")
        return full_config, any_saving

    @staticmethod
    def _merge_default_config(config, defaults, path=None):
        if path is None:
            path = []
        ...
        for key in defaults:
            if key in config:
                if isinstance(config[key], dict) and isinstance(defaults[key], dict):
                    _merge_default_config(config[key], defaults[key], path + [str(key)])
    ...
    def plot_indivs(self, indivs, nrows=None, ncols=None, folder=None, filename=None, cmap="rainbow", fig_format="png", save=None, remove_axis=False, fig_title=None):
