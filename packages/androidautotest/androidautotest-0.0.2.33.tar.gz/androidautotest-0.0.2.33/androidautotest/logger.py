...
class Logger:
    ...
    def __init__(self):
        ...
        self.__log_file_path = r'%s%slog_%s_%s_%s.txt' % (log_dir, PATHSEP, case_name, time_str, random_str)
        self.__html_file_path = r'%s%sreport_%s_%s_%s.html' % (log_dir, PATHSEP, case_name, time_str, random_str)
        self.__serial_file_path = r'%s%sserial_log_%s_%s_%s.txt' % (log_dir, PATHSEP, case_name, time_str, random_str)

    def __start(self):
        print(r'------------------------------------------------------------')
        print(r'case %s start...' % self.__case_name)
        
        self.__start_time = Timer.time()
    ...
    def info(self, msg):
        ...
        if not self.__is_start:
            self.__start()
        log_file = open(self.__log_file_path, mode='a+', encoding='utf-8')
        print(msg)
        time_log, time_str, random_str = random_id()
        ...
        if msg.startswith('[androidtest]') or msg.startswith('[matching]') or  msg.startswith('[tesseract]'):
            self.__log_info_list.append(line_join(time_log, msg))
        elif msg.startswith('[adb]'):
            serial_file = open(self.__serial_file_path, mode='a+')
    ...
    def end(self, status='pass'):
        ...
        if not self.__is_start:
            self.__start()
        self.__write_html_file(status)
        Command.write('')
        if status == 'pass':
            print('\033[37;42mcase %s end...\033[0m' % self.__case_name)
        else:
            print('\033[37;41mcase %s end...\033[0m' % self.__case_name)
        # print(r'case %s end...' % self.__case_name)
        print(r'------------------------------------------------------------')
        case_dir = Path.parent(CASE_PATH)
        SummaryLogger.write({"time":Timer.strftime("%Y%m%d_%H%M%S", self.__start_time),"name":self.__case_name,"dir":case_dir, "path":CASE_PATH, "url":self.__html_file_path,"status":status})
        #open browser
        #webbrowser.open_new_tab(self.__html_file_path)
        Command.exit(0)
        
    def __write_html_file(self, status):
        log_info_list = self.__log_info_list
        # time consuming
        consuming_time = Timer.time() - self.__start_time
        with open(self.__html_file_path, mode='a+', encoding='utf-8') as html_file:
            html_file.write('<meta http-equiv="Content-Type" content="text/html;charset=utf-8">')
            html_file.write('<body style="width:80%;margin-left:10%;">')
            if status == 'pass':
                ...
                html_file.write(r'<p>Test Case %s: <span style="color:green;font-weight:blod;">Pass</span></p>' % self.__case_name)
                html_file.write(r'<p>Time consuming: <span style="color:red;">%ds</span><p></div>' % consuming_time)
                html_file.write(r'</div>')
            else:
                html_file.write(r'<div id="top" style="background: #ffffff;padding-left:20px;box-shadow: 0px 0px 10px #888888;border-radius:5px;height:80px;font-size:20px;">')
                html_file.write(r'<p>Test Case %s: <span style="color:red;font-weight:blod;">Fail</span></p>' % self.__case_name)
                html_file.write(r'<p>Time consuming: <span style="color:red;">%ds</span><p></div>' % consuming_time)
                html_file.write(r'</div>')
            steps = []
            count = 0
            ...
            html_file.write('<a href="%s" style="background: green;color:#ffffff;margin-right:10px;border-radius:10px;height:40px;width:60px;line-height:40px;text-align:center;text-decoration:none;color:#ffffff;">Home</a>' % (Path.path_join(Path.parent(CASE_PATH),'summary_report.html')))
            html_file.write('</div>')
