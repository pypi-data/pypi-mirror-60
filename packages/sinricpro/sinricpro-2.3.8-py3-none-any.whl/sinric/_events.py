...
class Events(Signature):
    ...
    def raiseEvent(self, deviceId, event_name, data=None):
        ...
        try:
            ...
            elif event_name == 'devicePowerUsage':
                queue.put([
                    jsnHandle('devicePowerUsage', deviceId, {
                        "startTime": int(time()),
                        "voltage": data.get('voltage', 0),
                        "current": data.get('current', 0),
                        "power": data.get('power', 0),
                        "apparentPower": data.get('apparentPower', 0),
                        "reactivePower": data.get('reactivePower', 0),
                        "factor": data.get('factor', 0)
                    })
                    , 'device_power_event_response'])
