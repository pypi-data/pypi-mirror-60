...
from brainiac_client.constants import PICKLE_PROTOCOL
...
class BrainiacClient:
    def __init__(self, url: Optional[str] = None, stub: EngineStub = None, http=True):
        if url is None:
            url = "http://engine.libnosis.com/" if http else 'localhost:{}'.format(DEFAULT_PORT)
        self.url = url
    ...
    def runThirdPartyFunction(self,
        ...
        thpr = ThirdPartyRequest(client=self.client,
                                 ...
                                 function=cloudpickle.dumps(function, protocol=PICKLE_PROTOCOL),
                                 body=body,
