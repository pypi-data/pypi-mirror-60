...
from ..config import bundle_ext,basic_unit,basic_freq,bundle_key
import pandas as pd
import numpy as np
import array
...
def get_freq_filename(bundle, instrument, frequency):
    return os.path.join(bundle, instrument, frequency + bundle_ext)
...
class LocalQuotation:
    ...
    def load(self, instrument, frequency, start, end, bundle, period_only=False):
        ...
        if not period_only:
            ...
            self.df = read_data(bundle, instrument, basic_freq)
            ...
            start_time = np.asarray(self.df.index, dtype=f'datetime64[{basic_unit}]')
            end_time = start_time + np.timedelta64(1, f'{basic_unit}')
