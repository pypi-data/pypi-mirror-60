...
class AppConfigUtil(object):
    ...
    def check(self, ak: str, sk: str or None, app_id: int, auth_code: str = None, dict_obj: dict = None) -> bool:
        """
        校验ak权限
        :param ak:
        ...
        if auth_code is None:
            # 如果为None，则判断是否存在对应应用
            return app_id in permissions['app_ids']
        else:
            # 如果不为None，则判断应用及权限
            if app_id in permissions['app_ids'] and auth_code in permissions['auth_codes']:
                return True
            else:
                return False
