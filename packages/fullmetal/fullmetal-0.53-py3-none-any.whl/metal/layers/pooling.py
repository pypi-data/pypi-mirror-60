...
from metal.utils.functional import pool2D, pool2D_backward
...
class Pool2D(LayerBase):
    ...
    def forward(self, X, retain_derived=True):
        ...
        Y_ = np.zeros((n_ex, out_rows, out_cols, self.out_ch)).astype('float32')
        Y = pool2D(n_ex,out_rows,out_cols, self.out_ch,X_pad,s,fr, fc,Y_,self.mode)
    ...
    def backward(self, dLdY, retain_grads=True):
        ...
        for X, dy, out_row, out_col in zip(Xs, dLdY, out_rows, out_cols):
            ...
            dX_ = np.zeros_like(X_pad, dtype=np.float32)
            dX = pool2D_backward(n_ex,out_row,out_col,self.out_ch,fr,fc,s,self.mode,dX_,X,dy)
