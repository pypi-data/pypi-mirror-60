...
from ragex.community.constants import RAGE_PRODUCTION_ENVIRONMENT
...
class SettingsService(DbService):
    ...
    def inspect_environments_config(env_config: Dict[Text, Any]) -> None:
        ...
        if (
            RAGE_PRODUCTION_ENVIRONMENT not in rasa_config
        ...
        for k, v in rasa_config.items():
            if k in (RAGE_PRODUCTION_ENVIRONMENT, "worker"):
                continue
