...
def build_breaks (data,type_bin,num_bin=10,max_v=1,min_v=0):
    ...
    ARGS
        data (array-nD)
        type_bin (string): support 'eqf', 'eqw', 'sbp', 'pr'
    ...
    if type_bin == 'eqf':
        ...
        # step 4: identify breaks based on #bin            
...
def convert_bin (value, num_bin=10,  min_v= 0,  color_img=False, bins_break = [] , debug=0):
    ...
    **notes: ONLY apply to data range 0-1
    ...
    if value > min_v:
        bin_index = [i+1 for i in range(0,num_bin-1) if bins_break[i] <= value]
        if debug==1:
            print (bin_index)
            print (num_bin)
        if bin_index == []:
...
def embed_image (X_embedded, X, color_arr,colormap, file_name_emb, 
        ...
        dpi_v = 75,margin = 0,off_axis=1,show=0):
...
def fillup_image (cor_x, cor_y, X, colors,colormap,fig_size,file_name, 
        ...
        dpi_v = 75,margin = 0,show=0, off_axis = 1):
...
def generate_image (data, bins_breaks, colormap, file_name_emb,size_p, fig_size,
    marker,alpha_v,cmap_vmin, cmap_vmax,margin, 
    min_v, num_bin,
    X_embedded=[],cor_x=[],cor_y=[]):
    '''
    ...
    if (X_embedded == []):
        ...
        fillup_image (cor_x=cor_x, cor_y=cor_y, X = data, colors = colors_array,
            ...
            size_p = size_p,marker_p = marker,alpha_v = alpha_v,cmap_vmin = cmap_vmin, cmap_vmax = cmap_vmax,margin = margin)
    elif (cor_x == [] or cor_y == []):
