...
class LightField(SpectralArray):
    ...
    def get_decimated(self, q: int, **kwargs) -> 'LightField':
        """Downsample the spectrum after applying an anti-aliasing filter
        in the spectral domain.

        See Also:
            ``:func:plenpy.utilities.core.SpectralArray.get_decimated()``
        ...
        """
        return LightField.from_spectral_array(super().get_decimated(q, **kwargs))

    def get_resampled(self, num: int, **kwargs) -> 'LightField':
        """Resample the LightField using FFT in the spectral domain.

        See Also:
            ``:func:plenpy.utilities.core.SpectralArray.get_resampled()``
        ...
        """
        return LightField.from_spectral_array(super().get_resampled(num, **kwargs))
    ...
    def get_msi(self,
        ...
        """Get multispectral LightField from hyperspectral LightField using
