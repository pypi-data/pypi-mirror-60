...
class WeirMixin(OptimizationProblem):
    ...
    def pre(self):
        super().pre()
        self._weir_discrete_symbols = []
        self._weir_mx_path_variables = []
        self._weir_status_bounds = OrderedDict()
        self._weir_status_pairs = OrderedDict()

        for w in self.weirs():
            # Define symbol names
            status_sym = '{}__status'.format(w.symbol)
            # Store variable names
            self._weir_discrete_symbols.append(status_sym)
            # Store bounds
            self._weir_status_bounds[status_sym] = (0, 1)
            # Generate and store MX variables
            status_mx = MX.sym(status_sym)
            self._weir_mx_path_variables.append(status_mx)
