...
def ensemble_lightgbm(forecast_in, forecast_out):

    forecast_in_copy =  forecast_in.copy()
    forecast_in_copy["month"] = forecast_in_copy.index.month
...
def ensemble_tsfresh(forecast_in,forecast_out,season):
  ...
  pca2_results = pca2.fit(extracted_n_selected)
...
class AutomatedModel():
    ...
    def ensemble(self, forecast_in, forecast_out):
      ...
      ensemble_lgb_in, ensemble_lgb_out = ensemble_lightgbm(forecast_in,forecast_out)

      print("Building LightGBM Ensemble from PCA reduced TSFresh Features (ensemble_ts). This can take a long time.")

      ensemble_ts_in, ensemble_ts_out = ensemble_tsfresh(forecast_in, forecast_out, season)
      ...
      middle_in, performance = ensemble_first(middle_in,forecast_in)
