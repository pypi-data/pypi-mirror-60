...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  nepnarnoldisetlagpreconditioner_(NEP nep,PetscInt *lag, int *__ierr){
*__ierr = NEPNArnoldiSetLagPreconditioner(
	(NEP)PetscToPointer((nep) ),*lag);
}
SLEPC_EXTERN void PETSC_STDCALL  nepnarnoldigetlagpreconditioner_(NEP nep,PetscInt *lag, int *__ierr){
*__ierr = NEPNArnoldiGetLagPreconditioner(
	(NEP)PetscToPointer((nep) ),lag);
}
SLEPC_EXTERN void PETSC_STDCALL  nepnarnoldisetksp_(NEP nep,KSP ksp, int *__ierr){
*__ierr = NEPNArnoldiSetKSP(
	(NEP)PetscToPointer((nep) ),
	(KSP)PetscToPointer((ksp) ));
}
SLEPC_EXTERN void PETSC_STDCALL  nepnarnoldigetksp_(NEP nep,KSP *ksp, int *__ierr){
