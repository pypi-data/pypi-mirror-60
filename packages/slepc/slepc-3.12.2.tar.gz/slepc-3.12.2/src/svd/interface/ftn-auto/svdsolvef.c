...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  svdsolve_(SVD svd, int *__ierr){
*__ierr = SVDSolve(
	(SVD)PetscToPointer((svd) ));
}
SLEPC_EXTERN void PETSC_STDCALL  svdgetiterationnumber_(SVD svd,PetscInt *its, int *__ierr){
*__ierr = SVDGetIterationNumber(
	(SVD)PetscToPointer((svd) ),its);
}
SLEPC_EXTERN void PETSC_STDCALL  svdgetconvergedreason_(SVD svd,SVDConvergedReason *reason, int *__ierr){
*__ierr = SVDGetConvergedReason(
	(SVD)PetscToPointer((svd) ),reason);
}
SLEPC_EXTERN void PETSC_STDCALL  svdgetconverged_(SVD svd,PetscInt *nconv, int *__ierr){
*__ierr = SVDGetConverged(
	(SVD)PetscToPointer((svd) ),nconv);
}
SLEPC_EXTERN void PETSC_STDCALL  svdcomputeerror_(SVD svd,PetscInt *i,SVDErrorType *type,PetscReal *error, int *__ierr){
