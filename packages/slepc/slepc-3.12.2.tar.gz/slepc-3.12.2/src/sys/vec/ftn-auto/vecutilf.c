...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  vecnormalizecomplex_(Vec xr,Vec xi,PetscBool *iscomplex,PetscReal *norm, int *__ierr){
*__ierr = VecNormalizeComplex(
	(Vec)PetscToPointer((xr) ),
	(Vec)PetscToPointer((xi) ),*iscomplex,norm);
}
SLEPC_EXTERN void PETSC_STDCALL  veccheckorthogonality_(Vec V[],PetscInt *nv,Vec W[],PetscInt *nw,Mat B,PetscViewer viewer,PetscReal *lev, int *__ierr){
*__ierr = VecCheckOrthogonality(V,*nv,W,*nw,
	(Mat)PetscToPointer((B) ),
	(PetscViewer)PetscToPointer((viewer) ),lev);
}
SLEPC_EXTERN void PETSC_STDCALL  vecduplicateempty_(Vec v,Vec *newv, int *__ierr){
