...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  pepsetup_(PEP pep, int *__ierr){
*__ierr = PEPSetUp(
	(PEP)PetscToPointer((pep) ));
}
SLEPC_EXTERN void PETSC_STDCALL  pepsetoperators_(PEP pep,PetscInt *nmat,Mat A[], int *__ierr){
*__ierr = PEPSetOperators(
	(PEP)PetscToPointer((pep) ),*nmat,A);
}
SLEPC_EXTERN void PETSC_STDCALL  pepgetoperators_(PEP pep,PetscInt *k,Mat *A, int *__ierr){
*__ierr = PEPGetOperators(
	(PEP)PetscToPointer((pep) ),*k,A);
}
SLEPC_EXTERN void PETSC_STDCALL  pepgetnummatrices_(PEP pep,PetscInt *nmat, int *__ierr){
*__ierr = PEPGetNumMatrices(
	(PEP)PetscToPointer((pep) ),nmat);
}
SLEPC_EXTERN void PETSC_STDCALL  pepallocatesolution_(PEP pep,PetscInt *extra, int *__ierr){
