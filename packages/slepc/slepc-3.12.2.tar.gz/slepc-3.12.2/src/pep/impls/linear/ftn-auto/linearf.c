...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  peplinearsetlinearization_(PEP pep,PetscReal *alpha,PetscReal *beta, int *__ierr){
*__ierr = PEPLinearSetLinearization(
	(PEP)PetscToPointer((pep) ),*alpha,*beta);
}
SLEPC_EXTERN void PETSC_STDCALL  peplineargetlinearization_(PEP pep,PetscReal *alpha,PetscReal *beta, int *__ierr){
*__ierr = PEPLinearGetLinearization(
	(PEP)PetscToPointer((pep) ),alpha,beta);
}
SLEPC_EXTERN void PETSC_STDCALL  peplinearsetexplicitmatrix_(PEP pep,PetscBool *explicitmatrix, int *__ierr){
*__ierr = PEPLinearSetExplicitMatrix(
	(PEP)PetscToPointer((pep) ),*explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  peplineargetexplicitmatrix_(PEP pep,PetscBool *explicitmatrix, int *__ierr){
*__ierr = PEPLinearGetExplicitMatrix(
	(PEP)PetscToPointer((pep) ),explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  peplinearseteps_(PEP pep,EPS eps, int *__ierr){
*__ierr = PEPLinearSetEPS(
	(PEP)PetscToPointer((pep) ),
	(EPS)PetscToPointer((eps) ));
}
SLEPC_EXTERN void PETSC_STDCALL  peplineargeteps_(PEP pep,EPS *eps, int *__ierr){
