...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  pepqarnoldisetrestart_(PEP pep,PetscReal *keep, int *__ierr){
*__ierr = PEPQArnoldiSetRestart(
	(PEP)PetscToPointer((pep) ),*keep);
}
SLEPC_EXTERN void PETSC_STDCALL  pepqarnoldigetrestart_(PEP pep,PetscReal *keep, int *__ierr){
*__ierr = PEPQArnoldiGetRestart(
	(PEP)PetscToPointer((pep) ),keep);
}
SLEPC_EXTERN void PETSC_STDCALL  pepqarnoldisetlocking_(PEP pep,PetscBool *lock, int *__ierr){
*__ierr = PEPQArnoldiSetLocking(
	(PEP)PetscToPointer((pep) ),*lock);
}
SLEPC_EXTERN void PETSC_STDCALL  pepqarnoldigetlocking_(PEP pep,PetscBool *lock, int *__ierr){
