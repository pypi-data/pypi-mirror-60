...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  mfnsolve_(MFN mfn,Vec b,Vec x, int *__ierr){
*__ierr = MFNSolve(
	(MFN)PetscToPointer((mfn) ),
	(Vec)PetscToPointer((b) ),
	(Vec)PetscToPointer((x) ));
}
SLEPC_EXTERN void PETSC_STDCALL  mfngetiterationnumber_(MFN mfn,PetscInt *its, int *__ierr){
*__ierr = MFNGetIterationNumber(
	(MFN)PetscToPointer((mfn) ),its);
}
SLEPC_EXTERN void PETSC_STDCALL  mfngetconvergedreason_(MFN mfn,MFNConvergedReason *reason, int *__ierr){
