...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschursetrestart_(EPS eps,PetscReal *keep, int *__ierr){
*__ierr = EPSKrylovSchurSetRestart(
	(EPS)PetscToPointer((eps) ),*keep);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetrestart_(EPS eps,PetscReal *keep, int *__ierr){
*__ierr = EPSKrylovSchurGetRestart(
	(EPS)PetscToPointer((eps) ),keep);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschursetlocking_(EPS eps,PetscBool *lock, int *__ierr){
*__ierr = EPSKrylovSchurSetLocking(
	(EPS)PetscToPointer((eps) ),*lock);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetlocking_(EPS eps,PetscBool *lock, int *__ierr){
*__ierr = EPSKrylovSchurGetLocking(
	(EPS)PetscToPointer((eps) ),lock);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschursetpartitions_(EPS eps,PetscInt *npart, int *__ierr){
*__ierr = EPSKrylovSchurSetPartitions(
	(EPS)PetscToPointer((eps) ),*npart);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetpartitions_(EPS eps,PetscInt *npart, int *__ierr){
*__ierr = EPSKrylovSchurGetPartitions(
	(EPS)PetscToPointer((eps) ),npart);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschursetdetectzeros_(EPS eps,PetscBool *detect, int *__ierr){
*__ierr = EPSKrylovSchurSetDetectZeros(
	(EPS)PetscToPointer((eps) ),*detect);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetdetectzeros_(EPS eps,PetscBool *detect, int *__ierr){
*__ierr = EPSKrylovSchurGetDetectZeros(
	(EPS)PetscToPointer((eps) ),detect);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschursetdimensions_(EPS eps,PetscInt *nev,PetscInt *ncv,PetscInt *mpd, int *__ierr){
*__ierr = EPSKrylovSchurSetDimensions(
	(EPS)PetscToPointer((eps) ),*nev,*ncv,*mpd);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetdimensions_(EPS eps,PetscInt *nev,PetscInt *ncv,PetscInt *mpd, int *__ierr){
*__ierr = EPSKrylovSchurGetDimensions(
	(EPS)PetscToPointer((eps) ),nev,ncv,mpd);
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurgetsubcommpairs_(EPS eps,PetscInt *i,PetscScalar *eig,Vec v, int *__ierr){
*__ierr = EPSKrylovSchurGetSubcommPairs(
	(EPS)PetscToPointer((eps) ),*i,eig,
	(Vec)PetscToPointer((v) ));
}
SLEPC_EXTERN void PETSC_STDCALL  epskrylovschurupdatesubcommmats_(EPS eps,PetscScalar *s,PetscScalar *a,Mat Au,PetscScalar *t,PetscScalar *b,Mat Bu,MatStructure *str,PetscBool *globalup, int *__ierr){
