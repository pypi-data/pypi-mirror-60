...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  epssolve_(EPS eps, int *__ierr){
*__ierr = EPSSolve(
	(EPS)PetscToPointer((eps) ));
}
SLEPC_EXTERN void PETSC_STDCALL  epsgetiterationnumber_(EPS eps,PetscInt *its, int *__ierr){
*__ierr = EPSGetIterationNumber(
	(EPS)PetscToPointer((eps) ),its);
}
SLEPC_EXTERN void PETSC_STDCALL  epsgetconverged_(EPS eps,PetscInt *nconv, int *__ierr){
*__ierr = EPSGetConverged(
	(EPS)PetscToPointer((eps) ),nconv);
}
SLEPC_EXTERN void PETSC_STDCALL  epsgetconvergedreason_(EPS eps,EPSConvergedReason *reason, int *__ierr){
*__ierr = EPSGetConvergedReason(
	(EPS)PetscToPointer((eps) ),reason);
}
SLEPC_EXTERN void PETSC_STDCALL  epsgetinvariantsubspace_(EPS eps,Vec v[], int *__ierr){
*__ierr = EPSGetInvariantSubspace(
	(EPS)PetscToPointer((eps) ),v);
}
SLEPC_EXTERN void PETSC_STDCALL  epsgeteigenvector_(EPS eps,PetscInt *i,Vec Vr,Vec Vi, int *__ierr){
*__ierr = EPSGetEigenvector(
	(EPS)PetscToPointer((eps) ),*i,
	(Vec)PetscToPointer((Vr) ),
	(Vec)PetscToPointer((Vi) ));
}
SLEPC_EXTERN void PETSC_STDCALL  epsgetlefteigenvector_(EPS eps,PetscInt *i,Vec Wr,Vec Wi, int *__ierr){
*__ierr = EPSGetLeftEigenvector(
	(EPS)PetscToPointer((eps) ),*i,
	(Vec)PetscToPointer((Wr) ),
	(Vec)PetscToPointer((Wi) ));
}
SLEPC_EXTERN void PETSC_STDCALL  epsgeterrorestimate_(EPS eps,PetscInt *i,PetscReal *errest, int *__ierr){
*__ierr = EPSGetErrorEstimate(
	(EPS)PetscToPointer((eps) ),*i,errest);
}
SLEPC_EXTERN void PETSC_STDCALL  epscomputeerror_(EPS eps,PetscInt *i,EPSErrorType *type,PetscReal *error, int *__ierr){
