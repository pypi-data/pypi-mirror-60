...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  fncombinesetchildren_(FN fn,FNCombineType *comb,FN f1,FN f2, int *__ierr){
*__ierr = FNCombineSetChildren(
	(FN)PetscToPointer((fn) ),*comb,
	(FN)PetscToPointer((f1) ),
	(FN)PetscToPointer((f2) ));
}
SLEPC_EXTERN void PETSC_STDCALL  fncombinegetchildren_(FN fn,FNCombineType *comb,FN *f1,FN *f2, int *__ierr){
