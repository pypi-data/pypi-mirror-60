...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  mfnsetfromoptions_(MFN mfn, int *__ierr){
*__ierr = MFNSetFromOptions(
	(MFN)PetscToPointer((mfn) ));
}
SLEPC_EXTERN void PETSC_STDCALL  mfnsettolerances_(MFN mfn,PetscReal *tol,PetscInt *maxits, int *__ierr){
*__ierr = MFNSetTolerances(
	(MFN)PetscToPointer((mfn) ),*tol,*maxits);
}
SLEPC_EXTERN void PETSC_STDCALL  mfngetdimensions_(MFN mfn,PetscInt *ncv, int *__ierr){
*__ierr = MFNGetDimensions(
	(MFN)PetscToPointer((mfn) ),ncv);
}
SLEPC_EXTERN void PETSC_STDCALL  mfnsetdimensions_(MFN mfn,PetscInt *ncv, int *__ierr){
*__ierr = MFNSetDimensions(
	(MFN)PetscToPointer((mfn) ),*ncv);
}
SLEPC_EXTERN void PETSC_STDCALL  mfnseterrorifnotconverged_(MFN mfn,PetscBool *flg, int *__ierr){
*__ierr = MFNSetErrorIfNotConverged(
	(MFN)PetscToPointer((mfn) ),*flg);
}
SLEPC_EXTERN void PETSC_STDCALL  mfngeterrorifnotconverged_(MFN mfn,PetscBool *flag, int *__ierr){
