...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  nepnleigsseteps_(NEP nep,EPS eps, int *__ierr){
*__ierr = NEPNLEIGSSetEPS(
	(NEP)PetscToPointer((nep) ),
	(EPS)PetscToPointer((eps) ));
}
SLEPC_EXTERN void PETSC_STDCALL  nepnleigsgeteps_(NEP nep,EPS *eps, int *__ierr){
