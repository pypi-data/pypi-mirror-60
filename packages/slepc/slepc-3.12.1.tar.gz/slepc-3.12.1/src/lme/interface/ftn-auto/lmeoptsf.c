...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  lmesetfromoptions_(LME lme, int *__ierr){
*__ierr = LMESetFromOptions(
	(LME)PetscToPointer((lme) ));
}
SLEPC_EXTERN void PETSC_STDCALL  lmesetproblemtype_(LME lme,LMEProblemType *type, int *__ierr){
*__ierr = LMESetProblemType(
	(LME)PetscToPointer((lme) ),*type);
}
SLEPC_EXTERN void PETSC_STDCALL  lmegetproblemtype_(LME lme,LMEProblemType *type, int *__ierr){
*__ierr = LMEGetProblemType(
	(LME)PetscToPointer((lme) ),type);
}
SLEPC_EXTERN void PETSC_STDCALL  lmesettolerances_(LME lme,PetscReal *tol,PetscInt *maxits, int *__ierr){
*__ierr = LMESetTolerances(
	(LME)PetscToPointer((lme) ),*tol,*maxits);
}
SLEPC_EXTERN void PETSC_STDCALL  lmegetdimensions_(LME lme,PetscInt *ncv, int *__ierr){
*__ierr = LMEGetDimensions(
	(LME)PetscToPointer((lme) ),ncv);
}
SLEPC_EXTERN void PETSC_STDCALL  lmesetdimensions_(LME lme,PetscInt *ncv, int *__ierr){
*__ierr = LMESetDimensions(
	(LME)PetscToPointer((lme) ),*ncv);
}
SLEPC_EXTERN void PETSC_STDCALL  lmeseterrorifnotconverged_(LME lme,PetscBool *flg, int *__ierr){
*__ierr = LMESetErrorIfNotConverged(
	(LME)PetscToPointer((lme) ),*flg);
}
SLEPC_EXTERN void PETSC_STDCALL  lmegeterrorifnotconverged_(LME lme,PetscBool *flag, int *__ierr){
