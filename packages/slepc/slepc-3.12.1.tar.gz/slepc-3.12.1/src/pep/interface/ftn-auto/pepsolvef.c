...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  pepsolve_(PEP pep, int *__ierr){
*__ierr = PEPSolve(
	(PEP)PetscToPointer((pep) ));
}
SLEPC_EXTERN void PETSC_STDCALL  pepgetiterationnumber_(PEP pep,PetscInt *its, int *__ierr){
*__ierr = PEPGetIterationNumber(
	(PEP)PetscToPointer((pep) ),its);
}
SLEPC_EXTERN void PETSC_STDCALL  pepgetconverged_(PEP pep,PetscInt *nconv, int *__ierr){
*__ierr = PEPGetConverged(
	(PEP)PetscToPointer((pep) ),nconv);
}
SLEPC_EXTERN void PETSC_STDCALL  pepgetconvergedreason_(PEP pep,PEPConvergedReason *reason, int *__ierr){
*__ierr = PEPGetConvergedReason(
	(PEP)PetscToPointer((pep) ),reason);
}
SLEPC_EXTERN void PETSC_STDCALL  pepgeterrorestimate_(PEP pep,PetscInt *i,PetscReal *errest, int *__ierr){
*__ierr = PEPGetErrorEstimate(
	(PEP)PetscToPointer((pep) ),*i,errest);
}
SLEPC_EXTERN void PETSC_STDCALL  pepcomputeerror_(PEP pep,PetscInt *i,PEPErrorType *type,PetscReal *error, int *__ierr){
