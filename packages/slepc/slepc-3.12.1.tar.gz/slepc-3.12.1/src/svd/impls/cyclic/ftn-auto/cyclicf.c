...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  svdcyclicsetexplicitmatrix_(SVD svd,PetscBool *explicitmatrix, int *__ierr){
*__ierr = SVDCyclicSetExplicitMatrix(
	(SVD)PetscToPointer((svd) ),*explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  svdcyclicgetexplicitmatrix_(SVD svd,PetscBool *explicitmatrix, int *__ierr){
*__ierr = SVDCyclicGetExplicitMatrix(
	(SVD)PetscToPointer((svd) ),explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  svdcyclicseteps_(SVD svd,EPS eps, int *__ierr){
*__ierr = SVDCyclicSetEPS(
	(SVD)PetscToPointer((svd) ),
	(EPS)PetscToPointer((eps) ));
}
SLEPC_EXTERN void PETSC_STDCALL  svdcyclicgeteps_(SVD svd,EPS *eps, int *__ierr){
