...
extern "C" {
#endif
SLEPC_EXTERN void PETSC_STDCALL  svdcrosssetexplicitmatrix_(SVD svd,PetscBool *explicitmatrix, int *__ierr){
*__ierr = SVDCrossSetExplicitMatrix(
	(SVD)PetscToPointer((svd) ),*explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  svdcrossgetexplicitmatrix_(SVD svd,PetscBool *explicitmatrix, int *__ierr){
*__ierr = SVDCrossGetExplicitMatrix(
	(SVD)PetscToPointer((svd) ),explicitmatrix);
}
SLEPC_EXTERN void PETSC_STDCALL  svdcrossseteps_(SVD svd,EPS eps, int *__ierr){
*__ierr = SVDCrossSetEPS(
	(SVD)PetscToPointer((svd) ),
	(EPS)PetscToPointer((eps) ));
}
SLEPC_EXTERN void PETSC_STDCALL  svdcrossgeteps_(SVD svd,EPS *eps, int *__ierr){
