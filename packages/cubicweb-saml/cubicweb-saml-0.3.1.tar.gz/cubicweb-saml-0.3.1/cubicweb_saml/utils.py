    ...
    with request.registry['cubicweb.repository'].internal_cnx() as cnx:
        try:
            ...
            user = cnx.execute('INSERT CWUser U: U login %(login)s, '
                               ...
                               {
                                   'login': text_type(subject),
                                   'password': text_type(password),
                                   'group': default_group,
                               }).one().eid
            cnx.commit()
...
def generate_hash(*args):
    ...
    for element in args:
        hash_value.update(text_type(element).encode('utf-8'))

    hash_value.update(
        text_type('{:06d}'.format(randrange(1, pow(10, 6)))).encode('utf-8'))
