...
def get_scene_timesample(file_path: str,
    ...
    - ego_traj:     planned ego trajectory starting at the current position (x, y, heading, curvature, vel, acc)
    - object_array: information about the vehicles in the scene (list of lists, each object holding an ID-string and a
                    list holding [x, y, psi, obj_radius, vel])
    """
    # -- get timestamps ------------------------------------------------------------------------------------------------
...
def interp_1d(x: float,
    ...
    Input:
    -x:                 Desired index for new value
    -xp:                Existing series of indexes
    -fp_array:          Array of existing function return values (matching xp)
    - idx_col_heading:  List of ints holding the columns to be treated as headings (avoid jumps when crossing pi/-pi)
    """
