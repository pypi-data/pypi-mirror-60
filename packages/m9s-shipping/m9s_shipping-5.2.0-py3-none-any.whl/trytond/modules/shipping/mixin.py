...
class ShipmentCarrierMixin(PackageMixin):
    ...
    def label_wizard(cls, shipments):
        if len(shipments) == 0:
            raise UserError(gettext('no_shipments'))
        elif len(shipments) > 1:
            raise UserError(gettext('too_many_shipments'))
    ...
    def _get_ship_from_address(self, silent=False):
        ...
        if not silent:
            raise UserError(gettext('warehouse_address_missing'))
    ...
    def allow_label_generation(self):
        ...
        if self.state not in ('packed', 'done'):
            raise UserError(gettext('invalid_state'))

        if self.tracking_number:
            raise UserError(gettext('tracking_number_already_present'))
