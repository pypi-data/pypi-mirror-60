...
class Client():
    def __init__(self, password, ip='192.168.31.246', name="deep_racer"):
    ...
    def move_forward(self, speed=-0.7, timeout=1):
        self.move(0, speed); time.sleep(timeout); client.move(0, -0.0); time.sleep(0.5)

    def move_backward(self, speed=0.5, timeout=1):
        self.move(0, speed); time.sleep(timeout); client.move(0, -0.0); time.sleep(0.5)

    def turn_right(self, speed=-0.7, timeout=1):
        self.move(-1, speed); time.sleep(timeout); client.move(0, -0.0); time.sleep(0.5)

    def turn_left(self, speed=-0.7, timeout=1):
        self.move(1, speed); time.sleep(timeout); client.move(0, -0.0); time.sleep(0.5)
    ...
    def move(self, steering_angle, throttle):
    ...
    def upload_model(self, model_zip_path, model_name):
        ...
        return self.session.put(upload_models_url, data=multipart_data, headers=headers, verify=False)
    ...
    def _get(self, url, check_status_code=True):
        ...
        if check_status_code:
            if response.status_code != 200:
                raise DeepracerVehicleApiError("Get action failed with status code {}".format(response.status_code))
        return json.loads(response.text)

    def _put(self, url, data, check_success=True):
        self._get_csrf_token()
        logging.debug("> Put %s with %s",url, data)
        response = self.session.put(self.URL + url, json=data, headers=self.headers, verify=False)
        if check_success:
            if response.status_code != 200 or response.text.find("success\":true") < 0:
                raise DeepracerVehicleApiError("Put action failed with body text {}".format(response.text))
        return json.loads(response.text) 
    ...
    def _get_csrf_token(self):
        ...
        try:
            ...
            raise DeepracerVehicleApiError("The vehicle with URL '{}' did not respond".format(self.URL))
        ...
        post = self.session.post(login_url, data=payload, headers=self.headers, verify=False)
        if post.status_code != 200:
            raise DeepracerVehicleApiError("Log in failed. Error message {}".format(post.text))
