...
class Detector:
    ...
    def __init__(self, target, **kwargs):
        ...
        for t in self.target_objects:  # for each object set validate
            ...
            if count > 1 and not primary:
                logger.error(
                    "Multiple protein files found for {} one must be renamed to primary".format(
                        t
                    )
                )
                sys.exit(1)
    ...
    def add_target_object(self):
        ...
        if canonical_type not in self.canonical_types:  # regject
            logger.debug(
                "Type {} not recognized in {}.  Skipping".format(
                    canonical_type, self.canonical_types
                )
            )
            return
        organism_dir_path = os.path.dirname(os.path.dirname(target))  # org dir
        organism_dir = os.path.basename(
            os.path.dirname(os.path.dirname(target))
        )  # org dir
        ...
        file_url = "{}/{}/{}/{}".format(
            self.domain, organism_dir, target_dir, self.target_name
        )
        target_node_object = {
        ...
        if len(target_attributes) > 7 and target_ref_type:  # check parent
            ...
            ref_glob = "{}/{}*/*{}.*.gz".format(
                organism_dir_path, ".".join(target_attributes[1:3]), target_ref_type
            )
            ...
            if my_reference not in self.target_objects:  # new parent
                ...
                file_url = "{}/{}/{}/{}".format(
                    self.domain, organism_dir, ref_dir, parent_name
                )
                ref_node_object = {
    ...
    def detect_incongruencies(self):
        ...
        for reference in sorted(targets, key=lambda k: self.rank[targets[k]["type"]]):
            ...
            if reference not in self.passed:
                ...
                ref_method = getattr(
                    specification_checks,  # reads checks from spec
                    targets[reference]["type"],
                )  # type ex genome_main
                if not ref_method:  # if the target isnt in the hierarchy continue
                    logger.debug(
                        "Check for {} does not exist".format(targets[reference]["type"])
                    )
                    continue
            ...
            if self.target_objects[reference]["children"]:  # process children
                ...
                for c in children:
                    ...
                    if not child_method:
                        logger.warning(
                            "Check for {} does not exist".format(children[c]["type"])
                        )
                        continue
