...
class SimpleEnsemblingRunner(AbstractRunner):
    def runTrial(self, saver, seed,  n, s, low_bound, up_bound):
        ...
        if self.minimize_drawdown:
            ...
            df_res.loc[:, 'roll_blend'] = portfolio
            _results.loc[:, 'DD'] = portfolio

        if self.equaly_weighted:
            print('equally weighted')
            df_res = df_res.fillna(method='bfill')
            df_res.loc[:, 'roll_blend'] = df_res.mean(axis=1)
            _results.loc[:, 'eq_weight'] = df_res.loc[:, 'roll_blend']
        ...
        saver.saveResults(savingKey, portfolio, weight_mat, None, None)
