...
class ParalleLauncher():
    ...
    def saveResults(self, run, values, weights_df, recordings_df, activations_df):
        self.addRun(run)
    ...
    def checkRunExistence(self):
        sqliteConnection = None
        runExistence=None
        try:
            sqliteConnection = sqlite3.connect(self.db_path)
            sqlite_select_run_query = 'SELECT * FROM parallel_run where run = '+ run
            cursor = sqliteConnection.cursor()
            print("Successfully Connected to SQLite")
            ...
            print("Error while selecting run", error)
            runExistence = True
        finally:
            if (sqliteConnection):
                sqliteConnection.close()
