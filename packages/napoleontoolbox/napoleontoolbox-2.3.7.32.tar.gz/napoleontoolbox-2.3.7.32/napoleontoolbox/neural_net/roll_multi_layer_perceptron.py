...
class RollMultiLayerPerceptron(MultiLayerPerceptron, roller._RollingBasis):
    ...
    def eval_predictor_importance(self, features, actual_predictions, features_names, display = False):

        shapley_values = shap.DeepExplainer(self, features).explainer.data[0]
        deep_explainer = shap.DeepExplainer(self, features)
        expected_value = deep_explainer.expected_value
        shap_values_bis = deep_explainer.shap_values(features)
        ...
        shap_values = explainer_shap.shap_values(features,nsamples = 100, ranked_outputs=True,output_rank_order='max',rseed=None,return_variances=False)
    ...
    def unroll_features(self, dates, feature_names, display_slices = False):
        self.handle_callback('begin_fit')
        rows_list = []
        dates_list = []
        prediction_list = []
