    ...
    def ratio_computer(self,features_df=None, features_segmentation = {}):
        ...
        for key, features_values in extended_features_segmentation.items():
            ...
            if part_over_time_df is None:
                ...
                temp = grouped_features_df.abs().sum(axis=1)
                temp.columns = [key]
                part_over_time_df = pd.concat([part_over_time_df, temp],
                                              axis=1)

        part_over_time_df = part_over_time_df / part_over_time_df.sum(axis=1).to_frame()

        return features_importance_summarized, advanced_features_importance, part_over_time_df
