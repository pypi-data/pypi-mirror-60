...
def oc_check() -> None:
    ...
    oc_version = _subprocess_run(["oc", "version"])
...
def _do_import_image(namespace: str, image: str, stdout=None) -> None:
...
def import_thoth_s2i_image(
    namespace: str, image: Optional[str] = None, stdout=sys.stdout
) -> None:
...
def change_image_stream(
    build_configs: Dict[str, BuildConfig], new, cond_func,
    ...
    for build_config in build_configs.values():
        ...
        build_config.replace_in_file(build_config.get_image_stream_name(), new)
