...
from yawrap._sourcer import BODY_BEGIN, BODY_END, HEAD, _Resource
...
class Yawrap(Doc):
    ...
    def local_link(self, target, *args, **kwargs):
        rel_location = self.get_rel_path(target)
    ...
    def add(self, js_or_css_resource):
        assert isinstance(js_or_css_resource, _Resource), "Bad usage, expected CSS or JS" \
                                                          " resource definition, got %s." % type(
            js_or_css_resource).__name__
        self._additional_resources.append(js_or_css_resource)

    def get_rel_path(self, target_local_file):
