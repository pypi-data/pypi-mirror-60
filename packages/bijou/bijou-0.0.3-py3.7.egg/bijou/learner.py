...
class Learner():
    def __init__(self, model, opt, loss_func, data, metrics=None, callbacks=None, callback_attrs=None, device=None):
        """
        Args:
            model: pytorch 模型
            opt: 优化器
            ...
            callbacks: callbacks对象列表
            callback_attrs: callbacks类列表，其中的每个callback会被作为 learner 的属性，
        ...
        cb_attrs = tbox.listify(callback_attrs) + [cbks.Recorder]
        # if metrics:
