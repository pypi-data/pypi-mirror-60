...
from tkinter.ttk import Frame, Label, Entry, Button, Labelframe, OptionMenu, Label, Progressbar
...
class Sidebar(Frame):
    def __init__(self, master):
        ...
        self.clear_btn_grid = {'sticky': 'ew', 'padx': '.25cm', 'row': 2, 'column': 1}

        # status area
        # self.grid_rowconfigure(3, weight=1)
        # self.status = Label(self)
        # self.status.grid(sticky='sw')
    ...
    def toggle_clear_btn(self, show=False):
        if show:
            ...
            self.clear_btn.grid(**self.clear_btn_grid)
        else:
    ...
    # def set_progress(self, start=True):
    #     if start:
    #         self.status.config(text='Sending...', foreground='Yellow')
    #         # self.progress.grid(sticky='w')
    #         # self.progress.start()
    #         return
