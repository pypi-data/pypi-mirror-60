...
class BankStatementTestCase(unittest.TestCase):
    ...
    def test_cash_flows(self):
        bs = BankStatement()
        inflow = CashFlow('Inflow', CashFlowType.CREDIT)
        outflow1 = CashFlow('Outflow 1', CashFlowType.DEBIT)
        outflow2 = CashFlow('Outflow 2', CashFlowType.DEBIT)

        bs.inflows.append(inflow)
        bs.outflows.extend([outflow1, outflow2])

        self.assertListEqual([inflow, outflow1, outflow2], bs.cashflows())
