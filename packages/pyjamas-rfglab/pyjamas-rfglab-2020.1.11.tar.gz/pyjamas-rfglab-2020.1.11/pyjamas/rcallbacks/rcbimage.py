...
class RCBImage(RCallback):
    ...
    def cbMIPImage(self, slice_list: List[int]) -> bool:
        if slice_list is False:
            slice_list_str, ok_pressed = QtWidgets.QInputDialog.getText(None, "Maximum intenstiy projection",
                                                                        "Enter a range of slices (e.g. 0, 1, 4-8, 15): ",
        ...
        for i in range(num_slices):
            ...
            for aroi in therois:
                ...
                # If watershed did not work, just try a local threshold.
                   ...
                   contour_list = rimutils.local_threshold_segm(roi_im, window_size=self.DEFAULT_WINDOW_SZ/2, binary_dilation_radius=self.DEFAULT_BINARY_DILATIONS/2, border_objects=False)
