...
class SLA_Calculator(object):
    ...
    def check_working_days(self, start_time, country_holidays, open_hour):
        while start_time in country_holidays:
            ...
            start_time = pendulum.datetime(start_time.year, start_time.month, start_time.day, open_hour, 0, 0, tz=pendulum.local_timezone())
        while start_time.is_weekend():
            start_time = start_time.add(days=1)
            start_time = pendulum.datetime(start_time.year, start_time.month, start_time.day, open_hour, 0, 0, tz=pendulum.local_timezone())
        return start_time

    def calculate(self, start_time, open_hour, close_hour, country_name='US', sla_in_hours=4, province=None, state=None):
        sla_time = None
        ...
        if start_time < open_time:
            ...
            open_time = pendulum.datetime(start_time.year, start_time.month, start_time.day, open_hour, 0, 0, tz=pendulum.local_timezone())
            close_time = pendulum.datetime(start_time.year, start_time.month, start_time.day, close_hour, 0, 0, tz=pendulum.local_timezone())
