...
def qigua():
    ...
    number_trans = {"1":"一", "2":"二", "3":"三"}
    gua_name = number_trans.get(str(gua_number)[0]) + "方" + number_trans.get(str(gua_number)[1]) + "州"+ number_trans.get(str(gua_number)[2])  + "部" + number_trans.get(str(gua_number)[3])+"家"
    zhan_number = zhan_dict.get(qigua_number()[1])
    zhan = gua_details.get(zhan_number)
    return gua_number, gua_name, gua, gua_details, zhan_number, zhan
