...
class Cellpose():
    ...
    def __init__(self, device=mx.cpu(), pretrained_model=None, pretrained_size=None, diam_mean=27.):
    ...
    def eval(self, x, channels=None, rescale=1.0, do_3D=False, net_avg=True, progress=None):
        ...
        if self.pretrained_size is not None and rescale is None and not do_3D:
            diams = self.sz.eval(x, channels=channels, batch_size=self.batch_size)
        ...
        masks, flows, styles = self.cp.eval(x, rescale=rescale, channels=channels,
...
class SizeModel():
    ...
    def eval(self, x=None, feat=None, channels=None, batch_size=8, progress=None):
        ...
        if feat is None:
            for i in trange(nimg):
                img = x[i]
                feat = self.cp.eval([img], net_avg=False)[-1]
                if progress is not None:
                    progress.setValue(30)
                diam[i] = self.size_estimation(feat)
            if progress is not None:
                progress.setValue(50)
            masks = self.cp.eval(x, rescale=self.diam_mean/diam, net_avg=False)[0]
...
class CellposeModel():
    def __init__(self, device, pretrained_model=None, batch_size=8, diam_mean=27.):
    ...
    def eval(self, x, rescale=None, tile=True, net_avg=True, channels=None,
        ...
        if nimg>1 and do_3D:
            ...
            for p in range(3):
                ...
                print('running %s \n'%sstr[p])
                for i in iterator:
                    y, style = self.run_net(xsl[i].copy(), tile=True)
    ...
    def run_net(self, img, rsz=1.0, tile=True, bsize=224):
        ...
        imgi, ysub, xsub = utils.pad_image(imgi)

        if tile:
            y,style = self.run_tiled(imgi, bsize)
        else:
            y,style = self.net(imgi)
        style /= (style**2).sum()**0.5


        yup = np.transpose(y[:3], (1,2,0))
        yup = yup[np.ix_(ysub, xsub, np.arange(3))]
