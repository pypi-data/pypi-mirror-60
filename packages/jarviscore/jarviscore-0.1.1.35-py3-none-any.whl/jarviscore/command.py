from .message import *
from .socket import SendSocket

class CommandResponse():
    """
    The command response object is used to communicate the outcome of a command 
    to the caller for logging purposes.

    A Command Response consists of the following:
     - `Status`: a `string` indicating the message level
     - `Message`: a `string` with a message to be logged in the console.
...
class Command():
    ...
    def setup(self, socket: SendSocket):
        raise NotImplementedError()
        #client.log.debug(f"Loaded Command {self.name}")

    def teardown(self, socket: SendSocket):
        raise NotImplementedError()
