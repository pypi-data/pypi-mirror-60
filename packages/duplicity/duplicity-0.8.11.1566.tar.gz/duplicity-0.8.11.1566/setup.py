...
def get_source_files():
    '''gen list of source files, all from SCM'''
    ...
    if len(source_files) == 0:
        ...
        source_files = bzr.communicate()[0].split()

    # or use git to get files list
    if len(source_files) == 0:
        git = Popen([u"git", u"ls-files"], stdout=PIPE, universal_newlines=True)
        source_files = git.communicate()[0].split()

    assert len(source_files) > 0, u"No files list returned from any source."

    return [fn.strip() for fn in source_files]
