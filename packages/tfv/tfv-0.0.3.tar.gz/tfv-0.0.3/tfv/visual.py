...
class ParticlesScatter(Visual):
    ...
    scale : float
        Number used to scale marker size
    ...
    def __init__(self, axes, extractor, expression=None, datum='sigma', limits=(0, 1), show=None,
                 highlight=None, track=0, scale=0.001, shape='tri', count=10000, **kwargs):
    ...
    def __dynamic_update__(self):
        # Get logical index of particles to show
        if self.show is None:
            show = np.ones((self.extractor.np,), dtype=np.bool)
        else:
            show = self.extractor.get_raw_data(self.show, self._time_index)

        # Get elevation filter logical index
        in_limit = self.extractor.get_vertical_selection(self._time_index, self.datum, self.limits)

        # Get valid particles based on mask
        valid = np.invert(self.extractor.get_mask_vector(self._time_index))

        # Update show logical index
        show = show & in_limit & valid

        # Get index of particles which will be shown via sub-setting
        ns = np.sum(show)
        if ns != 0:
            step = np.ceil(ns / self.count)
            sub_set = np.arange(0, ns, step).astype(np.int)
            index = np.where(show)[0][sub_set]

            # Get logical index of particles to highlight
            if self.highlight is None:
                highlight = np.zeros((self.extractor.np,), dtype=np.bool)[index]
            else:
                highlight = self.extractor.get_raw_data(self.highlight, self._time_index)[index]
            highlight[0] = False
            nh = np.sum(highlight)

            # Update patch faces
            x = self.extractor.get_raw_data('x', self._time_index)[index]
            y = self.extractor.get_raw_data('y', self._time_index)[index]

            node_x = np.add(x.reshape(ns, 1), self.scale*self.shape_x)
            node_y = np.add(y.reshape(ns, 1), self.scale*self.shape_y)
            ...
            if self.expression is None:
                ...
                face_colours[highlight, :] = np.tile(y, (nh, 1))
                ...
                # Get face colours
                norm = self.patch.norm(data[index])
