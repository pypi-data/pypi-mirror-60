...
class MongoEventSourceRepository(EventPublisherRepository, metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, client: MongoClient, database="fenrys", collection="event_store"
    ):
    ...
    def max_version_for_object(self, object_id):  # noqa: D102
        max_version_event_cursor = (
            self.__collection.find({"object_id": object_id})
            .sort([("version", -1)])
            .limit(1)
        )
        max_version_event = list(max_version_event_cursor)
        max_version_event_cursor.close()
