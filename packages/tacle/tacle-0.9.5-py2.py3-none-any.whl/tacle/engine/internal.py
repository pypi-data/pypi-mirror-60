...
class InternalSolvingStrategy(DictSolvingStrategy):
    def __init__(self):
                    ...
                    if not pk_v.overlaps_with(pv_v):
                        ...
                        for fk_v, fv_v in itertools.product(fk, fv):
                            if (
                                ...
                                and is_lookup(
                                    pk_dict, fk_v.get_vector(1), fv_v.get_vector(1)
                                )
                            ):
                                result = {
                                    c.o_key: pk_v,
                ...
                for ok_v in ok_block:
                    for fk_v in fk_block:
                        ...
                        if key not in overlap:
                            overlap[key] = (
                                len(set(ok_v.get_vector(1)) & set(fk_v.get_vector(1)))
                                > 0
                            )
                        if overlap[key]:
                            candidate = True
        ...
        def aggregate(
            c: Aggregate, assignments: List[Dict[str, Group]], solutions: Solutions
        ):
            results = []
            ...
            for assignment in assignments:
                ...
                o_match = Filter.orientation(
                    x_group
                ) == indexing.Orientation.from_legacy_orientation(c.orientation)
                if o_match:
                    sums = c.operation.aggregate(
                        x_group.data, 0 if o_column else 1, x_group.is_partial
                    )
                    for y_vector_group in y_group:
                        match = pattern_finder(sums, y_vector_group.get_vector(1))
                        x_match = [
                            x_group.vector_subset(m + 1, m + y_length) for m in match
                        ]
                        for x in x_match:
                    ...
                    def check(start, end):
                        ...
                        for i in range(0, d2):
                            result = c.operation.aggregate(
                                x_data[start:end, i], 0, x_group.is_partial
                            )
        ...
        def xor_vector(c: MutualExclusiveVector, assignments, solutions):
            return self._generate_test_vectors(
                assignments, [c.x], lambda xb: c.test_data(*to_data(xb))
            )
