    ...
    def updateFromRequest(self, request, removed=None, function=None):
        ...
        if self._dataIsArray(data):
            ...
            for data_item in data:
                ...
                if isinstance(data_item, dict):
                    if function:
                        ...
                        if (cloned_data.get('id') != None or cloned_data.get('parent_id')) and  cloned_data.get('id') == cloned_data.get('parent_id'):
                            raise Exception(f'Недопустимость циклических ссылок !!!')
                        if data_item.get('id'):
            ...
            if function:
                ...
                cloned_data = self.clone_data(data)
                if cloned_data.get('id') == cloned_data.get('parent_id'):
                    raise Exception(f'Недопустимость циклических ссылок !!!')
                if request.get_id():
