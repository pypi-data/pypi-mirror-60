...
class Session:
    ...
    async def _select(self, relation_sql: psycopg2.sql.Composable, query_rules: Optional[QueryRulesRuleSetParser], returning_columns: Optional[List[str]], order: Optional[List[Dict[str, str]]], limit: Optional[int], _slice: Optional[Dict], chronicle_note: Optional[str], require_updatable: Optional[bool]) -> Optional[JsonLiteral]:
        fetch_result = True
    ...
    async def handle_delete(self, wspg_msg: Dict[str, Any]) -> Optional[JsonLiteral]:
        ...
        return await self._delete(relation_sql, query_rules, returning_columns, chronicle_note)

    async def _delete(self, relation_sql: psycopg2.sql.Composable, query_rules: Optional[QueryRulesRuleSetParser], returning_columns: Optional[List[str]], chronicle_note: Optional[str]) -> Optional[JsonLiteral]:
        if query_rules:
            where_sql = psycopg2.sql.SQL("WHERE {}").format(query_rules)
        else:
            where_sql = psycopg2.sql.SQL("")
        ...
        return await self._execute_sql(delete_sql, fetch_result, chronicle_note)
