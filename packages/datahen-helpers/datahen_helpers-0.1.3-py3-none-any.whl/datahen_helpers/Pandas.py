...
def get_recent_export(scraper_name, exporter_name):
  ...
  archive_file_path = f"{tmpdir.name}/{recent_export['file_name']}"
  ...
  tar.extractall(f"{tmpdir.name}")
  export_file_path = f"{tmpdir.name}/{export_file_tar_path}"
  tar.close()

  if recent_export['exporter_type'] == 'csv':
    df = pd.read_csv(export_file_path)
  else:
    df = pd.read_json(export_file_path)
