...
class S3Walker(AbstractWalker):
    ...
    def _list_files(self, subdir):
        ...
        paginator = self.client.get_paginator('list_objects')
        page_iterator = paginator.paginate(Bucket=self.bucket, Prefix=full_prefix, Delimiter='')
        for page in page_iterator:
            if 'Contents' in page:
                contents = page['Contents']
                for content in contents:
    ...
    def _include_dir(self, dirpath):
        ...
        for idx, part in enumerate(dirpath.split(self.separator)):
            ...
            if self._include_dirs is not None:
                for pattern in self._include_dirs:
                    if idx + 1 <= len(pattern):
                        if not fnmatch.fnmatch(part, pattern[idx]):
                            return False
