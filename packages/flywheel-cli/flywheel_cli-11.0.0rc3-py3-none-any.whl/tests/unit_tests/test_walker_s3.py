...
def test_list_files_w_include_dir(mocked_boto3_pagination, mocked_urlparse):
    mocked_boto3_pagination([{'Contents': [
    ...
    mocked_urlparse((None, 'bucket', '/'))
    walker = S3Walker(fs_url, filter_dirs=['dir1*'])

    file_list = list(walker.list_files('/'))
    assert len(file_list) == 2
    assert file_list[0].name == 'dir10/file1.txt'
    assert file_list[1].name == 'dir11/file2.txt'
...
def test_include_dir():
    ...
    assert should_include('foo/bar/baz', include=['foo'])
    assert should_include('foo/bar/baz', include=['foo/bar'])
    assert should_include('foo/bar/baz', include=['foo/bar/baz'])
    assert not should_include('foo/bar/baz', include=['bar'])
    assert not should_include('foo/bar/baz', include=['baz'])
    assert not should_include('foo/.bar/baz', include=['foo'])
    assert should_include('foo/.bar/baz', ignore_dot_files=False, include=['foo'])
    assert not should_include('foo/bar/baz', include=['foo'], exclude=['baz'])
    assert not should_include('foo/bar/baz', exclude=['foo'])
    assert should_include('foo/bar/baz', exclude=['foo2'])
    assert should_include('foo10/bar/baz', include=['foo1*'])
    assert not should_include('foo20/bar/baz', include=['foo1*'])
