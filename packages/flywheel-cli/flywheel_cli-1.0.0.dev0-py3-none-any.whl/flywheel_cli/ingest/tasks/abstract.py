...
class AbstractTask(ABC):
    ...
    def execute(self):
        ...
        try:
            self._load_ingest()
            self._check_ingest_status()
            self._initialize()
            self._execute()
            self.ingest_db.finish_task(self.task.id)
            self._after_complete()
        except IngestAbortedError:
            self.ingest_db.cancel_task(self.task.id)
        except Exception as exc:  # pylint: disable=broad-except
            self.handle_task_failure(exc)
    ...
    def _check_ingest_status(self):
        """Check that the ingest status is not aborted.
        Raises:
            IngestAbortedError: If the ingest status is aborted
        """
        if self.ingest.status == schemas.IngestStatus.aborted:
            raise IngestAbortedError(self.ingest_db.ingest_id)


class IngestAbortedError(Exception):
    """Ingest aborted"""
    def __init__(self, ingest_id):
        super().__init__(f"Ingest {ingest_id} was aborted")
