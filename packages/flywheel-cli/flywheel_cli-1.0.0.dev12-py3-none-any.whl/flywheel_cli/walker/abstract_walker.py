...
class AbstractWalker(ABC):
    ...
    def list_files(self, subdir=None):
        ...
        return self._list_files(subdir)
    ...
    def _should_include_file(self, info):
        ...
        if self._exclude_files is not None and self.match(self._exclude_files, info.name):
            return False

        if self._include_files is not None and not self.match(self._include_files, info.name):
            return False
