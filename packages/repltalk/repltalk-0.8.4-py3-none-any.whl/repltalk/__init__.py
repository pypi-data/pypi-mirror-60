...
class User():
	...
	async def get_comments(self, count=30, order='new'):
		client = self.client
	...
	async def get_posts(self, count=30, order='new'):
		client = self.client
...
class Client():
	...
	async def _get_user_comments(self, user_id, count, order):
	...
	async def _get_user_posts(self, user_id, count, order):
		return await self.perform_graphql(
			'user',
