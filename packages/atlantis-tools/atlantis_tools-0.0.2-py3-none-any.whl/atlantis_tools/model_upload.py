...
def model_upload_zip(file_path="",**kwargs):
    ...
    with open(file_path,"rb") as f:
        model_upload(file=f,**kwargs)

def model_upload_dir(dir_path="",**kwargs):
    ...
    with zipfile.ZipFile(zipData,mode="w") as zf:
        for i in walkpath(dir_path):
            zf.write(os.path.abspath(os.path.join(dir_path,i)),zipfile.ZIP_DEFLATED)
    if zipData.tell()/1024/1024>32:
        raise Exception("After zipped but those files also bigger than 32mb!")
    zipData.seek(0,0)
    model_upload(file=zipData.getvalue(),**kwargs)
