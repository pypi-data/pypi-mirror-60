...
class CAResponseField(Field):
    ...
    def update_all_errors(self, company="", api_call=""):
        for err in self.errors:
            if company:
                err.company = company
            if api_call:
                err.api_call = api_call
        return self.errors
...
class CAResponseSubField(Field):
    ...
    def was_found(self, field, value):
        if field.name in self.found:
            ...
            self.error(Error(check_name="Duplicate {} URL".format(field.name),
...
class BiidsField(CAResponseSubField):
    def __init__(self, name):
        ...
        self.rules = [rule.has_non_empty_field(x) for x in self.fields]
...
class MainDomainField(CAResponseSubField):
    def __init__(self, name):
        ...
        self.rules = [rule.has_non_empty_field(self.domain)]
