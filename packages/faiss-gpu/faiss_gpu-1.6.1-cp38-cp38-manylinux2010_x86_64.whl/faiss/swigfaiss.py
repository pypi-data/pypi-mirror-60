...
class StandardGpuResources(GpuResources):
    ...
    def setCudaMallocWarning(self, b):
        return _swigfaiss.StandardGpuResources_setCudaMallocWarning(self, b)

    def initializeForDevice(self, device):
        return _swigfaiss.StandardGpuResources_initializeForDevice(self, device)

    def getBlasHandle(self, device):
        return _swigfaiss.StandardGpuResources_getBlasHandle(self, device)

    def getDefaultStream(self, device):
        return _swigfaiss.StandardGpuResources_getDefaultStream(self, device)

    def getAlternateStreams(self, device):
        return _swigfaiss.StandardGpuResources_getAlternateStreams(self, device)

    def getPinnedMemory(self):
        return _swigfaiss.StandardGpuResources_getPinnedMemory(self)

    def getAsyncCopyStream(self, device):
        return _swigfaiss.StandardGpuResources_getAsyncCopyStream(self, device)
...
HostPinned = _swigfaiss.HostPinned

def allocMemorySpaceV(space, p, size):
    return _swigfaiss.allocMemorySpaceV(space, p, size)

def freeMemorySpace(space, p):
    return _swigfaiss.freeMemorySpace(space, p)
...
class GpuIndexFlatConfig(GpuIndexConfig):
    ...
    useFloat16Accumulator = property(_swigfaiss.GpuIndexFlatConfig_useFloat16Accumulator_get, _swigfaiss.GpuIndexFlatConfig_useFloat16Accumulator_set)
...
class GpuIndexIVFPQ(GpuIndexIVF):
    ...
    def getListCodes(self, listId):
        return _swigfaiss.GpuIndexIVFPQ_getListCodes(self, listId)
...
class ToGpuCloner(Cloner, GpuClonerOptions):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr
    resources = property(_swigfaiss.ToGpuCloner_resources_get, _swigfaiss.ToGpuCloner_resources_set)
    device = property(_swigfaiss.ToGpuCloner_device_get, _swigfaiss.ToGpuCloner_device_set)

    def __init__(self, resources, device, options):
        _swigfaiss.ToGpuCloner_swiginit(self, _swigfaiss.new_ToGpuCloner(resources, device, options))
...
def index_cpu_to_gpu_multiple(resources, devices, index, options=None):
    return _swigfaiss.index_cpu_to_gpu_multiple(resources, devices, index, options)
