...
# 新建屏幕保存方法,保存的是截取的图形
def TurtleScreen_save(self,filename,size=None,margin=5,full=False):
    """ 截屏命令，能截精灵所在的窗口的屏幕或者电脑桌面全屏与指定矩形区域。
        filename:截屏后要保存的图像文件名,推荐用png。
...
class Sprite(Turtle):
    ...
    def draw_grid2(self,rows=8,cols=8,w_length=50,h_length=50):
       """
             按行数和列数及格子边长画格子的方法。
             rows：行数，cols：列数，w_length：格子宽度，h_length：格子高度
       """
       grid_width = cols * w_length
       grid_height = rows * h_length
       isdown = self.isdown()           # 记录先前落笔状态
       pos = self.pos()                 # 记录先前坐标
       ...
       self.addy(grid_height+h_length)  # 到左上角y
       ...
       # 下面只是记录每个格子中心点坐标返回它们 
       self.addx(-grid_width-w_length)    # 到左上角y
       startx = self.xcor() + w_length/2  # 起始中心点x
       starty = self.ycor() - h_length/2  # 起始中心点y
       cors = []
       for r in range(rows):
         rcors = []
         for c in range(cols):
