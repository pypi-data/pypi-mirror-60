...
class util:
    ...
    def getName(aData, filetype='title', check=False):
        partType = 'Beam-RC'
        ...
        if secType == "Rectangular Beam" or secType == 'T Beam':
            ...
            secType = 'Column'
        if check:
            if not('BH' in str(aData.file_name)):
                print(
                    '   Warning: Material and section title syntax incompatible with GESU Automation modules.')
