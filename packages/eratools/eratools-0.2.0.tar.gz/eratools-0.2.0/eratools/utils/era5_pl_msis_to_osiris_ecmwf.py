...
def load_era5_data(mjd_start, mjd_end):
    ...
    if t_start.date() == t_end.date():
        era5 = xr.open_dataset(os.path.join(os.environ['ERA5_DATA_DIR'], f'{t_start.year}', f'{t_start.month:02}',
                                            f'ERA5_pl_{t_start.year}{t_start.month:02}{t_start.day:02}.nc'))
    else:
        era5 = xr.open_mfdataset([os.path.join(os.environ['ERA5_DATA_DIR'], f'{t_start.year}', f'{t_start.month:02}',
                                  ...
                                  os.path.join(os.environ['ERA5_DATA_DIR'], f'{t_end.year}', f'{t_end.month:02}',
                                               f'ERA5_pl_{t_end.year}{t_end.month:02}{t_end.day:02}.nc')],
                                 concat_dim='time')
...
def era5_pres_temp_nd_for_point(time, lat, lon, alt_grid):
    ...
    pres = era5i.erapl.pressureonalt(lat, lon, time, alt_grid)
    temp = era5i.erapl.temperatureonalt(lat, lon, time, alt_grid)
    nd = pres / temp / 1.3806503e-19
    return temp, nd

for orbit in range(103127, 103600, 8):
    try:
        ...
        for scan, info in att.groupby('ScanNumber'):
            try:
                spect = open_level1_spectrograph(scan, valid=False, load_radiation_hits=False)
                print(f'{scan}')
                if era5 is None:
                ...
                msis = msise00.run(time=t.datetime, altkm=np.arange(0, 110.5, .5), glat=latitude,
                                   glon=longitude).squeeze()
                ...
                msis.temperature_era5.loc[{'alt_km': slice(70, None)}] = msis.Tn.loc[{'alt_km': slice(70, None)}]
                ...
                msis.nd_era5.loc[{'alt_km': slice(70, None)}] = msis.nd.loc[{'alt_km': slice(70, None)}]
                ...
                vd_info.write(
                    ((info.StartMJD.item(), info.EndMJD.item(), mjd, latitude, longitude, 0., 100000., int(scan), 221,
