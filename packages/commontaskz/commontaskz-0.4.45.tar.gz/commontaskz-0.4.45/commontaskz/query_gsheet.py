...
class QueryGSheet(Query):
    ...
    @staticmethod
    def prep(url, key, a_range, sheet_id, sheet_name):
        url = "{url}/{sheet_id}/values/{sheet_name}!{a_range}" \
    ...
    def run(self, url=None, key=None, a_range=None, sheet_id=None, sheet_name=None, collapse=True):
        ...
        response = self.query(full_url, key)
...
class MapIDs(QueryGSheet):
    def __init__(self, key_idx=None, value_idx=None, key=None, url=None, a_range=None, sheet_id=None, sheet_name=None, max_retries=3,
    ...
    @defaults_from_attrs('key_idx', 'value_idx', 'url', 'key', 'a_range', 'sheet_id', 'sheet_name')
    def run(self, ids, key_idx=None, value_idx=None, url=None, key=None, a_range=None, sheet_id=None, sheet_name=None):
        full_url = self.prep(url, key, a_range, sheet_id, sheet_name)
        response = self.query(full_url, key)
        result = self.process("", full_url, response)
