...
class MakePivotErrorTask(MakeErrorTask):
    ...
    @defaults_from_attrs('titles')
    def run(self, titles=None, errors=[], collapse="", aggregate="", flatten=False) -> bool:
        """
        :param flatten:
        :param aggregate:
        :param collapse:
        ...
        :return: bool
        """
        print("in alert file", errors)
        self.make_file_name(titles)
        print("errors still", errors)
        self.make_error_list(errors, flatten)
        print("in alert file", self.problems)
        if self.problems:
