import os, sys, io, errno, yaml, json, requests
...
class tdcoa():
    ...
    def __init__(self, approot='.', printlogs=True):
                        ...
                        for sql in sqls:
                            ...
                            if sql != '\n;\n\n':  # exclude null statements (only ; and newlines)
                                ...
                                if '/*{{loop:' in sql:
                                    ...
                                    self.log('no loop commands found')
    ...
    def __get_sqlcommands(self, sql):
        ...
        while cmdstart in sql:
            ...
            if len(cmdlst) ==2:
                ...
                self.log('special command found', '%s = %s' %(cmdlst[0].strip(), cmdlst[1].strip()))
