...
from zuper_commons.text import box, indent, side_by_side
...
def debug_print0(
    ...
    *,
    ...
    if not isinstance(x, str):
        if is_TypeLike(x):
            x = cast(TypeLike, x)
            return debug_print_typelike(
                x, dp_compact, dpa=dpa, opt=opt, prefix=prefix, args=args
            )
...
def debug_print_typelike(
    x: TypeLike, dp_compact, dpa, opt: DPOptions, prefix: str, args
) -> str:
