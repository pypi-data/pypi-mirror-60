...
from tensorflow_model_analysis.types_compat import Any, Dict, List, Optional, Text, Type, Tuple, Union
...
def _combine_with_default_metrics(
  ...
  def metrics_for_output(output_name):
  ...
  def default_metrics_for_output(output_name):
...
def _verify_and_update_sub_key(model_name, output_name,
                               sub_key,
                               metric):
...
class _CompilableMetricsAccumulator(object):
  ...
  def __init__(self, metric_counts):
  ...
  def add_input(self, output_index, label,
  ...
  def add_weights(self, output_index, metric_index,
                  weights):
  ...
  def get_weights(self, output_index,
                  metric_index):
...
class _CompilableMetricsCombiner(beam.CombineFn):
  ...
  def _process_batch(self, accumulator):
