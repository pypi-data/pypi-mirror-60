...
def counts_varchar2(schema, table, column):
    sql_script = f"select {column} as verdi , count({column}) as antall " \
                 f"from {schema}.{table} group by {column}  having count({column}) >4 order by count({column}) desc"

    return sql_script


def count_num_date(schema, table, column):
    sql_script = f"select {column} as verdi , count({column}) as antall " \
                 f"from {schema}.{table} group by {column}  having count({column}) >4 order by {column} desc"
