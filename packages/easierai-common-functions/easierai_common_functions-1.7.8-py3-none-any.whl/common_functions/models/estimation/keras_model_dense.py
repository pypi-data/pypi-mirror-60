...
class Predictor:
    ...
    def get_model(self, shape, num_forecasts, inner_activation='relu'):
        ...
        if tf.__version__.split(".")[0] == '1':
            model.compile(optimizer='adam',loss=self.loss_fn, metrics=['mae'], lr=self.learning_rate)
        else:
            model.compile(loss=self.loss_fn, metrics=['mae'], lr=self.learning_rate)
        model.summary()
        self.model = model
        return model
    ...
    def get_model_configurable(self, shape, num_forecasts, inner_activation='relu', num_dense_layers=1, num_units_per_layer=[64], loss='mae', metrics=['mse']):
        ...
        if tf.__version__.split(".")[0] == '1':
            model.compile(optimizer='adam',loss=loss, metrics=metrics, lr=self.learning_rate)
        else:
            model.compile(loss=loss, metrics=metrics, lr=self.learning_rate)
        model.summary()
        self.model = model
        return model
