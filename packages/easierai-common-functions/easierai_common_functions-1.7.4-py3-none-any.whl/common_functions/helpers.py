...
def save_model(eslib, id, metadata, dict={}, inference_type=constants.ESTIMATOR, _id=None, save_tflite=False, calibration_data=None, save_tpu=False):
    ...
    try:
        ...
        if save_tflite and calibration_data is not None:
            success = edge_toolkit.convert_model_lite(calibration_data=calibration_data, model_name=filename + '.' + constants.MODEL_EXTENSION, keras_model=dict[constants.MODEL_EXTENSION])
            if success:
        ...
        if save_tpu and calibration_data is not None:
            success = edge_toolkit.convert_model_tpu(calibration_data=calibration_data, model_name=filename + '.' + constants.MODEL_EXTENSION, keras_model=dict[constants.MODEL_EXTENSION])
            if success:
                uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.TPU_EXTENSION, id, config['ML']['algorithm'])
                if not uploaded:
                    _logger.error('Error on uploading file to min.io filesystem(' + filename + '.' + constants.TPU_EXTENSION + ') - ')
                    return ''
    except Exception as e:
